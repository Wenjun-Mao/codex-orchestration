...
export async function transitionWorkflowOperationClaim({
  ...
  let record;
  let expectedOperationKind;
  let expectedTerminalAt;
  if (visibleTaskRecord !== null) {
    const { validateVisibleTaskCreationRecord } = await import("./task-creation-v07.mjs");
    record = validateVisibleTaskCreationRecord(visibleTaskRecord);
    expectedOperationKind = "visible-task-creation";
    ...
    expectedTerminalAt = exactRejection ? record.resolution.recorded_at : null;
  } else {
    const { validateSubagentOperation } = await import("./subagent-operations-v07.mjs");
    record = validateSubagentOperation(subagentOperation);
    expectedOperationKind = "subagent-operation";
    ...
    expectedTerminalAt = exactRejection ? record.attempt.reconciled_at : null;
  }
  ...
  return withProcessLock({
    ...
    if (
      operation === undefined
      || operation.operation_kind !== expectedOperationKind
      || operation.operation_id !== record.operation_id
      || operation.terminal_at !== expectedTerminalAt
    ...
    const current = authority.journal.contract_claims.find(
      (entry) => entry.contract_id === record.contract_id,
    );
    if (!current) throw new CliError("Native operation has no workflow contract claim");
    const expectedState = expectedTerminalAt === null ? "started" : "terminal-no-object";
    const expectedClaim = validateContractClaim({
      ...current,
      state: expectedState,
      operation_kind: expectedOperationKind,
      operation_id: record.operation_id,
      started_at: operation.started_at,
      terminal_at: expectedTerminalAt,
    });
    const startedTransition = expectedState === "terminal-no-object"
      && current.state === "started"
      && current.operation_kind === expectedOperationKind
    ...
    if (stableStringify(current) !== stableStringify(expectedClaim)) {
      ...
      const updated = journalWithClaims(
        authority.journal,
        claims,
        expectedTerminalAt ?? operation.started_at,
      );
