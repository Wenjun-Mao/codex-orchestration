...
export const WORKFLOW_JOURNAL_KIND = "codex-flow-v07-workflow-journal";
export const WORKFLOW_CONTRACT_CLAIM_KIND = "codex-flow-v07-workflow-contract-claim";
...
function validateContractClaim(value, label = "workflow contract claim") {
  ...
  if (value.schema_version !== WORKFLOW_JOURNAL_SCHEMA_VERSION || value.kind !== WORKFLOW_CONTRACT_CLAIM_KIND) {
    throw new CliError(`${label} is not a supported v0.7 workflow contract claim`);
  }
  const claim = {
  ...
  if (claim.state === "terminal-no-object" && (
    !(
      (claim.execution_kind === "task-thread" && claim.operation_kind === "visible-task-creation")
...
export function validateWorkflowJournal(value) {
  ...
  if (value.schema_version !== WORKFLOW_JOURNAL_SCHEMA_VERSION || value.kind !== WORKFLOW_JOURNAL_KIND) {
    throw new CliError("Unsupported v0.7 workflow journal");
  }
...
async function readPersistedJournal(stateRoot, runId, planId) {
  ...
  if (raw === null) throw new CliError(`Unknown v0.7 workflow journal: ${runId}/${planId}`);
...
async function scanStartedOperations(stateRoot, authority) {
  ...
  const validateVisibleTaskCreationRecord = visibleRecords.length === 0
    ? null
    : (await import("./task-creation-v07.mjs")).validateVisibleTaskCreationRecord;
  for (const { path, raw } of visibleRecords) {
    ...
    operations.set(record.contract_id, {
      ...
      terminal_at: record.status === "not-created"
  ...
  const validateSubagentOperation = subagentRecords.length === 0
    ? null
    : (await import("./subagent-operations-v07.mjs")).validateSubagentOperation;
...
async function operationTransitionDescriptor({ visibleTaskRecord = null, subagentOperation = null }) {
  if ((visibleTaskRecord === null) === (subagentOperation === null)) {
    throw new CliError("Workflow operation transition requires exactly one native operation record");
  }
  if (visibleTaskRecord !== null) {
    const { validateVisibleTaskCreationRecord } = await import("./task-creation-v07.mjs");
    const record = validateVisibleTaskCreationRecord(visibleTaskRecord);
    const exactRejection = record.status === "not-created"
      && record.resolution.reason_code === "selector-rejected-before-task-identity"
  ...
  const { validateSubagentOperation } = await import("./subagent-operations-v07.mjs");
...
function expectedOperationClaim(authority, descriptor) {
  ...
  return {
    ...
    expectedClaim: validateContractClaim({
      ...
      operation_id: descriptor.record.operation_id,
...
export async function commitWorkflowOperationPreparation({
  ...
  const descriptor = await operationTransitionDescriptor({ visibleTaskRecord, subagentOperation });
...
export async function assertWorkflowPreDispatchOrphanRecoverable({
  ...
  if (!["visible-task-creation", "subagent-operation"].includes(operationKind)) {
