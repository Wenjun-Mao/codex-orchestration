...
import { serialIntegrationStatus } from "./integration-v06.mjs";
import { taskReleaseStatus } from "./release-lifecycle.mjs";

const ARCHIVE_KIND = "codex-flow-v06-task-archive-operation";
...
function validateDispositionClaim(value) {
  ...
  if (value.schema_version !== 1 || value.kind !== "codex-flow-v06-archive-disposition-claim") {
    throw new CliError("Invalid archive disposition claim");
  }
  return {
    schema_version: 1,
    kind: "codex-flow-v06-archive-disposition-claim",
...
function validateHostIntent(value) {
  ...
  return {
    ...
    thread_id: requireText(value.thread_id, "host_intent.thread_id", { max: 128, safeId: true }),
...
function validateGitResolution(value) {
  requireExactFields(value, {
    required: ["kind", "integration_id", "verification_digest"],
...
function validateSetter(value) {
  ...
  return {
    ...
    reconciled_at: requireText(value.reconciled_at, "setter.reconciled_at", { max: 64 }),
  };
}
...
function validateArchiveObservation(value) {
  ...
  return {
    ...
    observed_at: requireText(value.observed_at, "archive observation.observed_at", { max: 64 }),
  };
}
...
export function validateArchiveOperation(value) {
  requireExactFields(value, {
    required: [
      "schema_version", "kind", "archive_id", "run_id", "plan_id", "revision_id",
      "task_id", "task_contract_digest", "release_id", "callback_id", "disposition_id",
  ...
  if (value.schema_version !== 1 || value.kind !== ARCHIVE_KIND) {
    throw new CliError("Invalid v0.6 task archive operation schema");
  }
  const record = {
    schema_version: 1,
    kind: ARCHIVE_KIND,
...
async function inspectWorktree(stateRoot, requested, gitOutcome) {
  requireExactFields(requested, {
    required: ["management", "path"],
  }, "Archive worktree request");
  ...
  if (management === "none") {
    if (requested.path !== null) throw new CliError("Unmanaged archive worktree path must be null");
    return { management, path: null, prepared_state: "not-applicable" };
  }
  const requestedPath = requireText(requested.path, "worktree.path", { max: 2048 });
  if (!isAbsolute(requestedPath)) throw new CliError("Host-managed worktree path must be absolute");
  const canonical = await realpath(requestedPath).catch(() => null);
  if (canonical === null) {
    if (await pathPresent(requestedPath)) throw new CliError("Host-managed worktree cannot be resolved");
    return { management, path: resolve(requestedPath), prepared_state: "absent" };
  }
...
async function resolvedAuthority(stateRoot, dispositionId) {
  ...
  if (
    ...
    || callback.receipt.task_contract_digest !== disposition.task_contract_digest
  ...
  if (
    ...
    || release.release_id !== callback.receipt.release_id
    || release.run_id !== callback.receipt.run_id
    || release.plan_id !== callback.receipt.plan_id
    || release.revision_id !== callback.receipt.revision_id
    || release.task_id !== callback.receipt.task_id
    || release.task_contract_digest !== callback.receipt.task_contract_digest
    || release.operation_id !== callback.receipt.operation_id
    || release.runtime_digest !== callback.receipt.runtime_digest
    || release.config_digest !== callback.receipt.config_digest
    || release.ready_thread_id !== release.acceptance.executor_thread_id
...
export async function prepareTaskArchive({
  stateRoot,
  dispositionId,
  taskObservation,
  hostId = null,
  ...
  const causal = {
    ...
    revision_id: authority.disposition.revision_id,
    task_id: authority.disposition.task_id,
    task_contract_digest: authority.disposition.task_contract_digest,
...
export async function reconcileTaskArchive({
  ...
  return withProcessLock({
    ...
    const timestamp = new Date(now).toISOString();
    const setter = current.setter ?? { outcome, reconciled_at: timestamp };
