...
function archiveWorktreeAuthority(creation) {
  ...
  if (observed === null || observed.mode !== "host-worktree" || observed.path === null) {
    throw new CliError(
      "Host-worktree task archive requires the exact persisted host-observed worktree path",
      73,
    );
  }
  return { management: "host-managed", path: observed.path };
}
