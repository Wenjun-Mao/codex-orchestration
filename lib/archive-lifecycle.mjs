...
function validateTaskObservation(value, label) {
  ...
  requireExactFields(value, {
    required: source === PRIVATE_ARCHIVE_OBSERVATION_SOURCE
      ? [
  ...
  const result = {
    ...
    source: requireEnum(
      value.source,
      ["host-observed", PRIVATE_ARCHIVE_OBSERVATION_SOURCE],
      `${label}.source`,
    ),
  ...
  if (result.source === PRIVATE_ARCHIVE_OBSERVATION_SOURCE) {
    const privateObservation = validatePrivateArchiveObservation(
      value.private_observation,
      `${label}.private_observation`,
    );
    if (
      privateObservation.thread_id !== result.thread_id
      || result.active_visible !== false
      || result.archived_visible !== true
    ) throw new CliError(`${label} private provenance does not prove the exact archived task`);
    result.private_observation = privateObservation;
  }
  return result;
}
...
function validateArchiveObservation(value) {
  ...
  if (
    observation.task.source === PRIVATE_ARCHIVE_OBSERVATION_SOURCE
    && observation.task.private_observation.observed_at !== observation.observed_at
  ) throw new CliError("Archive private observation timestamp does not match its private provenance");
  return observation;
}
...
export async function observePrivateTaskArchive({
  stateRoot,
  archiveId,
  codexHome,
  now = Date.now(),
  ...
  if (
    current.setter === null
    || !["accepted", "ambiguous"].includes(current.setter.outcome)
    || ![
      "accepted-awaiting-observation",
      "ambiguous",
      "archived-awaiting-worktree-reclamation",
    ...
    throw new CliError(
      "Private archive observation requires a dispatched archive with an unresolved or observed postcondition",
      73,
    );
  }
  const privateObservation = await observeCodexAppPrivateArchive({
    threadId: current.task.thread_id,
    codexHome,
    now,
  });
  return {
    schema_version: 1,
    kind: "codex-flow-v07-private-task-archive-observation-result",
    run_id: current.run_id,
    archive_id: current.archive_id,
    attempt_id: current.host_intent.attempt_id,
    private_host_surface: true,
    observation: {
      execution_kind: "task-thread",
      thread_id: current.task.thread_id,
...
export async function reconcileTaskArchive({
  ...
  return withProcessLock({
    ...
    const worktreeState = await reconciledWorktreeState(current.worktree);
    const observedAt = taskObservation.private_observation?.observed_at ?? reconciledAt;
