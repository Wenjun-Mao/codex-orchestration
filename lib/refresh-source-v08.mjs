...
import { validateIntegrationRecordV07 } from "./integration-v07.mjs";
...
import { validateWorkflowJournal } from "./workflow-journal-v07.mjs";
import {
  validateGeneratedTaskContract,
  validateWorkflowPlanRevision,
} from "./workflow-plan.mjs";
import { exactV078SourceApi } from "./refresh-v078-bridge.mjs";
...
function validateNamespacePackage(namespace, packageVersion) {
  if (namespace === "v0.7.8" && packageVersion === "0.7.8") return "exact-v0.7.8-adapter";
...
async function locateRefreshSourceSnapshot({ commonDir, namespace, runId }) {
  ...
  const contextBundle = validateRuntimeBundle(context.bundle, "refresh source bootstrap bundle");
  const manifestPath = resolve(root, "runtimes", contextBundle.bundle_sha256, "bundle.json");
  const manifest = validateRuntimeBundle(await readJson(manifestPath, { guardRoot: common }));
  ...
  await loadRuntimeBundleDirectory({ bundleRoot, bundle: manifest });
...
function sourceTaskState({
  ...
  const creation = contractId === null
    ? null
    : creations.find((entry) => entry.contract_id === contractId) ?? null;
  const release = contractId === null
    ? null
    : releases.find((entry) => entry.contract_id === contractId) ?? null;
...
async function deriveRefreshSourceAuthority(
  ...
  const allRecords = await Promise.all([
    boundedJsonRecords(resolve(root, "visible-task-creations", "records"), sourceApi.validateVisibleTaskCreationRecord, common, sourceApi.readJson),
    boundedJsonRecords(resolve(root, "releases", "records"), sourceApi.validateReleaseRecord, common, sourceApi.readJson),
    boundedJsonRecords(resolve(root, "integration-lifecycle", "records"), sourceApi.validateIntegrationRecordV07, common, sourceApi.readJson),
  ...
  const [creations, releases, integrations, dispositions, archives, subagents] = allRecords
...
export async function loadRefreshSourceAuthority(options) {
  ...
  if (located.adapter === "exact-v0.7.8-adapter") {
    const source = await deriveRefreshSourceAuthority(
      options,
      await exactV078SourceApi(located.bundle_root),
    );
    if (
      source.runtime.manifest.bundle_sha256 !== located.manifest.bundle_sha256
      || source.run.runtime_id !== located.locator.runtime_id
    ) throw new CliError("Exact v0.7.8 adapter authority drifted during source loading", 73);
    return source;
  }
...
function sourceCleanupPlan({ cliPath, repositoryRoot }, runId) {
  ...
  if (
    plan.kind !== "codex-flow-v07-cleanup-plan"
    || plan.run_id !== safeRunId
    || plan.mutation_performed !== false
    || !Array.isArray(plan.items)
    || !Array.isArray(plan.blocking_operation_ids)
...
function assertClosedRunCleanupComplete(plan, runId) {
  if (
    plan.run_status !== "closed"
    || plan.counts.close_blocked !== 0
    || plan.counts.cleanup_required !== 0
    || plan.blocking_operation_ids.length !== 0
...
export async function assertOtherRefreshSourceRunsSafe(source) {
  for (const run of Object.values(source.lifecycle.runs)) {
    ...
    if (run.status !== "closed") {
      throw new CliError(`Non-selected source run must be independently closed before refresh: ${run.run_id}`, 73);
    }
    ...
    assertClosedRunCleanupComplete(sourceCleanupPlan({
      cliPath: located.cli_path,
...
export async function assertRefreshNamespaceRemovalSafe({ source, handoff }) {
  ...
  for (const item of plan.items) {
    const discard = discarded.get(item.operation_id) ?? null;
    if (discard !== null) {
      if (
        item.cleanup_required !== false
  ...
  if (plan.blocking_operation_ids.some((operationId) => !allowedBlocking.has(operationId))) {
...
export function refreshSourceSummary(source) {
  return {
    ...
    executor_tasks: source.task_states.map((entry) => ({
      ...
      operation_id: entry.creation?.operation_id ?? entry.subagent?.operation_id ?? null,
      ready_thread_id: entry.creation?.ready?.thread_id ?? null,
      ...
      creation_status: entry.creation?.status ?? null,
