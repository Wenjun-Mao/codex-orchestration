...
import { sourceManagedFiles } from "./managed.mjs";

export const V06_RUNTIME_SCHEMA_VERSION = 1;
export const V06_RUNTIME_KIND = "codex-flow-v06-runtime-context";
export const V06_RUNTIME_BUNDLE_KIND = "codex-flow-v06-runtime-bundle";
export const V06_RUNTIME_DIRECTORY = "v0.6.1";
...
export function validateRuntimeBundle(value, label = "runtime bundle") {
  ...
  if (value.schema_version !== V06_RUNTIME_SCHEMA_VERSION) {
    throw new CliError(`${label}.schema_version must be ${V06_RUNTIME_SCHEMA_VERSION}`);
  }
  if (value.kind !== V06_RUNTIME_BUNDLE_KIND) {
    throw new CliError(`${label}.kind must be ${V06_RUNTIME_BUNDLE_KIND}`);
  }
...
export async function loadRuntimeBundleSource({ packageRoot }) {
  ...
  const managed = await sourceManagedFiles(root);
...
export function runtimeSnapshotRoot(gitCommonDirectory, bundleSha256) {
  ...
  return join(v06RuntimeRoot(gitCommonDirectory), "runtimes", digest);
}

export function runtimeContextPath(gitCommonDirectory, runtimeId) {
  const digest = validateDigest(runtimeId, "runtimeId");
  return join(v06RuntimeRoot(gitCommonDirectory), "contexts", `${digest}.json`);
}
...
async function readOptionalJson(path, guardRoot) {
  return readJson(path, { allowMissing: true, guardRoot });
}

export async function assertNoTrackedLegacyAuthority(repositoryRoot) {
  const root = validateAbsolutePath(repositoryRoot, "repositoryRoot");
  await assertNoSymlinkComponents(root, root, "Repository root");
  const authorityRoot = join(root, ".codex", "orchestration");
  const versionPath = join(authorityRoot, "version.json");
  const projectPath = join(authorityRoot, "project.json");
  const version = await readOptionalJson(versionPath, root);
  if (version !== null) {
    const packageVersion = typeof version?.package_version === "string"
      ? version.package_version
      : "unclassified";
    throw new CliError(
      `Tracked Codex Flow authority ${packageVersion} must be explicitly retired before v0.6 activation or adoption`,
    );
  }
  const project = await readOptionalJson(projectPath, root);
  if (project !== null) {
    throw new CliError(
      "Unclassified tracked Codex Flow authority must be explicitly retired before v0.6 activation or adoption",
    );
  }
  let entries;
  try {
    entries = await readdir(authorityRoot, { withFileTypes: true });
  } catch (error) {
    if (error?.code === "ENOENT") return;
    throw error;
  }
  if (entries.some((entry) => entry.name !== "v0.6")) {
    throw new CliError(
      "Unclassified tracked Codex Flow files must be explicitly retired before v0.6 activation or adoption",
    );
  }
}
...
export async function acquireRuntimeContext({ gitCommonDirectory, context, bundleSource }) {
  ...
  await assertNoTrackedLegacyAuthority(runtime.repository.root);
