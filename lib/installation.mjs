...
function desiredProjectConfig(gitRoot, existingConfig, agentsIntegration, options, conflicts) {
  ...
  const base = existingConfig ?? defaultProjectConfig(gitRoot, {
    ...
    agentsIntegration,
    ordinaryCompletionAuthority: options.ordinaryCompletionAuthority,
  });
  if (
    existingConfig?.ordinary_completion_authority === "legacy-mixed"
    && options.ordinaryCompletionAuthority === undefined
  ) {
    conflicts.push(conflict(
      "callback-authority-migration",
      "Legacy callback delivery mixed queue notifications with monitor integration; choose --callback-authority journal-monitor",
      ".codex/orchestration/project.json",
    ));
  }
  return validateProjectConfig({
    ...
    ordinary_completion_authority: options.ordinaryCompletionAuthority
      ?? (existingConfig?.ordinary_completion_authority === "legacy-mixed"
        ? "journal-monitor"
        : base.ordinary_completion_authority),
...
async function buildInstallationPlan(options) {
  ...
  const existingConfig = rawConfig ? validateProjectConfig(rawConfig, { allowLegacy: true }) : null;
