...
import { validateCallbackRecord } from "./callbacks.mjs";
...
import { validateUrgentSignalRecord } from "./urgent-signals.mjs";
import {
...
} from "./verifications.mjs";

export const RUN_CLOSURE_AUDIT_SCHEMA_VERSION = 1;
export const RUN_CLOSURE_AUDIT_KIND = "codex-flow-v09-run-closure-audit";
...
const BLOCKER_CODES = [
  ...
  "task-launch-in-flight",
  "task-launch-ambiguous",
  "task-launch-session-blocked",
  "selector-replan-required",
  "selector-replan-exhausted",
  "subagent-incomplete",
  "coordinator-work-incomplete",
  "committed-write-scope",
  "callback-missing",
  "callback-unconsumed",
  "disposition-missing",
...
function validateRepositoryEvidence(value) {
  ...
  const expectedSource = oneOf(
    ...
    ["activation-baseline", "combined-verification", "coordinator-verification"],
  ...
  if (expectedVerificationId !== null && !(
    VERIFICATION_ID.test(expectedVerificationId)
    || /^coordinator-verification-v1-[0-9a-f]{64}$/.test(expectedVerificationId)
  )) {
    throw new CliError("repository.expected_verification_id must be a workflow verification ID");
  }
  if ((expectedSource === "activation-baseline") !== (expectedVerificationId === null)) {
...
export function validateRunClosureAudit(value) {
  ...
  ) throw new CliError("Invalid v0.9 run-closure audit schema");
...
async function resolveRoots({ gitCommonDirectory = null, stateRoot = null }) {
  ...
  const expectedStateRoot = requestedStateRoot ?? runtimeRoot(commonDir);
  if (
    requestedStateRoot !== null
    && dirname(requestedStateRoot) !== resolve(commonDir, "codex-flow")
  ) {
    throw new CliError("stateRoot is not an exact versioned Git-common runtime namespace");
  }
  if (
...
function relativeLocator(stateRoot, path) {
  ...
  if (location === "" || location === ".." || location.startsWith(`..${sep}`)) {
    throw new CliError("Run-closure audit source escapes the v0.9 state root");
  }
...
function terminalCandidateOrder(left, right) {
  const completed = Date.parse(left.completedAt) - Date.parse(right.completedAt);
  return completed || left.verificationId.localeCompare(right.verificationId);
}

async function deriveRepositoryEvidence({ context, commonDir, authoritativePassVerifications, coordinatorWorks }) {
  ...
  // A no-change verification proves the executor's terminal worktree, not the
  // coordinator checkout. Only an integration-scoped verification can advance
  // the coordinator's expected Git anchor.
  const integrationVerifications = [...authoritativePassVerifications.values()]
    .filter((verification) => verification.integration_scope !== null)
    .sort(verificationOrder);
  const completedCoordinatorWorks = coordinatorWorks
    .filter((record) => record.state === "completed")
    .sort((left, right) => Date.parse(left.completed_at) - Date.parse(right.completed_at));
  const terminalCandidates = [
    ...completedCoordinatorWorks.map((record) => ({
      source: "coordinator-verification",
      verificationId: record.verification.verification_id,
      root: record.baseline.root,
      commonDir: record.common_dir,
      revision: record.result.final_revision,
      branch: normalizedBranch(record.baseline.branch),
      completedAt: record.completed_at,
    })),
    ...integrationVerifications.map((verification) => ({
      source: "combined-verification",
      verificationId: verification.verification_id,
      root: verification.repository.root,
      commonDir: verification.repository.common_dir,
      revision: verification.repository.completed_revision,
      branch: verification.repository.completed_branch,
      completedAt: verification.completed_at,
    })),
  ];
  for (const candidate of terminalCandidates) {
    candidate.root = await realpath(candidate.root).catch(() => candidate.root);
    candidate.commonDir = await realpath(candidate.commonDir).catch(() => candidate.commonDir);
  }
  // Multiple records may prove one Git fact, but an older proof cannot make a
  // rollback terminal after another authenticated result advanced the graph.
  ...
  for (const candidate of terminalCandidates) {
    ...
    if (existing === undefined || terminalCandidateOrder(existing, candidate) < 0) {
      factsByIdentity.set(identity, candidate);
    }
  }
  const terminalFacts = [...factsByIdentity.values()];
  const maximalFacts = terminalFacts.filter((candidate) => !terminalFacts.some((other) => (
    other.revision !== candidate.revision
    && gitIsAncestor(root, candidate.revision, other.revision)
  ))).sort(terminalCandidateOrder);
  const terminalConflict = maximalFacts.length > 1;
  const expected = maximalFacts.at(-1) ?? {
  ...
  return {
    evidence: validateRepositoryEvidence({
      root,
...
function verificationMatchesDisposition(verification, disposition) {
  const expected = {
    ...
    launch_id: disposition.launch_id,
  };
...
async function deriveClosureEvidence({ commonDir, stateRoot, runId }) {
  ...
  const sourceRecords = [];
  const lifecyclePath = resolve(stateRoot, "runs", "lifecycle.json");
  const lifecycle = validateRunLifecycleState(await readJson(lifecyclePath, { guardRoot: commonDir }));
  const run = lifecycle.runs[runId];
  if (run === undefined) throw new CliError(`Unknown v0.9 run: ${runId}`, 73);
  if (run.status !== "active") throw new CliError(`v0.9 run is not active: ${runId}`);
  ...
  const runtimePath = resolve(stateRoot, "contexts", `${run.runtime_id}.json`);
  const runtime = {
    context: validateRuntimeContext(await readJson(runtimePath, { guardRoot: commonDir })),
    path: runtimePath,
  };
  ...
  if (rawWorkflowJournal === null) {
    ...
    for (const task of currentRevision.tasks) {
      const covered = claimEntries.some((entry) => (
        ...
        && ["current", "started", "completed", "terminal-no-object"].includes(entry.claim.state)
      ));
      if (!covered) {
        addBlocker(blockers, {
  ...
  const launches = await scan({
    directory: resolve(stateRoot, "task-launches", "records"),
    recordKind: "task-launch",
    selectRunId: (raw) => raw?.run_id,
    validate: validateTaskLaunchRecord,
    selectId: (record) => record.launch_id,
  });
  ...
  const coordinatorWorks = await scan({
    directory: resolve(stateRoot, "coordinator-work", "records"),
    recordKind: "coordinator-work",
    selectRunId: (raw) => raw?.run_id,
    validate: validateCoordinatorWorkRecord,
    selectId: (record) => record.local_work_id,
  });
  const callbacks = await scan({
    directory: resolve(stateRoot, "callbacks", "journal"),
    recordKind: "callback",
    selectRunId: (raw) => raw?.receipt?.run_id,
    validate: validateCallbackRecord,
  ...
  const integrations = await scan({
    ...
    selectRunId: (raw) => raw?.run_id,
    validate: validateIntegrationRecord,
  ...
  const verifications = await scan({
    ...
    validate: validateVerificationRecord,
  ...
  const urgentSignals = await scan({
    ...
    validate: validateUrgentSignalRecord,
  ...
  const assertRecordAuthority = (record, label, canonical = canonicalFromRecord(record)) => {
    ...
    if (!entry) {
      addBlocker(blockers, {
        ...
        authority_id: record.launch_id ?? record.operation_id ?? record.callback_id
  ...
  for (const record of launches) assertRecordAuthority(record, "task-launch");
  for (const record of subagents) assertRecordAuthority(record, "subagent-operation");
  for (const record of coordinatorWorks) assertRecordAuthority(record, "coordinator-work");
  ...
  for (const record of urgentSignals) {
    const matchingLaunches = launches.filter((launch) => (
      ...
      && launch.start_claim?.executor_thread_id === record.signal.executor_id
      && launch.coordinator_binding.lineage_id === record.signal.recipient.lineage_id
    ));
    if (matchingLaunches.length !== 1) {
      addBlocker(blockers, {
        code: matchingLaunches.length === 0 ? "orphaned-authority" : "conflicting-authority",
        authority_kind: "urgent-signal",
        authority_id: record.urgent_id,
        task_id: null,
        detail: matchingLaunches.length === 0
          ? "The urgent signal has no exact active task-launch executor authority."
          : "The urgent signal executor matches multiple active task launches.",
      });
    }
    if (!["consumed", "superseded", "expired"].includes(record.state)) {
      addBlocker(blockers, {
        ...
        authority_kind: "urgent-signal",
        authority_id: record.urgent_id,
        task_id: matchingLaunches[0]?.task_id ?? null,
        detail: `The urgent signal remains ${record.state} and requires coordinator disposition.`,
      });
    }
  }

  const launchById = new Map(launches.map((record) => [record.launch_id, record]));
  const subagentById = new Map(subagents.map((record) => [record.operation_id, record]));
  const coordinatorWorkById = new Map(coordinatorWorks.map((record) => [record.local_work_id, record]));
  const callbacksByLaunch = groupBy(callbacks, (record) => record.receipt.launch_id);
  const callbackById = new Map(callbacks.map((record) => [record.callback_id, record]));
  const dispositionsByLaunch = groupBy(dispositions, (record) => record.launch_id);
  ...
  const inspectDisposition = (disposition, callback = null) => {
    ...
    if (disposition.state === "completed") {
      ...
      if (archive === null) {
        ...
        const archivedLaunch = launchById.get(disposition.launch_id) ?? null;
        const archivedCallback = callbackById.get(disposition.callback_id) ?? null;
        const archiveChain = {
          launch: archivedLaunch !== null
            && archivedLaunch.status === "active"
            && archivedLaunch.start_claim?.executor_thread_id === disposition.executor_thread_id
            && archive.launch_id === disposition.launch_id,
          ...
          callback: archivedCallback !== null
            && archivedCallback.receipt.launch_id === disposition.launch_id
        ...
        if (archive.state !== "completed") {
          addBlocker(blockers, {
            ...
            task_id: disposition.task_id,
            detail: archive.state === "archived-awaiting-worktree-reclamation"
              ? "The task is archived while its host-managed worktree reclamation remains pending."
              : `The visible task archive remains ${archive.state}.`,
          });
        }
      }
    }
  ...
  for (const entry of claimEntries.filter((candidate) => (
    ["started", "completed", "terminal-no-object"].includes(candidate.claim.state)
  ))) {
    ...
    if (claim.state === "terminal-no-object") {
      const operation = claim.execution_kind === "subagent"
        ? subagentById.get(claim.operation_id)
        : launchById.get(claim.operation_id);
      if (!operation) {
        addBlocker(blockers, {
          ...
          detail: "The terminal-no-object claim has no persisted native operation.",
        });
        continue;
      }
      if (claim.execution_kind === "subagent") seen.subagent.add(operation.operation_id);
      else seen.launch.add(operation.launch_id);
      if (claim.superseded_by_revision_digest === null) {
        const alreadyReplanned = claimEntries.some((candidate) => (
          candidate.claim.task_id === claim.task_id
          && candidate.claim.claim_id !== claim.claim_id
          && candidate.claim.superseded_by_revision_digest !== null
        ));
        addBlocker(blockers, {
          code: alreadyReplanned ? "selector-replan-exhausted" : "selector-replan-required",
          authority_kind: claim.execution_kind === "subagent"
            ? "subagent-operation"
            : "task-launch",
          authority_id: claim.operation_id,
          task_id: claim.task_id,
          detail: alreadyReplanned
            ? "The replacement selector was rejected after the task consumed its single replan child."
            : "The selector was rejected before native identity; admit one exact child revision or pause.",
        });
      }
      continue;
    }
    if (claim.execution_kind === "coordinator") {
      const operation = coordinatorWorkById.get(claim.operation_id);
      if (!operation) {
        addBlocker(blockers, {
          code: "native-operation-missing",
          authority_kind: "claim",
          authority_id: claim.claim_id,
          task_id: claim.task_id,
          detail: "The coordinator-work claim has no persisted local operation.",
        });
        continue;
      }
      seen.coordinatorWork.add(operation.local_work_id);
      if (claim.state !== "completed" || operation.state !== "completed") {
        addBlocker(blockers, {
          code: "coordinator-work-incomplete",
          authority_kind: "coordinator-work",
          authority_id: operation.local_work_id,
          task_id: operation.task_id,
          detail: "The coordinator-owned work has not completed with passing verification.",
        });
      }
      continue;
    }
    ...
    const launch = launchById.get(claim.operation_id);
    if (!launch) {
      addBlocker(blockers, {
        code: "native-operation-missing",
        authority_kind: "claim",
        authority_id: claim.claim_id,
        task_id: claim.task_id,
        detail: "The started visible-task claim has no persisted task launch.",
      });
      continue;
    }
    seen.launch.add(launch.launch_id);

    if (["prepared", "attempting", "awaiting-start"].includes(launch.status)) {
      addBlocker(blockers, {
        code: "task-launch-in-flight",
        authority_kind: "task-launch",
        authority_id: launch.launch_id,
        task_id: launch.task_id,
        detail: `The visible task launch remains ${launch.status}.`,
      });
      continue;
    }
    if (launch.status === "ambiguous") {
      addBlocker(blockers, {
        code: "task-launch-ambiguous",
        authority_kind: "task-launch",
        authority_id: launch.launch_id,
        task_id: launch.task_id,
        detail: "The visible task launch has unresolved host identity ambiguity.",
      });
      continue;
    }
    if (launch.status === "session-blocked") {
      addBlocker(blockers, {
        code: "task-launch-session-blocked",
        authority_kind: "task-launch",
        authority_id: launch.launch_id,
        task_id: launch.task_id,
        detail: "The visible task launch is blocked by its host session and is not terminal.",
      });
      continue;
    }
    if (launch.status !== "active" || launch.git_activation?.state !== "completed") {
      throw new CliError("Started task launch lacks completed executor activation", 73);
    }

    const callback = only(callbacksByLaunch.get(launch.launch_id) ?? [], blockers, {
      authorityKind: "callback",
      authorityId: launch.launch_id,
      taskId: launch.task_id,
      detail: "The active task launch has multiple terminal callback records.",
    });
    if (callback === null) {
      addBlocker(blockers, {
        ...
        authority_id: launch.launch_id,
        task_id: launch.task_id,
        detail: "The active task launch has no terminal callback.",
      });
      continue;
    }
    seen.callback.add(callback.callback_id);
    if (
      callback.receipt.launch_id !== launch.launch_id
      || callback.receipt.executor_thread_id !== launch.start_claim.executor_thread_id
    ) throw new CliError("Terminal callback does not match its active task launch", 73);
  ...
  for (const record of launches) {
    if (!seen.launch.has(record.launch_id)) {
      orphan(record, "task-launch", record.launch_id, record.task_id);
    }
  }
  ...
  for (const record of coordinatorWorks) {
    if (!seen.coordinatorWork.has(record.local_work_id)) {
      orphan(record, "coordinator-work", record.local_work_id, record.task_id);
    }
  }
  for (const record of callbacks) {
    ...
    if (!seen.archive.has(record.archive_id)) {
      if (record.state !== "completed") {
        addBlocker(blockers, {
          ...
          task_id: record.task_id,
          detail: record.state === "archived-awaiting-worktree-reclamation"
            ? "An unreferenced archived task still awaits host-managed worktree reclamation."
            : `An unreferenced task archive remains ${record.state}.`,
        });
      }
  ...
  const cleanupEvidencePlan = await cleanupPlan({ stateRoot, runId: run.run_id });
  for (const item of cleanupEvidencePlan.items.filter((entry) => entry.close_blocked)) {
    addBlocker(blockers, {
      code: "cleanup-unresolved",
      authority_kind: "cleanup-plan",
      authority_id: cleanupEvidencePlan.plan_id,
      task_id: item.task_id,
      detail: `Executor branch ${item.branch} remains unresolved: ${item.reason_codes.join(", ")}.`.slice(0, 512),
    });
  }
  for (const fence of cleanupEvidencePlan.unbound_branch_fences.filter((entry) => entry.close_blocked)) {
    addBlocker(blockers, {
      code: "cleanup-unresolved",
      authority_kind: "cleanup-plan",
      authority_id: cleanupEvidencePlan.plan_id,
  ...
  const cleanup = validateCleanupEvidence({
    plan_id: cleanupEvidencePlan.plan_id,
    mutation_performed: cleanupEvidencePlan.mutation_performed,
    counts: {
      host_worktree_tasks: cleanupEvidencePlan.counts.host_worktree_tasks,
      cleanup_required: cleanupEvidencePlan.counts.cleanup_required,
      cleanup_candidates: cleanupEvidencePlan.counts.cleanup_candidates,
      close_blocked: cleanupEvidencePlan.counts.close_blocked,
    },
  });
  ...
  const counts = {
    ...
    task_launches: launches.length,
    subagent_operations: subagents.length,
    coordinator_work: coordinatorWorks.length,
  ...
  const repositoryResult = await deriveRepositoryEvidence({
    context: runtime.context,
    commonDir,
    authoritativePassVerifications,
    coordinatorWorks,
  });
  const repository = repositoryResult.evidence;
  if (repositoryResult.terminalConflict) {
    addBlocker(blockers, {
      code: "conflicting-authority",
      authority_kind: "run",
      authority_id: run.run_id,
      task_id: null,
      detail: "The run has divergent maximal authenticated terminal Git facts.",
    });
  }
  inspectLiveRepository(repository, blockers, run.run_id);
  if (workflow !== null) {
    const contractFor = (contractId) => workflow.contracts.find((entry) => (
      entry.contract.contract_id === contractId
    ))?.contract ?? null;
    try {
      assertCommittedWriteScope({
        repositoryPath: repository.root,
        admissionRevision: runtime.context.repository.revision,
        finalRevision: repository.expected_head_revision,
        runPathFences: run.plan.path_fences,
        coordinatorWorks: coordinatorWorks
          .filter((record) => record.state === "completed")
          .map((record) => {
            const contract = contractFor(record.contract_id);
            if (contract === null) throw new CliError("Coordinator work has no exact workflow task contract", 73);
            return {
              taskId: record.task_id,
              baselineRevision: record.baseline.revision,
              finalRevision: record.result.final_revision,
              writePaths: contract.task.write_paths,
            };
          }),
        integrations: integrations
          .filter((record) => record.state === "reconciled")
          .map((record) => {
            const contract = contractFor(record.contract_id);
            if (contract === null) throw new CliError("Integration has no exact workflow task contract", 73);
            return {
              taskId: record.task_id,
              prepared_main_tip: record.prepared_main_tip,
      ...
      addBlocker(blockers, {
        code: "committed-write-scope",
        authority_kind: "run",
        authority_id: run.run_id,
        task_id: null,
        detail: error.message.slice(0, 512),
      });
    }
  }
...
export async function closeRunFromAudit({
  ...
  const run = requireText(runId, "runId", { max: 128, safeId: true });
  const roots = await resolveRoots({ gitCommonDirectory, stateRoot });
  if (roots.stateRoot !== runtimeRoot(roots.commonDir)) {
    throw new CliError(
      "Historical run-closure audits are read-only; close through the immutable owning runtime",
      73,
    );
  }
  return withActiveRunMutation({
