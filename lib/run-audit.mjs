...
async function deriveRepositoryEvidence({ context, commonDir, authoritativePassVerifications, coordinatorWorks }) {
  ...
  const finalVerification = [...authoritativePassVerifications.values()]
    ...
    .at(-1) ?? null;
  const finalCoordinatorWork = coordinatorWorks
    .filter((record) => record.state === "completed")
    .sort((left, right) => Date.parse(left.completed_at) - Date.parse(right.completed_at))
    .at(-1) ?? null;
  const expected = finalCoordinatorWork !== null
    ? {
      source: "coordinator-verification",
      verificationId: finalCoordinatorWork.verification.verification_id,
      root: finalCoordinatorWork.baseline.root,
      commonDir: finalCoordinatorWork.common_dir,
      revision: finalCoordinatorWork.result.final_revision,
      branch: normalizedBranch(finalCoordinatorWork.baseline.branch),
    }
