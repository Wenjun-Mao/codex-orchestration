...
import { validateSubagentOperation } from "./subagent-lifecycle.mjs";
...
function validateAuthority(value) {
  requireExactFields(value, {
    required: [
      ...
      "policy_digest", "repository_digest", "workflow_plan_id",
...
export function validateRunClosureAudit(value) {
  requireExactFields(value, {
    required: [
      ...
      "source_records", "blockers", "counts", "terminal_ready", "audited_at",
      "record_digest",
    ],
  }, "Run-closure audit");
  if (
...
async function deriveClosureEvidence({ commonDir, stateRoot, runId }) {
  ...
  if (rawWorkflowJournal === null) {
    ...
    for (const task of currentRevision.tasks) {
      const covered = claimEntries.some((entry) => (
        ...
        && ["current", "started"].includes(entry.claim.state)
      ));
      if (!covered) {
        addBlocker(blockers, {
    ...
    if (disposition.decision === "accepted-for-integration") {
      ...
      if (integration === null) {
        addBlocker(blockers, {
          ...
          if (
            verification !== null
            && integration.verification_id !== null
            && integration.verification_id !== verification.verification_id
          ) throw new CliError("Integration and disposition name different verification authority", 73);
      }
  ...
  for (const entry of claimEntries.filter((candidate) => candidate.claim.state === "started")) {
  ...
  const orphan = (record, kind, id, taskId) => addBlocker(blockers, {
    ...
    authority_id: id,
    task_id: taskId,
    detail: "The persisted record is not reachable from a started workflow contract claim.",
  });
