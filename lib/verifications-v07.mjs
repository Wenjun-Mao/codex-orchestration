...
const VERIFICATION_KIND = "codex-flow-v07-combined-verification";
const INDEX_KIND = "codex-flow-v07-verification-request-index";
...
export function validateVerificationRecordV07(value) {
  requireExactFields(value, {
    required: [
  ...
  if (value.schema_version !== 1 || value.kind !== VERIFICATION_KIND) {
    throw new CliError("Invalid v0.7 combined verification record");
  }
...
export function verificationRecordDigest(value) {
  const record = validateVerificationRecordV07(value);
...
async function listVerificationRecords(stateRoot) {
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    verificationId(id);
    const record = validateVerificationRecordV07(await readJson(
...
export async function runCombinedVerification({
  ...
  return withProcessLock({
    ...
    const record = validateVerificationRecordV07({
      ...base,
