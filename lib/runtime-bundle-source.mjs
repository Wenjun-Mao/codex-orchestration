...
function targetRelativePath(packageRoot, source) {
  ...
  if (relativeSource === "package.json") return relativeSource;
  if (["bin/", "lib/", "schemas/", "skills/"].some((prefix) => relativeSource.startsWith(prefix))) {
...
export async function sourceRuntimeBundleFiles(packageRoot) {
  const roots = ["bin", "lib", "schemas", "skills", "templates/roles", "templates/references"];
  const files = [];
  ...
  const packageMetadata = resolve(packageRoot, "package.json");
  const contents = await readFile(packageMetadata);
  files.push({
    source: packageMetadata,
    relativePath: targetRelativePath(packageRoot, packageMetadata),
    contents,
    hash: sha256(contents),
  });
