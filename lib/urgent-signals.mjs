...
// v0.9 urgent delivery is deliberately a journaled, one-attempt transport.
...
const ONE_SHOT_ERROR = "v0.9 urgent delivery permits exactly one direct attempt";
...
function requireUrgentId(value) {
...
export function urgentId(value) {
...
export function urgentAttemptId(urgentIdValue, recipient) {
  const id = requireUrgentId(urgentIdValue);
...
function urgentPathById(stateRoot, value) {
  const id = requireUrgentId(value);
...
function validateAttempt(value, urgentIdValue) {
  ...
  if (value.retry_reason !== null) {
    throw new CliError("v0.9 urgent delivery does not accept retry_reason", 73);
  }
  const recipient = validateRecipient(value.recipient, "attempt recipient");
  const id = attemptId(value.delivery_attempt_id);
  if (id !== urgentAttemptId(urgentIdValue, recipient)) {
...
export function validateUrgentSignalRecord(value) {
  requireExactFields(value, {
    required: [
      "schema_version",
      "kind",
  ...
  if (value.schema_version !== 1 || value.kind !== "urgent-signal-record") {
    throw new CliError("Unsupported v0.9 urgent-signal record");
  }
  const signal = validateUrgentSignal(value.signal);
  const id = requireUrgentId(value.urgent_id);
  if (urgentId(signal) !== id) throw new CliError("Urgent signal record has an invalid urgent_id");
  ...
  const supersededBy = value.superseded_by_urgent_id === null
    ? null
    : requireUrgentId(value.superseded_by_urgent_id);
...
function newRecord(signal, id, recipient, now) {
  return validateUrgentSignalRecord({
    schema_version: 1,
    kind: "urgent-signal-record",
    urgent_id: id,
    signal,
...
async function readRecord(path, root, { allowMissing = false } = {}) {
  ...
  return stored ? validateUrgentSignalRecord(stored) : null;
}

async function writeRecord(path, record, root) {
  const validated = validateUrgentSignalRecord(record);
...
export async function persistUrgentSignal(options) {
  requireExactFields(options, {
    required: ["stateRoot", "signal"],
    optional: ["now"],
  }, "v0.9 urgent persistence request");
  const stateRoot = options.stateRoot;
  const signal = validateUrgentSignal(options.signal);
...
export async function prepareUrgentAttempt(options) {
  requireExactFields(options, {
    required: ["stateRoot", "urgentId"],
    optional: ["attemptSequence", "retryReason", "now"],
  }, "v0.9 urgent attempt request");
  ...
  return withRecipientBindingLock({ stateRoot, recipient: snapshot.signalRecipient }, async (resolved) => {
    return withUrgentLock({ stateRoot, urgentId: id }, async ({ record, paths, root }) => {
      ...
      const attempt = {
        attempt_sequence: 1,
        delivery_attempt_id: urgentAttemptId(record.urgent_id, resolved.recipient),
        recipient: resolved.recipient,
        retry_reason: null,
...
async function listJsonFiles(root, stateRoot) {
  ...
  const result = [];
  await assertNoSymlinkComponents(trustedRoot, root, "v0.9 urgent-signal state path");
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.isSymbolicLink()) {
      throw new CliError(`v0.9 urgent-signal state contains a symbolic link: ${path}`, 73);
    }
...
export async function urgentSignalRecord({ stateRoot, urgentId: id }) {
  const root = guardRoot(stateRoot);
...
export async function urgentSignalStatus(stateRoot, { runId = null } = {}) {
  const root = guardRoot(stateRoot);
  const pending = [];
  let consumedCount = 0;
