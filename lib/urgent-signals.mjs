...
const DIRECT_ENVELOPE_LIMIT_BYTES = 2048;
...
function validateRecord(value) {
  ...
  if (value.schema_version !== 1 || value.kind !== "urgent-signal-record") {
    throw new CliError("Unsupported urgent-signal record; v0.4.1 does not migrate older urgent journals");
  }
...
function directEnvelope(record, attempt) {
  const envelope = {
    schema_version: 1,
  ...
  if (Buffer.byteLength(stableStringify(envelope), "utf8") > DIRECT_ENVELOPE_LIMIT_BYTES) {
    throw new CliError("Urgent direct envelope exceeds the 2 KiB delivery limit", 73);
  }
  return envelope;
}
  ...
  return withRecipientBindingLock({ stateRoot, recipient: snapshot.signalRecipient }, async (resolved) => {
    return withUrgentLock({ stateRoot, urgentId: id }, async ({ record, paths, root }) => {
      ...
      if (existing) {
        ...
        return {
          status: existing.outcome === null ? "already-prepared" : `already-${existing.outcome}`,
          dispatch_permitted: false,
          urgent_id: record.urgent_id,
          delivery_attempt_id: existing.delivery_attempt_id,
          direct_envelope: directEnvelope(record, existing),
        };
      }
      ...
      const envelope = directEnvelope(record, attempt);
...
export async function reconcileUrgentAttempt({
  ...
  const requestedOutcome = requireText(outcome, "outcome", { max: 32, safeId: true });
  if (!ATTEMPT_OUTCOMES.includes(requestedOutcome)) {
    throw new CliError(`outcome must be one of: ${ATTEMPT_OUTCOMES.join(", ")}`);
  }
  return withUrgentLock({ stateRoot, urgentId: id }, async ({ record, paths, root }) => {
    ...
    if (attempt.outcome !== null) {
      if (attempt.outcome !== requestedOutcome) {
        throw new CliError("Urgent delivery attempt already has a different immutable outcome", 73);
      }
      return {
        status: `already-${attempt.outcome}`,
        urgent_id: record.urgent_id,
        delivery_attempt_id: attempt.delivery_attempt_id,
      };
    }
    if (requestedOutcome === "failed" && attempt.observation_count > 0) {
      throw new CliError("Observed urgent delivery attempt cannot be reconciled as failed", 73);
    }
...
export async function consumeUrgentSignal({
  ...
  const requestedExecutor = requireText(executorId, "executor_id", { max: 128, safeId: true });
  const snapshot = await withUrgentLock({ stateRoot, urgentId: id }, async ({ record }) => ({
    signalRecipient: record.signal.recipient,
  }));
  const requested = requestedConsumer({ signalRecipient: snapshot.signalRecipient, recipient });
  return withRecipientBindingLock({ stateRoot, recipient: requested }, async (resolved) => {
    ...
    return withUrgentLock({ stateRoot, urgentId: id }, async ({ record, paths, root }) => {
      if (requestedExecutor !== record.signal.executor_id) {
        throw new CliError("executor_id does not match the persisted urgent signal", 73);
      }
