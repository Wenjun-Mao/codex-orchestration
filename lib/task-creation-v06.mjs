...
function renderBootstrap(value) {
  ...
  return [
    ...
    `Task contract: ${value.task_contract_id}`,
    marker,
    "",
...
export function validateVisibleTaskCreationRecord(value) {
  requireExactFields(value, {
    required: [
      ...
      "task_id", "task_digest", "task_contract_id", "task_title",
...
function paths(stateRoot, operationId, taskContractId = null) {
  ...
  if (taskContractId !== null) {
    requireDigest(taskContractId, "task_contract_id");
    result.claim = safeChild(resolve(root, "claims"), `${taskContractId}.json`);
    result.contract_lock = safeChild(resolve(root, "locks"), `${taskContractId}.lock.json`);
  }
  return result;
}
...
function validateClaim(value) {
  requireExactFields(value, {
    required: [
      "schema_version", "kind", "task_contract_id", "operation_id",
  ...
  return {
    ...
    task_contract_id: requireDigest(value.task_contract_id, "claim.task_contract_id"),
...
export async function prepareVisibleTaskCreation({ stateRoot, taskContract, requestedSelectors, now = Date.now() }) {
  ...
  return withProcessLock({
    ...
    if (rawClaim) {
      ...
      if (
        claim.task_contract_id !== contract.contract_id
    ...
    if (rawRecord) {
      ...
      const immutable = [
        ...
        "revision_digest", "task_id", "task_digest", "task_contract_id", "task_title",
...
function initialTurnFromHost(value, readyThreadId, launchNonce) {
  requireExactFields(value, {
...
export async function reconcileVisibleTaskCreation({
  ...
  return withProcessLock({
    ...
    if (result === "ready") {
      ...
      const ready = {
        thread_id: threadId,
        initial_turn: initialTurnFromHost(initialTurn, threadId, record.launch_nonce),
        recorded_at: timestamp,
      };
