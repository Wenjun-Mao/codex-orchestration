...
import { validateSelectorRationale } from "./model-routing-v06.mjs";
...
export async function prepareVisibleTaskCreation({ stateRoot, taskContract, requestedSelectors, now = Date.now() }) {
  ...
  const prepare = () => withProcessLock({
    ...
    if (rawRecord) {
      ...
      const immutable = [
        ...
        "selector_rationale", "launch_nonce", "bootstrap_digest",
      ];
  ...
  if (executorBranch === null) return prepare();
  const branchAuthority = branchClaimPaths(stateRoot, executorBranch);
  return withProcessLock({
    path: branchAuthority.lock,
    guardRoot: guardRoot(stateRoot),
    label: `visible task executor branch ${executorBranch}`,
  }, async () => {
    const conflict = (await visibleTaskClaims(stateRoot)).find((claim) => (
      claim.executor_branch === executorBranch
      && claim.contract_id !== contract.contract_id
    ));
    if (conflict) {
      throw new CliError(
        `Host-worktree executor branch is already claimed by another task contract: ${executorBranch}`,
        73,
      );
    }
    return prepare();
...
async function transitionTerminalNoObjectClaim(stateRoot, record) {
  ...
  const { transitionWorkflowTerminalNoObjectClaim } = await import("./workflow-journal-v06.mjs");
  await transitionWorkflowTerminalNoObjectClaim({
    stateRoot,
