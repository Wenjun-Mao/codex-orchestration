...
async function authoritativeFinalEvidence({
  ...
  const callback = await callbackRecordV06({ stateRoot, callbackId: current.callback_id });
  ...
  const release = await taskReleaseStatus({
    stateRoot,
    releaseId: callback.receipt.release_id,
  });
  const receipt = callback.receipt;
  if (
    ...
    || release.runtime_digest !== receipt.runtime_digest
    || release.config_digest !== receipt.config_digest
    || release.plan_id !== receipt.plan_id
    || release.revision_id !== receipt.revision_id
    || release.task_id !== receipt.task_id
    || release.task_contract_digest !== receipt.task_contract_digest
    || release.operation_id !== receipt.operation_id
    || release.common_dir !== verification.repository.common_dir
  ) throw new CliError("Disposition proof does not match an accepted task release", 73);
  ...
  const { serialIntegrationStatus } = await import("./integration-v06.mjs");
