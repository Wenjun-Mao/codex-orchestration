...
async function assertReceiptReleaseAuthority({ stateRoot, receipt }) {
  ...
  if (release.status !== "accepted" || release.acceptance === null) {
    throw new CliError("Task disposition requires an accepted canonical release", 73);
  }
  ...
  ) throw new CliError("Task disposition requires the exact ready visible-task operation", 73);
  assertCanonicalIdentityMatches({
...
export async function prepareTaskDisposition({
  ...
  const authority = await assertReceiptReleaseAuthority({ stateRoot, receipt: callback.receipt });
...
async function authoritativeFinalEvidence({
  ...
  if (current.callback_id !== null) {
    callback = await callbackRecordV06({ stateRoot, callbackId: current.callback_id });
    ...
    await assertReceiptReleaseAuthority({ stateRoot, receipt: callback.receipt });
  }
  if (!accepted) {
