...
import { callbackRecord, consumeCallback } from "./callbacks.mjs";
...
const DISPOSITION_KIND = "codex-flow-v09-task-disposition";
const DECISIONS = [
  "accepted-no-change",
...
export function validateDispositionRecord(value) {
  ...
  if (value.schema_version !== 1 || value.kind !== DISPOSITION_KIND) {
    throw new CliError("Invalid v0.9 task disposition schema");
  }
  ...
  if (record.callback_id === null) {
    throw new CliError("Terminal disposition requires a callback");
  }
  const expectedDispositionId = dispositionIdForCallback(record.callback_id);
...
function assertTerminalModelEvidenceMatches(receiptEvidence, launchEvidence) {
  const expectedConfigured = modelSelector(launchEvidence.task_contract.task);
  const expectedRequested = modelSelector(launchEvidence.selector_evidence.requested);
  if (
    stableStringify(receiptEvidence.configured) !== stableStringify(expectedConfigured)
    || stableStringify(receiptEvidence.requested) !== stableStringify(expectedRequested)
  ) {
    throw new CliError("Terminal model evidence does not match the visible-task selector evidence", 73);
  }

  const currentAccepted = launchEvidence.selector_evidence.accepted === null
    ? null
    : modelSelector(launchEvidence.selector_evidence.accepted);
  const currentObserved = observedModelSelector(launchEvidence.selector_evidence.observed);
  for (const [label, terminalValue, currentValue] of [
    ["accepted", receiptEvidence.accepted, currentAccepted],
    ["observed", receiptEvidence.observed, currentObserved],
  ]) {
    // A terminal receipt may truthfully omit evidence that was unavailable when
    // it was admitted and arrived later through the one-shot host reconcile.
    // It may never claim evidence absent from authority or contradict evidence
    // that authority already has.
    if (
      terminalValue !== null
      && (currentValue === null || stableStringify(terminalValue) !== stableStringify(currentValue))
    ) {
      throw new CliError(
        `Terminal ${label} model evidence does not match the visible-task selector evidence`,
        73,
      );
    }
  }
...
export async function assertTerminalReceiptAuthority({ stateRoot, receipt }) {
  ...
  if (launch.status !== "active" || launch.start_claim === null || launch.git_activation?.state !== "completed") {
    throw new CliError("Terminal receipt requires an active authenticated task launch", 73);
  }
  ...
  assertCanonicalIdentityMatches({
    ...
    coordinator_binding: launch.coordinator_binding,
    ...
    executor_thread_id: launch.start_claim.executor_thread_id,
  }, expected, "Authenticated task launch");

  const baseline = launch.task_contract.current_baseline.revision;
  if (
    launch.selector_evidence.requested.worktree.starting_revision !== baseline
    || receipt.git_outcome.baseline_revision !== baseline
  ) throw new CliError("Terminal Git baseline does not match the task-launch starting revision", 73);
  ...
  assertTerminalModelEvidenceMatches(receipt.model_evidence, launch);
  return { launch, identity: expected };
}
...
export async function prepareTaskDisposition({
  ...
  requireEnum(decision, DECISIONS, "disposition decision");
  const callback = await callbackRecord({ stateRoot, callbackId });
...
async function authoritativeFinalEvidence({
  ...
  if (current.callback_id !== null) {
    callback = await callbackRecord({ stateRoot, callbackId: current.callback_id });
  ...
  const { serialIntegrationStatus } = await import("./integration.mjs");
...
export async function finalizeTaskDisposition({
  ...
  return withProcessLock({
    ...
    const { closeReportRoutesForAssignment } = await import("./report-routes.mjs");
    await closeReportRoutesForAssignment({
      stateRoot,
      assignmentId: current.launch_id,
      reason: "terminal",
      now,
    });
    return current;
