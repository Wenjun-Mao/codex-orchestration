...
function runQueue(threadId, message) {
  for (const binary of codexBinaryCandidates()) {
    ...
    if (result.error) {
      if (["ENOENT", "EACCES"].includes(result.error.code)) {
        return { outcome: "unavailable", reason: result.error.code };
      }
      return { outcome: "ambiguous", reason: result.error.code ?? "spawn-error" };
    }
...
function validateCallbackRecord(value) {
  ...
  const observedBy = value.delivery.observed_by_recipient === null
    ? null
    : validateRecipient(value.delivery.observed_by_recipient, "Terminal callback observed-by recipient");
  const consumedBy = value.delivery.consumed_by_recipient === null
    ? null
    : validateRecipient(value.delivery.consumed_by_recipient, "Terminal callback consumed-by recipient");
  ...
  if (["persisted", "enqueue-attempted", "enqueued"].includes(state) && (observedBy !== null || consumedBy !== null)) {
    throw new CliError(`Terminal callback ${state} record cannot contain observation or consumption identity`);
  }
  if (state !== "consumed" && consumedBy !== null) {
...
async function authoritativeConsumer({ stateRoot, record, recipient }) {
  const requested = validateRecipient(recipient, "consumer recipient");
  if (requested.lineage_id !== record.receipt.recipient.lineage_id) {
...
export async function observeCallback({ stateRoot, callbackId: id, recipient }) {
  return withCallbackLock({ stateRoot, callbackId: id }, async ({ record, paths, root }) => {
    const consumer = await authoritativeConsumer({ stateRoot, record, recipient });
    if (await markExpired(record, paths.record, root)) {
    ...
    record.delivery.observed_by_recipient = consumer;
...
export async function consumeCallback({ stateRoot, callbackId: id, recipient, executorId }) {
  ...
  return withCallbackLock({ stateRoot, callbackId: id }, async ({ record, paths, root }) => {
    const consumer = await authoritativeConsumer({ stateRoot, record, recipient });
    if (executorId !== record.receipt.executor_id) {
...
export async function callbackStatus(stateRoot) {
  ...
  for (const path of await listJsonFiles(journalRoot, stateRoot)) {
    ...
    pending.push({
      ...
      recipient_generation: record.delivery.recipient.generation,
      ...
      effective_delivery: recordExpired(record) ? "expired-due" : record.delivery.state,
      age_seconds: age,
    });
  }
  return {
