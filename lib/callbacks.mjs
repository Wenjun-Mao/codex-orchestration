...
import { terminalCallbackIdForV4, validateTerminalReceiptV4 } from "./task-results.mjs";

const CALLBACK_KIND = "codex-flow-v09-terminal-callback";
const CALLBACK_ID = /^terminal-v4-[0-9a-f]{64}$/;
...
function callbackId(value) {
  ...
  if (!CALLBACK_ID.test(result)) throw new CliError("callback_id must be a v4 callback ID");
  return result;
}
...
export function validateCallbackRecord(value) {
  requireExactFields(value, {
    required: [
  ...
  if (value.schema_version !== 1 || value.kind !== CALLBACK_KIND) {
    throw new CliError("Invalid v0.9 terminal callback record");
  }
  const receipt = validateTerminalReceiptV4(value.receipt);
  const id = callbackId(value.callback_id);
  if (terminalCallbackIdForV4(receipt) !== id) throw new CliError("Terminal callback identity is invalid");
...
async function readRecord(stateRoot, id, { allowMissing = false } = {}) {
  ...
  return value === null ? null : validateCallbackRecord(value);
}

async function writeRecord(stateRoot, record) {
  const validated = validateCallbackRecord(record);
...
async function assertPersistedDispositionAuthority({ stateRoot, dispositionId, record }) {
  ...
  if (
    ...
    || disposition.kind !== "codex-flow-v09-task-disposition"
    ...
    || disposition.launch_id !== record.receipt.launch_id
...
export async function deliverCallback({ stateRoot, receipt, expectedRunId = null, now = Date.now() }) {
  const payload = validateTerminalReceiptV4(receipt);
  ...
  const id = terminalCallbackIdForV4(payload);
  ...
  return withProcessLock({
    ...
    // Dispositions depend on callbacks, so load their shared launch/task
    // authority check lazily rather than creating a static module cycle.
    const { assertTerminalReceiptAuthority } = await import("./dispositions.mjs");
    await assertTerminalReceiptAuthority({ stateRoot, receipt: payload });
...
export async function observeCallback({ stateRoot, callbackId: id, recipient, now = Date.now() }) {
...
export async function callbackStatus({ stateRoot, runId = null }) {
  const pending = [];
  let consumedCount = 0;
  for (const path of await listJsonFiles(resolve(stateRoot, "callbacks", "journal"), stateRoot)) {
    const record = validateCallbackRecord(await readJson(path, { guardRoot: guardRoot(stateRoot) }));
...
export async function callbackRecord({ stateRoot, callbackId: id }) {
