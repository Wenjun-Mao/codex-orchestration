...
const CALLBACK_KIND = "codex-flow-v07-terminal-callback";
...
export function validateCallbackRecordV07(value) {
  requireExactFields(value, {
    required: [
  ...
  if (value.schema_version !== 1 || value.kind !== CALLBACK_KIND) {
    throw new CliError("Invalid v0.7 terminal callback record");
  }
...
async function readRecord(stateRoot, id, { allowMissing = false } = {}) {
  ...
  return value === null ? null : validateCallbackRecordV07(value);
}

async function writeRecord(stateRoot, record) {
  const validated = validateCallbackRecordV07(record);
...
async function assertPersistedDispositionAuthority({ stateRoot, dispositionId, record }) {
  ...
  if (
    ...
    || disposition.kind !== "codex-flow-v07-task-disposition"
...
export async function deliverCallbackV07({ stateRoot, receipt, expectedRunId = null, now = Date.now() }) {
  ...
  return withProcessLock({
    ...
    // Dispositions depend on callbacks, so load their shared release/task
...
export async function observeCallbackV07({ stateRoot, callbackId: id, recipient, now = Date.now() }) {
...
export async function consumeCallbackV07({
  stateRoot,
  callbackId: id,
  recipient,
  executorThreadId,
...
export async function callbackStatusV07({ stateRoot, runId = null }) {
  const pending = [];
  let consumedCount = 0;
  for (const path of await listJsonFiles(resolve(stateRoot, "callbacks", "journal"), stateRoot)) {
    const record = validateCallbackRecordV07(await readJson(path, { guardRoot: guardRoot(stateRoot) }));
...
export async function callbackRecordV07({ stateRoot, callbackId: id }) {
