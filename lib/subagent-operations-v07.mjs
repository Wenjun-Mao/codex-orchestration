...
const ATTEMPT_OUTCOMES = ["accepted", "selector-rejected-before-agent-identity", "ambiguous"];
...
function validateSelection({ model, reasoning_effort, selector_rationale, fork_turns, mode }, label) {
  ...
  return {
    ...
    selector_rationale: validateSelectorRationale(selector_rationale, `${label}.selector_rationale`),
...
function operationSeed(operation) {
  return {
    ...
    selector_rationale: operation.selector_rationale,
...
function validateOperation(value) {
  ...
  if (["selector-rejected-before-agent-identity", "ambiguous"].includes(state) && (
...
function isPreDispatchOperation(operation) {
  return operation.state === "prepared"
    && operation.attempt === null
    && operation.agent_id === null
    && operation.result === null
    && operation.coordinator_disposition === null
    && operation.created_at === null
    && operation.completed_at === null
    && operation.disposed_at === null;
}
...
async function removeExactPreDispatchState(
  ...
  for (const file of files) {
    ...
    if (file.expected === undefined) {
      if (raw !== null) {
        throw new CliError(`Native subagent pre-dispatch ${file.label} is not owned by this recovery`);
      }
      continue;
    }
    if (raw === null) {
      if (!allowMissingOwned) {
        throw new CliError(`Native subagent pre-dispatch ${file.label} disappeared before recovery`);
      }
      continue;
    }
    if (stableStringify(raw) !== stableStringify(file.expected)) {
      throw new CliError(`Native subagent pre-dispatch ${file.label} does not match the exact owned state`);
    }
    owned.push(file.path);
  }
  for (const path of owned) {
    await assertNoSymlinkComponents(root, path, "Native subagent pre-dispatch state");
...
export async function prepareSubagentOperation({
  ...
  if (
    ...
    || selection.selector_rationale !== contract.task.selector_rationale
  ...
  const prepared = await withProcessLock({
    ...
    let rawClaim = await readJson(location.claim, {
      allowMissing: true,
      guardRoot: guardRoot(stateRoot),
    });
    let existing = await readJson(location.record, {
      allowMissing: true,
      guardRoot: guardRoot(stateRoot),
    });
    if (existing !== null) {
      ...
      if (rawClaim !== null) {
        ...
        if (
          ...
          || claim.operation_identity_digest !== sha256(stableStringify(immutable(current)))
          || claim.prepared_at !== current.prepared_at
        ) throw new CliError("Generated subagent contract claim does not match its operation", 73);
      }
      ...
      if (
        ...
        && isPreDispatchOperation(current)
      ) {
        ...
        throw new CliError("Native subagent operation is not replayable");
      }
    }
    if (existing !== null) throw new CliError("Native subagent recovery did not clear its exact operation", 73);
    if (rawClaim !== null) {
      const claim = validateClaim(rawClaim);
      if (
        claim.contract_id !== contract.contract_id
        || claim.operation_id !== operationId
        || claim.operation_identity_digest !== sha256(stableStringify(immutable(record)))
    ...
    await commitWorkflowOperationPreparation({
      stateRoot,
      subagentOperation: record,
      persistNative: async () => {
        await ensureExactJson(location.record, record, {
          guardRoot: guardRoot(stateRoot),
          mode: 0o600,
        });
...
export async function reconcileSubagentOperationAttempt({
  ...
  const location = paths(stateRoot, { operationId });
  const reconciled = await withProcessLock({
    path: location.operationLock,
    guardRoot: guardRoot(stateRoot),
    label: `native subagent ${operationId}`,
  }, async () => {
  ...
  if (reconciled.state === "selector-rejected-before-agent-identity") {
    const { transitionWorkflowOperationClaim } = await import("./workflow-journal-v07.mjs");
    await transitionWorkflowOperationClaim({ stateRoot, subagentOperation: reconciled });
  }
  return reconciled;
}
