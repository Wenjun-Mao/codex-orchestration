...
// v0.6 urgent delivery is deliberately a journaled, one-attempt transport.
...
export function urgentIdForV06(value) {
  return `urgent-v1-${sha256(stableStringify(signalIdentity(validateUrgentSignalV06(value))))}`;
}
...
export function urgentAttemptIdForV06(urgentIdValue, recipient) {
...
export function urgentSignalPathsV06(stateRoot, input) {
  const signal = validateUrgentSignalV06(input);
  const root = resolve(stateRoot, "urgent-signals");
  const id = urgentIdForV06(signal);
  return {
    urgentRoot: root,
    urgentId: id,
...
function validateAttempt(value, urgentIdValue) {
  ...
  if (id !== urgentAttemptIdForV06(urgentIdValue, recipient)) {
...
export function validateUrgentSignalRecordV06(value) {
  ...
  if (value.schema_version !== 1 || value.kind !== "urgent-signal-record") {
    throw new CliError("Unsupported v0.6 urgent-signal record");
  }
  const signal = validateUrgentSignalV06(value.signal);
  const id = urgentId(value.urgent_id);
  if (urgentIdForV06(signal) !== id) throw new CliError("Urgent signal record has an invalid urgent_id");
...
function newRecord(signal, id, recipient, now) {
  return validateUrgentSignalRecordV06({
    schema_version: 1,
    kind: "urgent-signal-record",
    urgent_id: id,
    signal,
...
async function readRecord(path, root, { allowMissing = false } = {}) {
  ...
  return stored ? validateUrgentSignalRecordV06(stored) : null;
}

async function writeRecord(path, record, root) {
  const validated = validateUrgentSignalRecordV06(record);
...
async function withUrgentLock({ stateRoot, urgentId: id }, operation) {
  ...
  const paths = urgentSignalPathsV06(stateRoot, initial.signal);
...
export async function persistUrgentSignalV06(options) {
  ...
  const paths = urgentSignalPathsV06(stateRoot, signal);
  const root = guardRoot(stateRoot);
...
export async function prepareUrgentAttemptV06(options) {
  ...
  return withRecipientBindingLock({ stateRoot, recipient: snapshot.signalRecipient }, async (resolved) => {
    return withUrgentLock({ stateRoot, urgentId: id }, async ({ record, paths, root }) => {
      ...
      const attempt = {
        attempt_sequence: 1,
        delivery_attempt_id: urgentAttemptIdForV06(record.urgent_id, resolved.recipient),
        recipient: resolved.recipient,
        retry_reason: null,
...
export async function reconcileUrgentAttemptV06({
  stateRoot,
  urgentId: id,
  deliveryAttemptId,
  hostCallResult,
...
export async function observeUrgentSignalV06({
  stateRoot,
  urgentId: id,
  deliveryAttemptId,
  recipient,
...
export async function consumeUrgentSignalV06({
  stateRoot,
  urgentId: id,
  recipient,
  senderExecutorId,
...
export async function expireUrgentSignalV06({ stateRoot, urgentId: id, now = Date.now() }) {
...
async function listJsonFiles(root, stateRoot) {
  ...
  const result = [];
  await assertNoSymlinkComponents(trustedRoot, root, "v0.6 urgent-signal state path");
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.isSymbolicLink()) {
      throw new CliError(`v0.6 urgent-signal state contains a symbolic link: ${path}`, 73);
    }
...
export async function urgentSignalRecordV06({ stateRoot, urgentId: id }) {
  const root = guardRoot(stateRoot);
...
export async function urgentSignalStatusV06(stateRoot, { runId = null } = {}) {
  const root = guardRoot(stateRoot);
  const pending = [];
  let consumedCount = 0;
