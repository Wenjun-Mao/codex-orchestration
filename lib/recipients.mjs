...
function validateRegistry(value) {
  ...
  if (!Number.isFinite(Date.parse(current.bound_at))) throw new CliError("Recipient registry current bound_at is invalid");
  ...
  const bindings = value.bindings.map((binding, index) => {
    ...
    if (!Number.isFinite(Date.parse(normalized.bound_at))) {
...
export async function bindRecipient({
  ...
  return withProcessLock({
    ...
    if (existing) {
      ...
      return { status: "already-bound", recipient: current };
    }
