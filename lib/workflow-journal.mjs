...
const CLAIM_STATES = ["current", "started", "terminal-no-object", "revoked"];
const OPERATION_KINDS = ["task-launch", "subagent-operation"];
...
function assertPersistedClaims(journal, operations) {
  const claims = journal.contract_claims.map((claim) => {
    ...
    if (!operation) {
      if (["started", "terminal-no-object"].includes(claim.state)) {
    ...
    const expectedState = operation.terminal_at === null ? "started" : "terminal-no-object";
...
async function operationTransitionDescriptor({ taskLaunchRecord = null, subagentOperation = null }) {
  if ((taskLaunchRecord === null) === (subagentOperation === null)) {
    throw new CliError("Workflow operation transition requires exactly one native operation record");
  }
...
function expectedOperationClaim(authority, descriptor) {
  ...
  const expectedState = descriptor.terminal_at === null ? "started" : "terminal-no-object";
  return {
    current,
    expectedState,
...
export async function commitWorkflowOperationPreparation({
  ...
  const descriptor = await operationTransitionDescriptor({ taskLaunchRecord, subagentOperation });
...
export async function transitionWorkflowOperationClaim({
  ...
  return withProcessLock({
    ...
    const startedTransition = expectedState === "terminal-no-object"
