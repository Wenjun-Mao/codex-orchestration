...
async function assertPersistedDispositionAuthority({ stateRoot, dispositionId, record }) {
  ...
  if (!disposition) throw new CliError("Callback consumption requires an authoritative persisted disposition", 73);
  requireExactFields(disposition, {
    required: [
  ...
  }, "Persisted callback disposition");
  if (
    ...
    || disposition.revision_id !== record.receipt.revision_id
    || disposition.task_id !== record.receipt.task_id
    || disposition.task_contract_digest !== record.receipt.task_contract_digest
...
export async function consumeCallbackV06({
  ...
  return withRecipientBindingLock({ stateRoot, recipient: requested }, async (resolved) => {
    if (resolved.stale) throw new CliError("Consumer binding is stale; use the current coordinator generation", 73);
    return withCallbackLock(stateRoot, id, async (record) => {
      if (record.receipt.executor_id !== executorId) {
        throw new CliError("executor_id does not match the callback", 73);
      }
