...
import { visibleTaskCreationStatus } from "./task-creation-v06.mjs";
import {
...
function canonicalIdentityFromContract(contract, operationId, readyThreadId) {
  return {
...
export function releaseIdFor(options) {
  ...
  const { taskContract, operationId, readyThreadId } = options;
  const contract = validateGeneratedTaskContract(taskContract);
  return releaseIdFromIdentity(canonicalIdentityFromContract(contract, operationId, readyThreadId));
}

function renderExecutorPrompt({ contract, releaseId, operationId, readyThreadId }) {
  return [
    "# Codex Flow v0.6 accepted task release",
    "",
    ...
    "Return exactly one terminal receipt through the accepted v0.6 callback path.",
    "",
    `Release: ${releaseId}`,
...
function immutableRecord(record) {
  return Object.fromEntries([
    ...
    "ready_thread_id", "task_contract", "prompt_digest", "prompt_length",
  ].map((key) => [key, record[key]]));
}
...
function assertCreationMatchesContract(creation, contract, operationId) {
  ...
  const expected = canonicalIdentityFromContract(contract, operationId, creation.ready.thread_id);
  const actual = {
...
export async function prepareTaskRelease(options) {
  ...
  const creation = await visibleTaskCreationStatus({ stateRoot, operationId, now });
...
export async function authenticateTaskReleaseExecutorWorktree({
  ...
  const observed = creation.selector_evidence.observed?.worktree ?? null;
  const binding = requested.mode === "host-worktree" ? observed : observed ?? requested;
  if (binding === null || binding.path === null) {
    throw new CliError("Release acceptance requires the exact persisted host-observed worktree path", 73);
  }
  const expectedRoot = await realpath(binding.path).catch(() => null);
  if (expectedRoot === null) throw new CliError("Persisted release worktree path does not exist", 73);
  const snapshot = gitSnapshot(absolutePath(repositoryPath, "repositoryPath"));
  ...
  if (snapshot.revision !== current.task_contract.current_baseline.revision) {
  ...
  if (binding.mode === "host-worktree" && snapshot.branch !== binding.executor_branch) {
