...
async function cleanupExecutorGit(commonDir, memberRecord, archiveAuthority) {
  if (resolve(archiveAuthority.launch.common_dir) !== resolve(commonDir)) {
  ...
  const expectedTip = outcome.kind === "unchanged" ? outcome.final_revision : outcome.commit;
  const ref = `refs/heads/${memberRecord.branch}`;
  if (attachedWorktreeRefs(commonDir).includes(ref)) {
    throw new CliError("Executor branch remains attached after task archival", 73);
  }
  const tip = localBranchTip(commonDir, ref);
  if (tip === null) return true;
  if (tip !== expectedTip) throw new CliError("Executor branch changed before iteration cleanup", 73);
  runGit(
    commonDir,
    ["update-ref", "-d", ref, expectedTip],
    "Unable to delete the exact executor branch",
  );
  if (localBranchTip(commonDir, ref) !== null) {
    throw new CliError("Executor branch still exists after iteration cleanup", 73);
  }
  return true;
}
  ...
  for (const role of roles) {
    ...
    for (const candidate of current.members.filter((entry) => entry.role === role && !entry.retained)) {
      ...
      if (activeCandidate.archive_attempt?.state === "accepted") {
        if (await pathExists(activeCandidate.worktree_path)) continue;
        if (
          activeCandidate.role === "executor"
          && !(await cleanupExecutorGit(commonDir, activeCandidate, eligibility.authority))
        ) continue;
        current = await updateIteration(commonDir, iterationId, async (record) => ({
          ...record,
          members: record.members.map((entry) => entry.member_id === activeCandidate.member_id ? {
