...
const TASK_LAUNCH_REFERENCE_KIND = "codex-flow-task-launch-iteration-reference-v1";
...
function member(value, label) {
  ...
  if (record.thread_id !== null && record.provisional_id !== null) {
    throw new CliError(`${label} cannot have both ready and provisional host identity`);
  }
  if (
    record.thread_id === null
    && record.provisional_id === null
    && !(record.role === "executor" && record.authority.kind === "task-launch")
  ) {
    throw new CliError(`${label} must have a ready or provisional host identity`);
  }
  ...
  const legacyExpectedId = `iteration-member-v1-${sha256(stableStringify({
    role: record.role,
    host_id: record.host_id,
    authority: record.authority,
  }))}`;
  const stableLaunchAuthority = record.authority.kind === "task-launch"
    && record.authority.authority_digest === taskLaunchReferenceDigest(
      record.authority.state_root,
      record.authority.authority_id,
    );
  const stableExpectedId = `iteration-member-v1-${sha256(stableStringify({
    role: record.role,
    authority: {
      kind: record.authority.kind,
      authority_id: record.authority.authority_id,
      state_root: record.authority.state_root,
    },
  }))}`;
  if (record.member_id !== (stableLaunchAuthority ? stableExpectedId : legacyExpectedId)) {
    throw new CliError(`${label}.member_id does not match its authority`);
  }
  return record;
}

function taskLaunchReferenceDigest(stateRoot, launchId) {
  return sha256(stableStringify({
    schema_version: 1,
    kind: TASK_LAUNCH_REFERENCE_KIND,
    launch_id: launchId,
...
function memberFor({ role, hostId, threadId, provisionalId = null, parentThreadId = null, retained, authority: memberAuthority, requestedTitle = null, observedTitle = null, worktreePath = null, branch = null, stableLaunchIdentity = false, now }) {
  const normalizedAuthority = authority(memberAuthority, "member authority");
  const timestampValue = new Date(now).toISOString();
  return member({
    member_id: `iteration-member-v1-${sha256(stableStringify(stableLaunchIdentity
      ? {
          role,
          authority: {
            kind: normalizedAuthority.kind,
            authority_id: normalizedAuthority.authority_id,
            state_root: normalizedAuthority.state_root,
          },
        }
      : { role, host_id: hostId, authority: normalizedAuthority }))}`,
    role,
    host_id: hostId,
    thread_id: threadId,
...
/**
 * Authenticate the durable end state of one assignment-backed coordinator
 * without attempting closeout or reclamation. Historical refresh admission
 * uses this read-side contract only after the assignment and report owners
 * have independently authenticated their records.
 */
export async function assertArchivedCoordinatorIterationSettled({
  commonDir,
  iterationId,
  assignmentId,
  stateRoot,
  hostId,
  threadId,
  reportingParentThreadId,
  requiredResultTip,
}) {
  const common = resolve(commonDir);
  const resultTip = requireText(requiredResultTip, "required_result_tip", { max: 64 });
  if (!REVISION.test(resultTip)) {
    throw new CliError("required_result_tip must be a concrete Git revision");
  }
  const iteration = await iterationStatus({ commonDir: common, iterationId });
  const assignment = await assignmentAuthority({ stateRoot, assignmentId });
  const coordinators = iteration.members.filter((entry) => entry.role === "coordinator");
  if (
    iteration.assignment_id !== assignmentId
    || iteration.state !== "closed"
    || coordinators.length !== 1
  ) throw new CliError("Historical coordinator iteration is not exactly closed", 73);

  const coordinator = withEffectiveCoordinatorBinding(coordinators[0]);
  if (
    coordinator.state !== "archived"
    || coordinator.retained !== false
    || coordinator.host_id !== hostId
    || coordinator.thread_id !== threadId
    || coordinator.reporting_parent_thread_id !== reportingParentThreadId
    || coordinator.authority.kind !== "assignment"
    || coordinator.authority.authority_id !== assignmentId
    || coordinator.authority.authority_digest !== assignmentMembershipAuthorityDigest(assignment)
    || coordinator.authority.state_root !== resolve(stateRoot)
    || coordinator.archive_attempt?.state !== "accepted"
    || coordinator.archive_attempt.completed_at === null
    || coordinator.archive_attempt.branch_tip === null
  ) throw new CliError("Historical coordinator archive authority does not match its assignment", 73);

  if (await pathExists(coordinator.worktree_path)) {
    throw new CliError("Historical coordinator worktree still exists", 73);
  }
  await assertNoIterationAttachmentAfterAbsence(
    common,
    coordinator,
    coordinator.archive_attempt.branch_tip,
  );
  if (coordinator.branch !== "detached") {
    const ref = `refs/heads/${coordinator.branch}`;
    if (attachedWorktreeRefs(common).includes(ref) || localBranchTip(common, ref) !== null) {
      throw new CliError("Historical coordinator branch remains present", 73);
    }
  }
  ...
  const primary = await authenticatedPrimaryWorktree(common, worktreeInventory(common));
  if (!revisionIsAncestor(common, resultTip, primary.head)) {
    throw new CliError("Authenticated primary does not preserve the audited result", 73);
  }
  return {
    iteration,
    coordinator,
...
export async function cancelIteration({ commonDir, iterationId, assignmentId, now = Date.now() }) {
  ...
  const current = await reconcileExecutorIterationMembers({ commonDir, iterationId, now });
...
function launchStateRootForBinding(assignment, binding) {
  return resolve(assignment.common_dir, "codex-flow", binding.namespace);
}

function assertLaunchAssignmentBinding(assignment, launch, stateRoot) {
  const binding = assignment.execution_bindings.find((entry) => (
    launchStateRootForBinding(assignment, entry) === resolve(stateRoot)
    && entry.run_id === launch.run_id
  ));
  if (
    binding === undefined
    || launch.common_dir !== assignment.common_dir
    || launch.coordinator_binding.thread_id !== assignment.sender.thread_id
    || launch.runtime_context_digest !== binding.runtime_context_digest
    || launch.configuration_digest !== binding.configuration_digest
    || launch.plan_id !== binding.plan_id
    || launch.revision_digest !== binding.revision_digest
  ) throw new CliError("Executor launch does not match its assignment execution authority", 73);
...
async function launchesForAssignment(assignment) {
  const launches = [];
  const requestedTitle = iterationTitle("Executor", assignment.iteration_label, assignment.purpose);
  for (const binding of assignment.execution_bindings) {
    const stateRoot = launchStateRootForBinding(assignment, binding);
    const directory = resolve(stateRoot, "task-launches", "records");
    let entries;
    try { entries = await readdir(directory, { withFileTypes: true }); }
    catch (error) { if (error?.code === "ENOENT") continue; throw error; }
    for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
      if (!entry.isFile() || !entry.name.endsWith(".json")) continue;
      const launch = validateTaskLaunchRecord(await readJson(resolve(directory, entry.name), {
        guardRoot: assignment.common_dir,
      }));
      if (
        launch.run_id !== binding.run_id
        || launch.coordinator_binding.thread_id !== assignment.sender.thread_id
        || launch.task_title !== requestedTitle
      ) continue;
      assertLaunchAssignmentBinding(assignment, launch, stateRoot);
      if (launch.creation_evidence !== null || launch.start_claim !== null) {
        launches.push({ launch, stateRoot });
      }
    }
  }
  return launches;
...
function executorMembersForLaunch(iteration, launch, stateRoot) {
  return iteration.members.filter((entry) => (
    entry.role === "executor"
    && entry.authority.kind === "task-launch"
    && entry.authority.authority_id === launch.launch_id
    && entry.authority.state_root === resolve(stateRoot)
  ));
}

function effectiveLaunchMember(assignment, launch, stateRoot, existing, now) {
  assertLaunchAssignmentBinding(assignment, launch, stateRoot);
  const threadId = launch.start_claim?.executor_thread_id
    ?? launch.creation_evidence?.ready_thread_id
    ?? null;
  const provisionalId = threadId === null
    ? launch.creation_evidence?.provisional_id ?? existing?.provisional_id ?? null
    : null;
  const observedHost = launch.creation_evidence?.host_id ?? "unknown";
  const hostId = observedHost === "unknown"
    ? existing?.host_id ?? assignment.sender.host_id
    : observedHost;
  if (existing !== null && existing !== undefined) {
    if (observedHost !== "unknown" && existing.host_id !== observedHost) {
      throw new CliError("Iteration executor owning host conflicts with launch authority", 73);
    }
    if (existing.thread_id !== null && threadId !== null && existing.thread_id !== threadId) {
      throw new CliError("Iteration executor identity conflicts with launch authority", 73);
    }
    if (
      threadId === null
      && existing.provisional_id !== null
      && provisionalId !== null
      && existing.provisional_id !== provisionalId
    ) throw new CliError("Iteration executor provisional identity conflicts with launch authority", 73);
  }
  const worktreePath = launch.git_activation?.worktree_path ?? null;
  const branch = launch.selector_evidence.requested.worktree.executor_branch;
  if (existing !== null && existing !== undefined) {
    if (existing.reporting_parent_thread_id !== assignment.sender.thread_id) {
      throw new CliError("Iteration executor reporting parent conflicts with assignment authority", 73);
    }
    if (existing.requested_title !== launch.task_title || existing.branch !== branch) {
      throw new CliError("Iteration executor binding conflicts with launch authority", 73);
    }
    if (
      existing.worktree_path !== null
      && worktreePath !== null
      && existing.worktree_path !== worktreePath
    ) throw new CliError("Iteration executor worktree conflicts with launch authority", 73);
    return member({
      ...existing,
      host_id: hostId,
      thread_id: existing.thread_id ?? threadId,
      provisional_id: existing.thread_id === null && threadId === null ? provisionalId : null,
      worktree_path: existing.worktree_path ?? worktreePath,
      updated_at: new Date(now).toISOString(),
    }, "member");
  }
  const authorityValue = {
    kind: "task-launch",
    authority_id: launch.launch_id,
    authority_digest: taskLaunchReferenceDigest(stateRoot, launch.launch_id),
  ...
  return memberFor({
    ...
    threadId,
    provisionalId,
    parentThreadId: assignment.sender.thread_id,
    retained: false,
    authority: authorityValue,
    requestedTitle: launch.task_title,
    observedTitle: null,
    worktreePath,
    branch,
    stableLaunchIdentity: true,
    now,
  });
}

function sameMemberProjection(left, right) {
  return stableStringify({
    host_id: left.host_id,
    thread_id: left.thread_id,
    provisional_id: left.provisional_id,
    reporting_parent_thread_id: left.reporting_parent_thread_id,
    requested_title: left.requested_title,
    worktree_path: left.worktree_path,
    branch: left.branch,
  }) === stableStringify({
    host_id: right.host_id,
    thread_id: right.thread_id,
    provisional_id: right.provisional_id,
    reporting_parent_thread_id: right.reporting_parent_thread_id,
    requested_title: right.requested_title,
    worktree_path: right.worktree_path,
    branch: right.branch,
  });
}

export async function reconcileExecutorIterationMembers({ commonDir, iterationId, now = Date.now() }) {
  const assignment = await assignmentAuthority({
    stateRoot: assignmentStateRoot(commonDir),
    assignmentId: (await iterationStatus({ commonDir, iterationId })).assignment_id,
  });
  const launches = await launchesForAssignment(assignment);
  return withWorktreeOwnershipLock(assignment.common_dir, () => updateIteration(
    ...
    async (current) => {
      ...
      let changed = false;
      for (const { launch: initialLaunch, stateRoot } of launches) {
        // Reread under the iteration update so a stale caller view can never
        // downgrade a projection already published by another operation.
        const launch = await taskLaunchStatus({ stateRoot, launchId: initialLaunch.launch_id });
        assertLaunchAssignmentBinding(assignment, launch, stateRoot);
        const matches = executorMembersForLaunch({ ...current, members }, launch, stateRoot);
        if (matches.length > 1) {
          throw new CliError("Iteration contains duplicate executor members for one launch reference", 73);
        }
        if (matches.length === 0 && ["cancelled", "closed"].includes(current.state)) {
          throw new CliError("Terminal iteration is missing an assigned launch obligation", 73);
        }
        const existing = matches[0] ?? null;
        const effective = effectiveLaunchMember(assignment, launch, stateRoot, existing, now);
        if (existing === null) {
          await assertIterationWorktreeOwnership(
            assignment.common_dir,
            assignment.iteration_id,
          ...
          members.push(effective);
          changed = true;
        } else if (!sameMemberProjection(existing, effective)) {
          await assertIterationWorktreeOwnership(
            assignment.common_dir,
            assignment.iteration_id,
            effective,
          ...
          members = members.map((entry) => entry.member_id === existing.member_id ? effective : entry);
          changed = true;
        }
      }
      return changed
        ? { ...current, members, updated_at: new Date(now).toISOString() }
        : current;
    },
  ));
}

export async function iterationLaunchProjectionStatus({ commonDir, iterationId }) {
  const iteration = await iterationStatus({ commonDir, iterationId });
  const assignment = await assignmentAuthority({
    stateRoot: assignmentStateRoot(commonDir),
    assignmentId: iteration.assignment_id,
  });
  const launches = await launchesForAssignment(assignment);
  const projections = launches.map(({ launch, stateRoot }) => {
    const matches = executorMembersForLaunch(iteration, launch, stateRoot);
    if (matches.length > 1) {
      return { launch_id: launch.launch_id, state_root: stateRoot, publication: "duplicate", member_id: null, effective: null };
    }
    const existing = matches[0] ?? null;
    const effective = effectiveLaunchMember(assignment, launch, stateRoot, existing, Date.parse(iteration.updated_at));
    return {
      launch_id: launch.launch_id,
      state_root: stateRoot,
      publication: existing === null ? "absent" : sameMemberProjection(existing, effective) ? "current" : "stale",
      member_id: existing?.member_id ?? effective.member_id,
      effective: {
        host_id: effective.host_id,
        thread_id: effective.thread_id,
        provisional_id: effective.provisional_id,
        worktree_path: effective.worktree_path,
        branch: effective.branch,
      },
    };
  });
  return { iteration, launch_projections: projections };
}

function refreshRetirementEvidence(value, index) {
  const label = `refresh retirement evidence[${index}]`;
  requireExactFields(value, {
    required: [
      "source_task_id", "thread_id", "host_id", "attempt_id", "result_digest",
      "observed_at", "completed_at", "worktree_path", "branch", "branch_tip",
    ],
  }, label);
  const branchTip = value.branch_tip === null
    ? null
  ...
  return {
    ...
    host_id: requireText(value.host_id, `${label}.host_id`, { max: 128, safeId: true }),
    attempt_id: requireText(value.attempt_id, `${label}.attempt_id`, { max: 128, safeId: true }),
    result_digest: digest(value.result_digest, `${label}.result_digest`),
...
/**
 * Transfers the exact archive/Git result already authenticated by bounded
 * refresh into iteration-owned cleanup progress before its source namespace is
 * deleted. This is intentionally not a second cleanup path: it records only
 * completed refresh facts and never calls the host or Git.
 */
export async function retireExecutorIterationMembersForRefresh({
  commonDir,
  iterationId,
  sourceStateRoot,
  retirements,
  ...
  if (!Array.isArray(retirements)) throw new CliError("refresh retirements must be an array");
  const evidence = retirements.map(refreshRetirementEvidence);
  if (new Set(evidence.map((entry) => entry.source_task_id)).size !== evidence.length) {
    throw new CliError("refresh retirements contain duplicate source tasks", 73);
  }
  const byTask = new Map(evidence.map((entry) => [entry.source_task_id, entry]));
  const root = resolve(sourceStateRoot);
  return updateIteration(commonDir, iterationId, async (current) => {
    ...
    for (let index = 0; index < members.length; index += 1) {
      ...
      if (
        ...
        || existing.authority.kind !== "task-launch"
        || existing.authority.state_root !== root
      ) continue;
      const launch = await taskLaunchStatus({
        stateRoot: root,
        launchId: existing.authority.authority_id,
      });
      const retirement = byTask.get(launch.task_id) ?? null;
      if (existing.state === "archived") continue;
      if (retirement === null) {
        throw new CliError(`Refresh lacks completed cleanup evidence for executor task ${launch.task_id}`, 73);
      }
      if (
        existing.thread_id !== retirement.thread_id
        || existing.host_id !== retirement.host_id
        || existing.worktree_path !== retirement.worktree_path
        || existing.branch !== retirement.branch
      ) throw new CliError("Refresh cleanup evidence conflicts with executor iteration authority", 73);
      if (existing.archive_attempt !== null) {
        throw new CliError("Refresh cannot replace existing executor archive progress", 73);
      }
      members[index] = member({
        ...
        archive_attempt: {
          ...
          state: "accepted",
          started_at: retirement.observed_at,
          completed_at: retirement.completed_at,
          result_digest: retirement.result_digest,
          reason: "refresh-retired",
          branch_tip: retirement.branch_tip,
        },
        updated_at: new Date(now).toISOString(),
      }, "member");
      changed = true;
...
export async function registerExecutorIterationMember({ assignment, launch, stateRoot, now = Date.now() }) {
  const persisted = await taskLaunchStatus({ stateRoot, launchId: launch.launch_id });
  assertLaunchAssignmentBinding(assignment, persisted, stateRoot);
  return reconcileExecutorIterationMembers({
...
export async function closeoutIterationWithOwningHost({
  ...
  let current = await reconcileExecutorIterationMembers({ commonDir, iterationId, now: entryNow });
