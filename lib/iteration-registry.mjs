...
import { resolvedTaskArchiveAuthority } from "./archive-lifecycle.mjs";
...
async function assertIterationTipPreserved(commonDir, memberRecord, expectedTip) {
  const primary = await authenticatedPrimaryWorktree(commonDir, worktreeInventory(commonDir));
  if (!revisionIsAncestor(commonDir, expectedTip, primary.head)) {
...
async function reclaimableIterationWorktree({
  ...
  if (!revisionIsAncestor(commonDir, persisted.branch_tip, primary.head)) {
    throw new CliError("Iteration work is not preserved in the authenticated source checkout", 73);
  }
...
async function reclaimAcceptedIterationMember({
  ...
  if (!(await pathExists(memberRecord.worktree_path))) {
    return cleanupIterationGit(commonDir, memberRecord, memberRecord.archive_attempt.branch_tip);
  }
  ...
  return withProcessLock({
    ...
    if (!(await pathExists(persisted.worktree_path))) {
      return cleanupIterationGit(commonDir, persisted, persisted.archive_attempt.branch_tip);
    }
    ...
    return cleanupIterationGit(commonDir, persisted, reclaimable.expected_tip);
  });
}
...
async function cleanupIterationGit(commonDir, memberRecord, expectedTip) {
  ...
  if (expectedTip !== null) await assertIterationTipPreserved(commonDir, memberRecord, expectedTip);
