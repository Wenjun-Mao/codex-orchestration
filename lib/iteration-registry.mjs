...
async function coordinatorGitAuthority(commonDir, memberRecord) {
  if (
    ...
    || memberRecord.branch === "detached"
    || !memberRecord.branch.startsWith("codex/")
  ...
  if (
    ...
    || owned.locked
    || owned.prunable
    || owned.branch !== memberRecord.branch
    || owned.head === null
...
async function assertCoordinatorTipPreserved(commonDir, memberRecord, expectedTip) {
  ...
  if (resolve(memberRecord.worktree_path) === resolve(primary.path)) {
    throw new CliError("Coordinator cleanup refuses the source checkout", 73);
  }
}
...
async function cleanupIterationGit(commonDir, memberRecord, expectedTip) {
  ...
  if (memberRecord.role === "coordinator") {
    await assertCoordinatorTipPreserved(commonDir, memberRecord, expectedTip);
  }
  const ref = `refs/heads/${memberRecord.branch}`;
  if (attachedWorktreeRefs(commonDir).includes(ref)) {
  ...
  for (const role of roles) {
    ...
    for (const candidate of current.members.filter((entry) => entry.role === role && !entry.retained)) {
      ...
      if (activeCandidate.archive_attempt?.state === "accepted") {
        ...
        if (!(await cleanupIterationGit(
          commonDir,
          activeCandidate,
          activeCandidate.archive_attempt.branch_tip,
        ))) continue;
        current = await updateIteration(commonDir, iterationId, async (record) => ({
          ...record,
          members: record.members.map((entry) => entry.member_id === activeCandidate.member_id ? {
      ...
      if (
        ...
        current = await updateIteration(commonDir, iterationId, async (record) => {
          const persisted = record.members.find((entry) => entry.member_id === activeCandidate.member_id);
          if (persisted.state === "archived" || persisted.archive_attempt !== null) return record;
