...
import { assignmentAuthority } from "./assignment-authority.mjs";
...
async function assertIterationTipPreserved(
  ...
  const preservedByAuthenticatedIntegration = effective.role === "executor"
...
async function observeExactArchive(observeArchivedThread, candidate, suppliedObservation, now) {
...
export async function closeoutIterationWithOwningHost({
  ...
  let observation = closeoutTaskObservation(taskObservation, candidate.thread_id, now);
  ...
  if (candidate.role === "executor") {
    ...
    if (archive === null) {
      if (observation === null) {
        observation = await observeExactArchive(
          observeArchivedThread,
          candidate,
          null,
          now,
