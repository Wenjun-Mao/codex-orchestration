...
import { validateCallbackRecordV07 } from "./callbacks-v07.mjs";
import { cleanupPlanV07 } from "./cleanup-v07.mjs";
import { validateDispositionRecord } from "./dispositions.mjs";
import {
  integrationRecordDigest,
  validateIntegrationRecordV07,
} from "./integration-v07.mjs";
...
import { validateSubagentOperation } from "./subagent-operations-v07.mjs";
import { validateVisibleTaskCreationRecord } from "./task-creation-v07.mjs";
import { validateUrgentSignalRecordV07 } from "./urgent-signals-v07.mjs";
import {
...
export const RUN_CLOSURE_AUDIT_KIND = "codex-flow-v07-run-closure-audit";
...
export function validateRunClosureAudit(value) {
  ...
  ) throw new CliError("Invalid v0.7 run-closure audit schema");
...
async function resolveRoots({ gitCommonDirectory = null, stateRoot = null }) {
  ...
  const expectedStateRoot = v07RuntimeRoot(commonDir);
  if (requestedStateRoot !== null && requestedStateRoot !== expectedStateRoot) {
    throw new CliError("stateRoot is not the exact v0.7 Git-common runtime namespace");
  }
  if (
...
function relativeLocator(stateRoot, path) {
  ...
  if (location === "" || location === ".." || location.startsWith(`..${sep}`)) {
    throw new CliError("Run-closure audit source escapes the v0.7 state root");
  }
...
async function deriveClosureEvidence({ commonDir, stateRoot, runId }) {
  ...
  if (run.status !== "active") throw new CliError(`v0.7 run is not active: ${runId}`);
  ...
  const callbacks = await scan({
    ...
    validate: validateCallbackRecordV07,
  ...
  const integrations = await scan({
    ...
    selectRunId: (raw) => raw?.run_id,
    validate: validateIntegrationRecordV07,
  ...
  const verifications = await scan({
    ...
    validate: validateVerificationRecordV07,
  ...
  const urgentSignals = await scan({
    ...
    validate: validateUrgentSignalRecordV07,
  ...
  for (const entry of claimEntries.filter((candidate) => (
    ...
    if (claim.state === "terminal-no-object") {
      const operation = claim.execution_kind === "subagent"
        ...
        : visibleById.get(claim.operation_id);
      if (!operation) {
        addBlocker(blockers, {
      ...
      else seen.visible.add(operation.operation_id);
  ...
  const cleanupPlan = await cleanupPlanV07({ stateRoot, runId: run.run_id });
