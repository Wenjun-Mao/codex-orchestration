...
function validateArchiveEvidence(value, label) {
  ...
  requireExactFields(value, {
    required: [
      "archive_intent_id", "thread_id", "host_id", "active_visible", "archived_visible", "observed_at",
    ],
  }, label);
  if (value.active_visible !== false || value.archived_visible !== true) {
    throw new CliError(`${label} must prove archived and absent active visibility`, 73);
  }
  return {
...
export function validateRefreshHandoff(value) {
  ...
  for (const item of handoff.cleanup) {
    const observedAt = item.archive_evidence === null
      ? null
      : timestampMilliseconds(item.archive_evidence.observed_at, "archive_evidence.observed_at");
  ...
  if (handoff.source_retirement !== null) {
    ...
    const cleanupTimes = handoff.cleanup.flatMap((item) => [
      item.archive_evidence?.observed_at,
...
function normalizeArchiveRequest(value, cleanup) {
  ...
  for (const item of cleanup) {
    ...
    if (observed.thread_id !== item.thread_id || observed.host_id !== item.host_id) {
...
export async function applyRefresh({
  ...
  return withProcessLock({
    ...
    if (handoff.state === "consumed") {
      ...
      normalizeArchiveRequest(archiveEvidence, handoff.cleanup);
      return finishCleanStartConsumption(common, handoff, timestamp, hooks);
    }
    if (handoff.state === "prepared") {
      await assertPreparedSourceUnchanged(common, handoff);
      const observations = normalizeArchiveRequest(archiveEvidence, handoff.cleanup);
