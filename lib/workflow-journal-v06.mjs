...
async function scanStartedOperations(stateRoot, authority) {
  ...
  for (const { path, raw } of await listJournalRecords(visibleRoot, stateRoot)) {
  ...
  for (const { path, raw } of await listJournalRecords(subagentRoot, stateRoot)) {
...
export async function assertWorkflowTaskContractCurrent({ stateRoot, runId, planId, taskContract }) {
  ...
  if (status.claim.state !== "current" || !status.start_permitted) {
