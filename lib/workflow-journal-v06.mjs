...
function validateContractClaim(value, label = "workflow contract claim") {
  requireExactFields(value, {
    required: [
      ...
      "started_at", "terminal_at", "revoked_at",
    ],
  }, label);
  ...
  if (claim.state === "current" && (
    ...
    || claim.started_at !== null || claim.terminal_at !== null || claim.revoked_at !== null
  ...
  if (claim.state === "terminal-no-object" && (
    ...
    || claim.operation_kind !== "visible-task-creation"
    || claim.operation_id === null
    || claim.started_at === null
    || claim.terminal_at === null
    || claim.revoked_at !== null
  )) throw new CliError(`${label} terminal-no-object state requires a visible creation operation and terminal evidence`);
  ...
  for (const { path, raw } of visibleRecords) {
    ...
    operations.set(record.contract_id, {
      ...
      terminal_at: record.status === "not-created"
        && record.resolution.reason_code === "host-rejected-before-create"
...
function deriveClaims(journal, operations) {
  return journal.contract_claims.map((claim) => {
    ...
    if (claim.state !== "current") {
      throw new CliError("Workflow claim state conflicts with its persisted native operation");
    }
...
export async function transitionWorkflowTerminalNoObjectClaim({
  stateRoot,
  visibleTaskRecord,
}) {
  ...
  ) throw new CliError("Terminal-no-object transition requires exact selector rejection before identity");
  ...
  return withProcessLock({
    ...
    if (
      operation === undefined
      || operation.operation_kind !== "visible-task-creation"
      || operation.operation_id !== record.operation_id
      || operation.terminal_at !== record.resolution.recorded_at
    ) throw new CliError("Terminal-no-object operation does not match the persisted visible-task record");
    const claims = deriveClaims(authority.journal, operations);
    const claim = claims.find((entry) => entry.contract_id === record.contract_id);
    if (!claim || claim.state !== "terminal-no-object") {
      throw new CliError("Selector rejection did not produce a terminal-no-object workflow claim");
    }
    if (stableStringify(claims) !== stableStringify(authority.journal.contract_claims)) {
      const updated = journalWithClaims(authority.journal, claims, record.resolution.recorded_at);
