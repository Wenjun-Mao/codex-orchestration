...
function validateLegacyProjectConfig(value) {
  requireExactFields(value, {
    required: [
      "schema_version", "project_id", "max_parallel_executors", "default_model",
      "default_reasoning_effort", "agents_integration", "git_lifecycle",
    ],
  }, "v0.5.1 project configuration");
  if (value.schema_version !== 4) {
    throw new CliError("Retirement supports only an exact schema-v4 v0.5.1 project configuration");
  }
  requireText(value.project_id, "v0.5.1 project_id", { max: 128, safeId: true });
  const agents = value.agents_integration;
  if (agents?.mode === "managed") {
    requireExactFields(agents, { required: ["mode"] }, "v0.5.1 agents_integration");
  } else if (agents?.mode === "external") {
    requireExactFields(agents, {
      required: ["mode", "path", "sha256", "contract_version", "attested"],
    }, "v0.5.1 agents_integration");
    const externalPath = requireText(agents.path, "v0.5.1 external instruction path", { max: 512 });
    if (
      externalPath.startsWith("/") || externalPath.includes("\\")
      || externalPath.split("/").some((part) => part === "" || part === "." || part === "..")
    ) throw new CliError("v0.5.1 external instruction path must be repository-relative");
    digest(agents.sha256, "v0.5.1 external instruction sha256");
    if (agents.contract_version !== "1" || agents.attested !== true) {
      throw new CliError("v0.5.1 external instruction authority is not exactly attested");
    }
  } else {
    throw new CliError("v0.5.1 agents_integration.mode must be managed or external");
  }
...
export async function planLegacyRetirement({ repositoryRoot, reason, plannedAt }) {
  ...
  try { ({ value: config, raw: configRaw } = await json(root, configPath, "v0.5.1 project configuration")); config = validateLegacyProjectConfig(config); }
