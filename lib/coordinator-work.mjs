...
export async function startCoordinatorWork({ stateRoot, taskContract, repositoryPath, now = Date.now() }) {
  ...
  const record = validateCoordinatorWorkRecord({
    schema_version: 1,
    kind: COORDINATOR_WORK_KIND,
    local_work_id: localWorkId(identity),
    ...identity,
    baseline: {
      root: snapshot.root,
  ...
  const paths = recordPaths(stateRoot, record.local_work_id);
  ...
  if (existing !== null) {
    ...
    if (stableStringify(recovered) !== stableStringify(record)) {
...
function assertAncestor(repositoryPath, baseline, finalRevision) {
  const result = spawnSync("git", ["merge-base", "--is-ancestor", baseline, finalRevision], {
    cwd: repositoryPath,
    env: { ...process.env, GIT_OPTIONAL_LOCKS: "0" },
    encoding: "utf8",
  });
  if (result.status !== 0) throw new CliError("Coordinator work final revision does not descend from its baseline", 73);
}

export async function completeCoordinatorWork({ stateRoot, localWorkId: id, repositoryPath, checks, now = Date.now() }) {
  ...
  return withProcessLock({
    ...
    const contractEntry = await workflowTaskContractStatus({
      stateRoot,
    ...
    const snapshot = gitSnapshot(repositoryPath);
    if (snapshot.cleanliness !== "clean" || snapshot.root !== current.baseline.root || snapshot.commonDir !== current.common_dir) {
      throw new CliError("Coordinator work completion requires the clean authoritative repository", 73);
    }
    assertAncestor(repositoryPath, current.baseline.revision, snapshot.revision);
