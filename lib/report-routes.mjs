...
export async function registerCoordinatorReportRoute({
  ...
  const assignmentIdentity = `${sha256(stableStringify({
    ...assignmentDraft,
  ...
  const routeDraft = {
    assignment: {
      ...assignmentDraft,
      assignment_id: `coordinator-assignment-v1-${assignmentIdentity}`,
    },
    sender: senderIdentity,
    recipient: recipientIdentity,
  };
  ...
  return withProcessLock({
    ...
    if ((await activeRoutesForSender(stateRoot, route.sender)).length > 0) {
      throw new CliError("Sender already has an active upstream report route", 73);
    }
    await writeRoute(stateRoot, route, { exclusive: true });
    return { status: "registered", route };
