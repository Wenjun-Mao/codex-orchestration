...
export async function bindGitOwnership({ git, operationId, now = Date.now() }) {
  ...
  if (!["local", "worktree"].includes(operation.request.environment?.type)) {
    throw new CliError("Git ownership requires a project-backed task operation");
  }
  const canonical = await realpath(operation.request.environment.project_path).catch(() => null);
  ...
  const base = {
    ...
    upstream: (() => {
      const controllerUpstream = branchUpstream(git.root, git.branch);
      return controllerUpstream === null
        ? null
        : pinUpstream(git.root, {
          remote: controllerUpstream.remote,
