...
export async function bindGitOwnership({ git, operationId, now = Date.now() }) {
  ...
  return withProcessLock({
    ...
    if (operation.request.environment.type === "host-worktree") {
      ...
      if (snapshot.branch === operation.request.environment.starting_branch) {
        throw new CliError("Host-created executor must not own its source branch");
      }
    }
    ...
    const authorityRoot = operation.request.environment.type === "host-worktree"
      ? operation.request.environment.repository_path
      : git.root;
    const authorityBranch = operation.request.environment.type === "host-worktree"
    ...
    const existing = await readJson(state.ownership, {
      allowMissing: true,
      guardRoot: stateGuard(git.stateRoot),
    });
    if (existing) {
      const bound = validateOwnership(existing);
      const unchanged = (
    ...
    const claimed = (await records(resolve(state.root, "ownership"), validateOwnership, git.stateRoot))
      .find((item) => item.branch === base.branch || item.worktree_path === base.worktree_path);
    ...
    if (operation.request.environment.type === "host-worktree" && !worktreeClean(canonical)) {
...
export async function gitLifecycleAudit({ git, config, inspectRemotes = true }) {
  ...
  const operations = new Set((await taskOperationStatus({ stateRoot: git.stateRoot })).map((item) => item.operation_id));
  ...
  const backlogCount = items.filter((item) => item.backlog).length;
  return {
    ...
    blocked: backlogCount >= config.git_lifecycle.block_at,
  };
}
...
export async function applyGitCleanupPlan({
  ...
  return withProcessLock({
    ...
    const completeAction = async (candidate, action) => {
      completedActions.push(`${candidate.operation_id}:${action}`);
