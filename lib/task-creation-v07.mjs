...
function validatePrivateResolution(value) {
  ...
  requireExactFields(value, {
    required: [
      ...
      "ready_thread_id", "binding_digest", "state_digest", "session_digest",
...
export function validateVisibleTaskCreationRecord(value) {
  requireExactFields(value, {
    required: [
      ...
      "attempt", "provisional", "ready", "private_resolution", "resolution", "worktree_binding",
  ...
  )) throw new CliError("Provisional identity record predates its native attempt");
  ...
  )) throw new CliError("Private task resolution predates its native attempt");
  if (
...
function recordView(record, extras = {}, now = Date.now()) {
  ...
  return {
    ...
    reconciliation_open: ["attempting", "provisional"].includes(value.status)
      && Date.parse(value.attempt.reconcile_by) > now,
    ...extras,
  };
}
...
export async function recordVisibleTaskCreationAttempt({
  ...
  return withProcessLock({
    ...
    if (record.attempt !== null) {
      ...
      return recordView(record, { dispatch_permitted: false }, now instanceof Date ? now.getTime() : now);
    }
  ...
  return withProcessLock({
    ...
    if (result === "provisional") {
      ...
      if (record.status === "provisional") {
        ...
        return recordView(record, {}, Date.parse(timestamp));
      }
      if (record.status !== "attempting") {
    ...
    if (result === "ready") {
      ...
      if (privateAuthority !== null) {
        if (
          ...
          throw new CliError("Private task resolution requires an already recorded provisional identity");
        }
        if (
          privateAuthority.provisional_client_thread_id !== record.provisional.client_thread_id
    ...
    return recordView(terminal, {}, Date.parse(timestamp));
  });
}
...
export async function resolvePrivateVisibleTaskCreation({
  ...
  const record = (await readRecord(stateRoot, operationId)).record;
  if (record.status !== "provisional" || record.provisional === null) {
    throw new CliError("Private task resolution requires an open provisional creation record");
  }
  const recovered = await resolveCodexAppPrivateTask({
    provisionalClientThreadId: record.provisional.client_thread_id,
  ...
  return {
    ...
    reconcile_request: {
      ...
      operation_id: record.operation_id,
      outcome: "ready",
      provisional_client_thread_id: record.provisional.client_thread_id,
      ...
      selector_evidence: {
        accepted: clone(record.selector_evidence.accepted),
        observed: recovered.observed_selectors,
      },
      reconciled_at: recovered.resolution.resolved_at,
    },
  };
}
...
export async function visibleTaskCreationStatus({ stateRoot, operationId, now = Date.now() }) {
  ...
  return withProcessLock({
    ...
    const record = (await readRecord(stateRoot, operationId)).record;
    return recordView(record, {}, now instanceof Date ? now.getTime() : now);
  });
}
