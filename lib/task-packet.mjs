...
export function validateTaskEnvironment(value, label = "environment") {
  ...
  const type = requireEnum(value.type, ["local", "worktree", "projectless"], `${label}.type`);
  const projectPath = value.project_path === null
    ? null
    : requireText(value.project_path, `${label}.project_path`, { max: 1024 });
  if (type !== "projectless" && projectPath === null) {
    throw new CliError(`${label}.project_path is required for local and worktree tasks`);
  }
  if (type !== "projectless" && !isAbsolute(projectPath)) {
    throw new CliError(`${label}.project_path must be absolute for local and worktree tasks`);
  }
...
export function validateTaskPacket(value) {
  ...
  if (value.schema_version !== 2) throw new CliError("Unsupported task packet schema_version");
...
export function applyTaskDefaults(packet, projectConfig) {
  ...
  if (value.stop_policy.ordinary_completion !== projectConfig.ordinary_completion_authority) {
    throw new CliError(
      `Task ordinary-completion authority ${value.stop_policy.ordinary_completion} does not match project authority ${projectConfig.ordinary_completion_authority}`,
    );
  }
  return {
    ...value,
