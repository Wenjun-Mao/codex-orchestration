...
export function validateTaskEnvironment(value, label = "environment") {
  ...
  requireExactFields(value, { required: ["type", "repository_path", "starting_branch"] }, label);
...
export function validateTaskPacket(value) {
  ...
  if (value.schema_version !== 3) throw new CliError("Unsupported task packet schema_version");
...
function renderValidatedTaskPacket(value) {
  const environment = value.environment.type === "host-worktree"
    ? `\`host-worktree\` from \`${value.environment.starting_branch}\``
