...
function validateOperationRecord(value) {
  ...
  if (value.schema_version !== 7 || value.kind !== OPERATION_KIND) {
...
export async function rejectTaskOperationBeforeRelease({
  ...
  return withProcessLock({
    ...
    return withProcessLock({
      ...
      if (await readJson(branchClaimPath, { allowMissing: true, guardRoot }) !== null) {
        throw new CliError("Task operation rejection is not allowed while a Git branch claim requires recovery");
      }
