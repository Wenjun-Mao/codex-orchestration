...
function validateSettledBranchClaim(value, request, observed, operationId) {
  ...
  return {
    claim: result,
    local_branch_state: requireEnum(value.local_branch_state, ["deleted", "already-absent"], "resolution branch claim.local_branch_state"),
  };
}
...
function validateOperationRecord(value) {
  ...
  if (value.schema_version !== 8 || value.kind !== OPERATION_KIND) {
...
async function settleBranchClaim({ record, claimPath, guardRoot }) {
  const raw = await readJson(claimPath, { allowMissing: true, guardRoot });
  if (raw === null) return null;
  const settlement = validateSettledBranchClaim({ claim: raw, local_branch_state: "already-absent" }, record.request, record.observed, record.operation_id);
  ...
  if (tip !== null) gitResult(source, ["update-ref", "-d", ref, claim.baseline_revision], "Git claimed branch deletion");
  return { claim, local_branch_state: tip === null ? "already-absent" : "deleted" };
}
    ...
    return withProcessLock({
      ...
      if (record.status === "rejected-before-release") {
        ...
        if (record.resolution.branch_claim_settlement !== null) {
          ...
          if (raw !== null && !settledClaimFor(record, validateSettledBranchClaim({ claim: raw, local_branch_state: "already-absent" }, record.request, record.observed, record.operation_id).claim)) {
      ...
      const branchClaimSettlement = executionPathState === "absent"
        ? await settleBranchClaim({ record, claimPath: branchClaimPath, guardRoot })
        : null;
