...
function validateOperationRecord(value) {
  ...
  const hasBaseline = Object.hasOwn(value.request, "baseline");
  const hasEnvironment = Object.hasOwn(value.request, "environment");
  ...
  if (hasBaseline) {
    request.baseline = validateTaskBaseline(value.request.baseline, "request.baseline");
    request.environment = validateTaskEnvironment(value.request.environment, "request.environment");
  }
  return {
    schema_version: 1,
    kind: OPERATION_KIND,
