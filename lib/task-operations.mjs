...
function qualifyObservationEvidence(evidence, request, actualKind) {
  ...
  const placement = result.project_placement;
  if (expectedProjectId === null) {
    if (placement.source !== "not-applicable" || placement.value !== null) {
      throw new CliError("Projectless and inherited reconciliation must mark project placement not-applicable");
    }
  } else {
    if (placement.source === "unavailable") {
      throw new CliError("Project-backed reconciliation requires host-observed or host-accepted project placement");
    }
    if (placement.source === "not-applicable") {
      throw new CliError("Project-backed reconciliation cannot mark project placement not-applicable");
    }
    if (placement.value !== expectedProjectId) {
      throw new CliError("Observed project placement does not match the requested target project ID");
    }
  }
  return result;
}
...
function validateRejectionResolution(value, request, observed) {
  if (value === null) return null;
  requireExactFields(value, {
    required: ["disposition", "reason_code", "host_object_state", "execution_path_state", "recorded_at"],
...
function validateOperationRecord(value) {
  ...
  if (value.schema_version !== 6 || value.kind !== OPERATION_KIND) {
  ...
  if (["observed", "rejected-before-release"].includes(status) !== (observed !== null)) {
    throw new CliError("Task operation status and host observation are inconsistent");
  }
  const resolution = validateRejectionResolution(value.resolution, request, observed);
