...
export async function captureRefreshGitAuthority({
  ...
  if (external.upstream !== null || external.matching_remote !== null) {
    throw new CliError("Refresh refuses an executor branch retained by an upstream or matching remote ref", 73);
  }
...
export async function removeRefreshExecutorWorktree(authority) {
  ...
  const {
    worktree_inventory_digest: currentInventory,
    tags_at_tip: currentTags,
    ...currentStable
  } = current;
  const {
    worktree_inventory_digest: expectedInventory,
    tags_at_tip: expectedTags,
    ...expectedStable
  } = expected;
