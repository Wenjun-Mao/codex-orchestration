...
function validateConsumedRefreshRecoveryAuthority(value) {
  requireExactFields(value, {
    required: [
      ...
      "target_coordinator_thread_id", "source_tree_digest", "source_retired_at",
      "consumed_at",
    ],
  ...
  const authority = {
    ...
    source_retired_at: requireTimestamp(
      value.source_retired_at,
      "consumed refresh recovery source_retired_at",
    ),
    consumed_at: requireTimestamp(value.consumed_at, "consumed refresh recovery consumed_at"),
  };
  if (
  ...
  if (Date.parse(authority.consumed_at) < Date.parse(authority.source_retired_at)) {
    throw new CliError("Consumed refresh recovery predates source retirement", 73);
  }
...
export async function recoverRefreshReportLocator({
  ...
  if (
    ...
    || disposition.report_observation.observer_thread_id !== route.sender.thread_id
    || Date.parse(disposition.report_observation.observed_at) > Date.parse(refreshAuthority.source_retired_at)
    || Date.parse(disposition.approved_at) < Date.parse(refreshAuthority.consumed_at)
