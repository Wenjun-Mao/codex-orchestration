...
import { assertInstalledDistributionIdentity } from "../distribution-identity.mjs";
import {
...
} from "../adapters/codex-app/private-archive-observer.mjs";
import { loadRuntimeBundleSource, runtimeBindingFromContext } from "../runtime-context.mjs";
import { createWorkflowPlanRevision } from "../workflow-plan.mjs";
import {
  retireOrphanedRepositoryReportLocator,
  retireRepositoryReportLocatorsForRun,
  validateReportHookLocator,
} from "../report-hook.mjs";
import { validateReportRoute } from "../report-routes.mjs";
import { issueRefreshAdmissionCapability } from "../refresh-admission-capability.mjs";
import {
  assignmentForExecutionRetirement,
  assignmentStateRoot,
...
export const REFRESH_PREPARATION_KIND = "codex-flow-refresh-v1-preparation";
...
const ORPHAN_LOCATOR_RECOVERY_KIND = "codex-flow-v095-orphan-locator-recovery-v1";
const CONSUMED_REFRESH_RECOVERY_AUTHORITY_KIND =
  "codex-flow-v095-consumed-refresh-recovery-authority-v1";

function clone(value) {
  return JSON.parse(stableStringify(value));
}

function orphanLocatorRecoveryId(value) {
  return `refresh-locator-recovery-v1-${sha256(stableStringify({
    refresh_authority_digest: value.refresh_authority_digest,
    ...
    report_observation: value.report_observation,
  }))}`;
}

function consumedRefreshRecoveryAuthorityDigest(value) {
  const { authority_digest: ignored, ...seed } = value;
  return sha256(stableStringify(seed));
}

function validateConsumedRefreshRecoveryAuthority(value) {
  requireExactFields(value, {
    required: [
      "schema_version", "kind", "authority_digest", "refresh_id", "handoff_digest",
      "source_namespace", "source_run_id", "source_coordinator_thread_id",
      "target_coordinator_thread_id", "source_tree_digest", "consumption_observed_at",
    ],
  }, "consumed refresh recovery authority");
  const authority = {
    schema_version: value.schema_version,
    kind: value.kind,
    authority_digest: requireDigest(
      value.authority_digest,
      "consumed refresh recovery authority_digest",
    ),
    refresh_id: requireText(value.refresh_id, "consumed refresh recovery refresh_id", {
      max: 128,
      safeId: true,
    }),
    handoff_digest: requireDigest(value.handoff_digest, "consumed refresh recovery handoff_digest"),
    source_namespace: requireText(
      value.source_namespace,
      "consumed refresh recovery source_namespace",
      { max: 128, safeId: true },
    ),
    source_run_id: requireText(value.source_run_id, "consumed refresh recovery source_run_id", {
      max: 128,
      safeId: true,
    }),
    source_coordinator_thread_id: requireText(
      value.source_coordinator_thread_id,
      "consumed refresh recovery source_coordinator_thread_id",
      { max: 256, safeId: true },
    ),
    target_coordinator_thread_id: requireText(
      value.target_coordinator_thread_id,
      "consumed refresh recovery target_coordinator_thread_id",
      { max: 256, safeId: true },
    ),
    source_tree_digest: requireDigest(
      value.source_tree_digest,
      "consumed refresh recovery source_tree_digest",
    ),
    consumption_observed_at: requireTimestamp(
      value.consumption_observed_at,
      "consumed refresh recovery consumption_observed_at",
    ),
  };
  if (
    authority.schema_version !== 1
    || authority.kind !== CONSUMED_REFRESH_RECOVERY_AUTHORITY_KIND
  ) throw new CliError("Unsupported consumed refresh recovery authority", 73);
  if (authority.authority_digest !== consumedRefreshRecoveryAuthorityDigest(authority)) {
    throw new CliError("Consumed refresh recovery authority identity is invalid", 73);
  }
  return authority;
}

function validateOrphanLocatorRecoveryDisposition(value) {
  requireExactFields(value, {
    required: [
      "schema_version", "kind", "disposition_id", "refresh_authority_digest",
      "handoff_digest", "locator_sha256", "route_sha256", "source_tree_digest",
      "approved_by", "approved_at", "report_observation",
    ],
  }, "orphan locator recovery disposition");
  requireExactFields(value.approved_by, {
    required: ["host_id", "thread_id"],
  }, "orphan locator recovery approved_by");
  requireExactFields(value.report_observation, {
    required: ["status", "observer_thread_id", "observed_at", "records_digest"],
  }, "orphan locator recovery report_observation");
  const disposition = {
    schema_version: value.schema_version,
    kind: value.kind,
    disposition_id: requireText(value.disposition_id, "orphan locator recovery disposition_id", { max: 128, safeId: true }),
    refresh_authority_digest: requireDigest(
      value.refresh_authority_digest,
      "orphan locator recovery refresh_authority_digest",
    ),
    handoff_digest: requireDigest(value.handoff_digest, "orphan locator recovery handoff_digest"),
    locator_sha256: requireDigest(value.locator_sha256, "orphan locator recovery locator_sha256"),
    route_sha256: requireDigest(value.route_sha256, "orphan locator recovery route_sha256"),
    source_tree_digest: requireDigest(value.source_tree_digest, "orphan locator recovery source_tree_digest"),
    approved_by: {
      host_id: requireText(value.approved_by.host_id, "orphan locator recovery approved_by.host_id", { max: 128, safeId: true }),
      thread_id: requireText(value.approved_by.thread_id, "orphan locator recovery approved_by.thread_id", { max: 256, safeId: true }),
    },
    approved_at: requireTimestamp(value.approved_at, "orphan locator recovery approved_at"),
    report_observation: {
      status: requireEnum(
        value.report_observation.status,
        ["none-observed-before-source-removal"],
        "orphan locator recovery report_observation.status",
      ),
      observer_thread_id: requireText(
        value.report_observation.observer_thread_id,
        "orphan locator recovery report_observation.observer_thread_id",
        { max: 256, safeId: true },
      ),
      observed_at: requireTimestamp(
        value.report_observation.observed_at,
        "orphan locator recovery report_observation.observed_at",
      ),
      records_digest: requireDigest(
        value.report_observation.records_digest,
        "orphan locator recovery report_observation.records_digest",
      ),
    },
  };
  if (disposition.schema_version !== 1 || disposition.kind !== ORPHAN_LOCATOR_RECOVERY_KIND) {
    throw new CliError("Unsupported orphan locator recovery disposition", 73);
  }
  if (disposition.report_observation.records_digest !== sha256(stableStringify([]))) {
    throw new CliError("Orphan locator recovery must bind the observed empty report inventory", 73);
  }
  if (disposition.disposition_id !== orphanLocatorRecoveryId(disposition)) {
    throw new CliError("Orphan locator recovery disposition identity is invalid", 73);
  }
  return disposition;
}
...
function validateSourceIdentity(value) {
  ...
  return {
    ...
    adapter: requireEnum(value.adapter, ["v0.8-source-export"], "source.adapter"),
...
export async function authenticateRefreshSkill({ packageRoot, invokingSkillPath }) {
  ...
  ) throw new CliError("Loaded skill, package, and CLI package identity disagree; reload the Codex App", 73);
  try {
    assertInstalledDistributionIdentity({
    ...
    if (error instanceof CliError) {
      throw new CliError(`Loaded skill, package, and CLI ${error.message}; reload the Codex App`, 73);
    }
    throw error;
  }
...
export function refreshSourceCutoverBlocker(source) {
  for (const state of source.task_states) {
    if (state.task.execution_kind !== "task-thread") continue;
    const creation = state.creation ?? state.launch ?? null;
    if (creation === null) continue;
    if (["attempting", "provisional", "ambiguous"].includes(creation.status)) {
    ...
    if (
      ...
      && creation.selector_evidence.requested.worktree.mode === "host-worktree"
      && creation.worktree_binding?.state !== "completed"
    ) return `Visible executor worktree must be bound under the source snapshot before refresh: ${state.task.task_id}`;
  }
  return null;
}

export function refreshDiscardCreationAuthority(taskState) {
  const hasCreation = Object.hasOwn(taskState, "creation");
  const hasLaunch = Object.hasOwn(taskState, "launch");
  if (hasCreation === hasLaunch) {
    throw new CliError(`Discard source task has contradictory operation authority: ${taskState.task.task_id}`, 73);
  }
  if (hasLaunch) {
    return taskState.launch;
  }
  return taskState.creation;
}

export async function classifyFreshStartPredecessors({
  commonDir,
  currentNamespace,
  excludedRefreshSourceNamespace = null,
}) {
  const common = resolve(commonDir);
  const discovered = await refreshNamespaceCandidates({
    commonDir: common,
    currentNamespace,
  });
  const candidates = discovered.filter((candidate) => (
    candidate.namespace !== excludedRefreshSourceNamespace
  ));
  const reconciliations = await Promise.all(candidates.map(async (candidate) => {
    const legacy = await reconcileSettledV097CoordinatorPredecessor({ commonDir: common, candidate });
    if (legacy.status !== "not-applicable") return legacy;
    return classifyV09HistoricalPredecessor({ commonDir: common, candidate });
  }));
  return {
    candidates,
    reconciliations,
    blocked_reconciliation: reconciliations.find((entry) => entry.status === "blocked") ?? null,
    settled_predecessors: reconciliations.flatMap((entry) => {
      if (entry.status === "reconciled") return [entry.summary];
      return entry.summaries ?? [];
    }),
    unresolved_candidates: candidates.flatMap((candidate, index) => {
      const classification = reconciliations[index];
      if (["reconciled", "settled-history"].includes(classification.status)) return [];
      return [{
        ...candidate,
        ...(classification.selected_run_id === undefined
          ? {}
          : { selected_run_id: classification.selected_run_id }),
      }];
    }),
  };
}

// Fresh activation reuses refresh's exact settled-predecessor proof while it
// holds the repository-wide run lock. It never admits a merely inactive,
// malformed, active, or otherwise unsupported historical namespace.
export async function assertFreshStartPredecessors({ commonDir, currentNamespace }) {
  const pending = await readHandoff(resolve(commonDir), { allowMissing: true });
  if (pending !== null) {
    throw new CliError(
      `Pending refresh handoff ${pending.refresh_id} must be resumed before ordinary activation`,
      73,
    );
  }
  const classification = await classifyFreshStartPredecessors({ commonDir, currentNamespace });
  if (classification.blocked_reconciliation !== null) {
    throw new CliError(classification.blocked_reconciliation.reason, 73);
  }
  if (classification.unresolved_candidates.length > 0) {
    throw new CliError(
      `Clean start required before activation; incompatible Codex Flow namespace exists: ${classification.unresolved_candidates[0].namespace}. `
      + "Run codex-flow unplug plan and review the exact repository cleanup first",
      73,
    );
  }
  return classification.settled_predecessors;
}
...
export async function inspectRefresh({
  ...
  try {
    ...
    const predecessorClassification = await classifyFreshStartPredecessors({
      commonDir: common,
      currentNamespace,
    });
    if (predecessorClassification.blocked_reconciliation !== null) {
      return inspection("blocked", { target }, predecessorClassification.blocked_reconciliation.reason);
    }
    const settledPredecessors = predecessorClassification.settled_predecessors;
    const candidates = predecessorClassification.unresolved_candidates;
    if (candidates.length === 0) {
      return inspection("fresh", {
        target,
        ...(settledPredecessors.length === 0 ? {} : { settled_predecessors: settledPredecessors }),
      }, null);
    }
    ...
    const selectedRunId = candidate.selected_run_id ?? candidate.lifecycle.active_run_id
    ...
    await assertOtherRefreshSourceRunsSafe(source);
    const cutoverBlocker = refreshSourceCutoverBlocker(source);
...
function normalizeDecisionRequest(value, source) {
  ...
  const taskIds = source.task_states
    .filter((entry) => (
      entry.task.execution_kind === "task-thread"
      || (
        entry.task.execution_kind === "coordinator"
        && entry.claim?.state !== "completed"
      )
    ))
    .map((entry) => entry.task.task_id)
    .sort();
  if (stableStringify(decisions.map((entry) => entry.source_task_id)) !== stableStringify(taskIds)) {
    throw new CliError("Refresh decisions must cover every unfinished refresh-managed task exactly once", 73);
  }
  for (const decision of decisions) {
    const state = source.task_states.find((entry) => entry.task.task_id === decision.source_task_id);
    if (state.task.execution_kind === "coordinator" && decision.disposition !== "discard") {
      throw new CliError(`Unfinished coordinator work must be discarded and reissued: ${decision.source_task_id}`, 73);
    }
...
function requiredReplacementTasks(source, decisions) {
  ...
  function addDependencies(taskId) {
    ...
    for (const dependencyId of linkedTaskIds) {
      ...
      if (
        dependency.embodied
        || dependency.completed_no_change
        || (
          dependency.task.execution_kind === "coordinator"
          && dependency.claim?.state === "completed"
        )
      ) continue;
...
function buildReplacementEvidence(source, targetWorkflow, mappings) {
  ...
  for (const mapping of mappings) {
    ...
    evidence.push({
      ...
      source_operation_id: sourceState.launch?.launch_id
        ?? sourceState.creation?.operation_id
        ?? sourceState.subagent?.operation_id
        ?? sourceState.claim?.operation_id
        ?? null,
...
async function buildCleanupEvidence(source, decisions) {
  ...
  for (const decision of decisions.filter((entry) => entry.disposition === "discard")) {
    ...
    const authority = refreshDiscardCreationAuthority(task);
    if (authority === null) continue;
    const launchBacked = Object.hasOwn(task, "launch");
    if (!launchBacked && ["prepared", "not-created", "session-blocked"].includes(authority.status)) {
      if (
        authority.provisional !== null
        || authority.ready !== null
        || authority.worktree_binding !== null
      ) throw new CliError(`No-object discard contains contradictory task identity: ${task.task.task_id}`, 73);
      continue;
    }
    const launchReady = launchBacked
      && authority.status === "active"
      && authority.start_claim !== null
      && authority.git_activation?.state === "completed";
    const creationReady = !launchBacked
      && authority.status === "ready-unreleased"
      && authority.ready !== null;
    if (task.task.execution_kind !== "task-thread" || (!launchReady && !creationReady)) {
      throw new CliError(`Discard requires exact ready task authority: ${task.task.task_id}`, 73);
    }
    const requested = authority.selector_evidence.requested.worktree;
    const observed = launchBacked
      ? {
          mode: requested.mode,
          path: authority.git_activation.worktree_path,
        }
      : authority.selector_evidence.observed?.worktree;
    const worktreeBindingCompleted = launchBacked
      ? authority.git_activation.state === "completed"
      : authority.worktree_binding?.state === "completed";
    ...
    } else if (launchBacked || requested.mode !== "local" || observed?.mode !== "local" || authority.worktree_binding !== null || task.task.mode !== "read") {
      throw new CliError(`Discard executor placement is contradictory: ${task.task.task_id}`, 73);
    }
    const executorId = launchBacked ? authority.launch_id : authority.operation_id;
    const executorThreadId = launchBacked
      ? authority.start_claim.executor_thread_id
      : authority.ready.thread_id;
    const observedLaunchHostId = launchBacked
      ? authority.creation_evidence?.host_id ?? "unknown"
      : null;
    const executorHostId = observedLaunchHostId === null || observedLaunchHostId === "unknown"
      ? source.runtime.context.host.host_id
      : observedLaunchHostId;
    result.push({
      source_task_id: task.task.task_id,
      creation_operation_id: executorId,
      archive_intent_id: `refresh-archive-v1-${sha256(stableStringify({
        source_digest: source.source_digest,
        source_task_id: task.task.task_id,
        creation_operation_id: executorId,
        thread_id: executorThreadId,
        host_id: executorHostId,
      }))}`,
      thread_id: executorThreadId,
      host_id: executorHostId,
...
export async function prepareRefresh({
  ...
  return withProcessLock({
    ...
    const predecessorClassification = await classifyFreshStartPredecessors({
      commonDir: common,
      currentNamespace: `v${targetAuthority.package_version}`,
    });
    if (predecessorClassification.blocked_reconciliation !== null) {
      throw new CliError(predecessorClassification.blocked_reconciliation.reason, 73);
    }
    const settledSource = predecessorClassification.settled_predecessors.find((entry) => (
      entry.namespace === sourceNamespace && entry.run_id === sourceRunId
    ));
    if (settledSource !== undefined) {
      if (existing !== null) {
        throw new CliError(
          `Pending refresh handoff ${existing.refresh_id} takes precedence over settled-source preparation`,
          73,
        );
      }
      if (predecessorClassification.unresolved_candidates.length !== 0) {
        throw new CliError(
          "Settled-source preparation cannot bypass another unresolved predecessor namespace",
          73,
        );
      }
      ...
      if (
        sourceResume !== null
        || !Array.isArray(decisionRequest)
        || decisionRequest.length !== 0
        || !Array.isArray(replacementRequest)
        || replacementRequest.length !== 0
        || workflowDraft !== null
        || !fencePlanIsEmpty(normalizedTargetFences)
      ) throw new CliError(
        "A settled-source refresh preparation must request only a fresh activation",
        73,
      );
      const coordinatorThreadId = requireText(
        targetCoordinatorThreadId,
        "target_coordinator_thread_id",
        { max: 256, safeId: true },
      );
      return {
        schema_version: REFRESH_SCHEMA_VERSION,
        kind: REFRESH_PREPARATION_KIND,
        status: "fresh-start-required",
        mutation_performed: false,
        source: settledSource,
        target: {
          package_version: targetAuthority.package_version,
          bundle_sha256: targetAuthority.bundle_sha256,
          coordinator_thread_id: coordinatorThreadId,
        },
        next_action: {
          command: "run activate",
          refresh_id: null,
        },
      };
    }
    ...
    if (targetWorkflow !== null) {
      assertWorkflowReservationCovered(normalizedTargetFences, targetWorkflow);
    }
    const sourceAssignment = await assignmentForExecutionRetirement({
      stateRoot: assignmentStateRoot(common),
      hostId: source.run.binding.host.host_id,
      threadId: source.run.binding.lineage.thread_id,
      runId: source.run.run_id,
    });
    if (hasReplacementWork && sourceAssignment !== null && sourceAssignment.state !== "open") {
      throw new CliError(
        `Refresh replacement requires an open assignment; settle ${sourceAssignment.state} assignment ${sourceAssignment.assignment_id} first`,
        73,
      );
    }
...
async function retainRefreshAssignmentRetirement({ commonDir, source, handoff }) {
  const assignmentRoot = assignmentStateRoot(commonDir);
  const assignment = await assignmentForExecutionRetirement({
    stateRoot: assignmentRoot,
    hostId: source.run.binding.host.host_id,
    threadId: source.run.binding.lineage.thread_id,
    runId: source.run.run_id,
  });
  if (assignment === null) return { status: "no-unsettled-assignment", assignment: null };
  const binding = assignment.execution_bindings.find((entry) => (
    entry.namespace === source.namespace && entry.run_id === source.run.run_id
  ));
  if (binding === undefined) {
    throw new CliError("Refresh source is not bound to its open assignment", 73);
  }
  // Publish every launch-backed executor obligation while the retiring source
  // namespace is still readable. Cancellation must not later infer that no
  // executor existed merely because publication was interrupted before refresh.
  await reconcileExecutorIterationMembers({
    commonDir,
    iterationId: assignment.iteration_id,
    now: Date.parse(handoff.source_retirement.retired_at),
  });
  const terminalDigest = sha256(stableStringify(source.run.terminal));
  const updated = await recordAssignmentExecutionRetirement({
    stateRoot: assignmentRoot,
    ...
    retirement: {
      ...
      terminal: source.run.terminal,
      terminal_digest: terminalDigest,
      evidence_source: "refresh",
      evidence_digest: sha256(stableStringify({
        refresh_id: handoff.refresh_id,
        source_namespace: handoff.intent.source.namespace,
        source_run_id: handoff.intent.source.run_id,
        source_retirement: handoff.source_retirement,
      })),
      resource_disposition: "refresh-retired",
      owner_thread_id: assignment.sender.thread_id,
      next_action: null,
      observed_at: handoff.source_retirement.retired_at,
    },
  });
  return { status: "retained", assignment: updated };
}

async function reconcileRefreshAssignmentLaunches({ commonDir, source, now }) {
  const assignment = await assignmentForExecutionRetirement({
    stateRoot: assignmentStateRoot(commonDir),
    hostId: source.run.binding.host.host_id,
    threadId: source.run.binding.lineage.thread_id,
    runId: source.run.run_id,
  });
...
async function retainRefreshIterationCleanup({ commonDir, source, handoff, now }) {
  const assignment = await assignmentForExecutionRetirement({
    stateRoot: assignmentStateRoot(commonDir),
    hostId: source.run.binding.host.host_id,
    threadId: source.run.binding.lineage.thread_id,
  ...
  if (assignment === null) return;
  await retireExecutorIterationMembersForRefresh({
    commonDir,
    iterationId: assignment.iteration_id,
    sourceStateRoot: source.root,
    retirements: handoff.cleanup.map((item) => ({
      source_task_id: item.source_task_id,
      thread_id: item.thread_id,
      host_id: item.host_id,
      attempt_id: item.archive_intent_id,
      result_digest: item.archive_evidence.proof_digest,
      observed_at: item.archive_evidence.private_observation.observed_at,
      completed_at: item.branch_deleted_at ?? item.worktree_removed_at
        ?? item.archive_evidence.private_observation.observed_at,
      worktree_path: item.git_authority?.worktree_path ?? null,
      branch: item.git_authority?.branch ?? null,
      branch_tip: item.git_authority?.head ?? null,
    })),
    now,
  });
}
...
export async function applyRefresh({
  ...
  return withProcessLock({
    ...
    if (handoff.state === "prepared") {
      ...
      await reconcileRefreshAssignmentLaunches({
        commonDir: common,
        source,
    ...
    if (handoff.state === "archive-observed") {
      ...
      await assertRefreshNamespaceRemovalSafe({ source, handoff });
      await retainRefreshIterationCleanup({
        commonDir: common,
        source,
        handoff,
        now: Date.parse(timestamp),
      });
      const sourceWasActiveAtPreparation = handoff.intent.source.run_status === "active";
      if (sourceWasActiveAtPreparation) {
        ...
        } else if (source.run.status !== "abandoned" || source.run.terminal.reason !== expectedReason) {
          throw new CliError("Refresh active source changed terminal state after handoff preparation", 73);
        }
      } else {
        await assertPreparedSourceUnchanged(common, handoff);
        if (source.run.status !== handoff.intent.source.run_status) {
          throw new CliError("Refresh already-terminal source changed after handoff preparation", 73);
        }
      }
      ...
      if (sourceWasActiveAtPreparation) {
        const expectedReason = `Superseded by bounded refresh ${handoff.refresh_id}`;
        if (
          source.run.status !== "abandoned" || source.run.terminal.reason !== expectedReason
      ...
      handoff = await writeTransition(common, handoff, {
        ...
        source_retirement: {
          // This timestamp records refresh recognition, not the source's own
          // terminal event. An already-terminal source retains that event in
          // its authenticated snapshot and may predate handoff preparation.
          retired_at: timestamp,
          method: sourceWasActiveAtPreparation ? "snapshot-abandon" : "already-terminal",
          terminal_status: source.run.status,
    ...
    if (handoff.state === "source-retired") {
      ...
      const assignmentRetirement = await retainRefreshAssignmentRetirement({
        commonDir: common,
        source,
        handoff,
      });
      await hooks.afterAssignmentRetirement?.(clone(assignmentRetirement));
    }
...
/**
 * Retire the one known pre-retirement locator left by an already-consumed
 * refresh. Every surviving authority byte is supplied explicitly; absence of
 * the source namespace is checked only after those records authenticate.
 */
export async function recoverRefreshReportLocator({
  commonDir,
  refreshAuthority: refreshAuthorityInput,
  ...
  const common = resolve(commonDir);
  const refreshAuthority = validateConsumedRefreshRecoveryAuthority(refreshAuthorityInput);
  const locator = validateReportHookLocator(locatorInput);
  const route = validateReportRoute(routeInput);
  const disposition = validateOrphanLocatorRecoveryDisposition(dispositionInput);
  const sourceRoot = resolve(common, "codex-flow", refreshAuthority.source_namespace);
  if (
    locator.state_root !== sourceRoot
    || route.state !== "active"
    || route.assignment.run_id !== refreshAuthority.source_run_id
    || route.assignment.common_dir !== common
    || route.sender.thread_id !== refreshAuthority.source_coordinator_thread_id
    || refreshAuthority.target_coordinator_thread_id !== locator.sender_thread_id
    || route.sender.thread_id !== locator.sender_thread_id
    || route.sender.host_id !== route.recipient.host_id
    || route.route_id !== locator.route_id
  ) throw new CliError("Orphan locator evidence does not bind the consumed refresh source", 73);
  if (
    disposition.refresh_authority_digest !== refreshAuthority.authority_digest
    || disposition.handoff_digest !== refreshAuthority.handoff_digest
    || disposition.locator_sha256 !== sha256(stableStringify(locator))
    || disposition.route_sha256 !== sha256(stableStringify(route))
    || disposition.route_sha256 !== locator.route_sha256
    || disposition.source_tree_digest !== refreshAuthority.source_tree_digest
    || disposition.approved_by.host_id !== route.recipient.host_id
    || disposition.approved_by.thread_id !== route.recipient.thread_id
    || disposition.report_observation.observer_thread_id !== route.recipient.thread_id
    || Date.parse(disposition.report_observation.observed_at) > Date.parse(refreshAuthority.consumption_observed_at)
    || Date.parse(disposition.approved_at) < Date.parse(refreshAuthority.consumption_observed_at)
  ) throw new CliError("Orphan locator recovery disposition does not match its exact authority", 73);
  return withProcessLock({
    path: repositoryLockPath(common),
    guardRoot: common,
    label: "Orphan report locator recovery",
  }, async () => {
    const sourceExists = await lstat(sourceRoot).then(() => true).catch((error) => {
      if (error?.code === "ENOENT") return false;
      throw error;
    ...
    if (sourceExists) {
      throw new CliError("Orphan locator recovery is not available while the source namespace still exists", 73);
    }
    return retireOrphanedRepositoryReportLocator({
      commonDir: common,
      locator,
      route,
      recovery: {
        kind: "authenticated-consumed-refresh-orphan",
        refresh_id: refreshAuthority.refresh_id,
        handoff_digest: refreshAuthority.handoff_digest,
        source_tree_digest: refreshAuthority.source_tree_digest,
        disposition_id: disposition.disposition_id,
      },
      now: Date.parse(requireTimestamp(recoveredAt, "recovered_at")),
    });
  });
}
...
export async function consumeRefreshActivation({
  ...
  return withProcessLock({
    ...
    const unrelatedPredecessors = await classifyFreshStartPredecessors({
      commonDir: common,
      currentNamespace: basename(resolve(stateRoot)),
      excludedRefreshSourceNamespace: handoff.intent.source.namespace,
    });
    if (unrelatedPredecessors.blocked_reconciliation !== null) {
      throw new CliError(unrelatedPredecessors.blocked_reconciliation.reason, 73);
    }
    if (unrelatedPredecessors.unresolved_candidates.length > 0) {
      throw new CliError(
        `Refresh target admission is blocked by unrelated Codex Flow namespace: ${unrelatedPredecessors.unresolved_candidates[0].namespace}`,
        73,
      );
    }
    ...
    if (sourceExists) {
      ...
      await retainRefreshAssignmentRetirement({ commonDir: common, source, handoff });
    ...
    const targetRuntimeBinding = runtimeBindingFromContext(runtime);
    const targetExecutionBinding = {
      run_id: runId,
      runtime_context_digest: targetRuntimeBinding.runtime_context_hash,
      configuration_digest: targetRuntimeBinding.config_hash,
      repository_digest: targetRuntimeBinding.repository_hash,
      repository_root: runtime.repository.root,
      repository_branch: runtime.repository.branch,
      plan_id: workflow.plan_id,
      revision_digest: workflow.revision_digest,
      namespace: basename(resolve(stateRoot)),
      bound_at: activationTimestamp,
    };
    const assignmentBinding = await bindAssignmentRefreshExecution({
      commonDir: common,
      sender: {
        host_id: runtime.host.host_id,
        thread_id: runtime.lineage.thread_id,
      },
      source: {
        run_id: handoff.intent.source.run_id,
        runtime_context_digest: handoff.intent.source.runtime_context_digest,
        plan_id: handoff.intent.source.workflow_plan_id,
        namespace: handoff.intent.source.namespace,
      },
      targetExecutionBinding,
      beforeBinding: async (boundAssignment) => {
        await hooks.afterAssignmentBindingPersisted?.({
          target_run_id: runId,
        });
        const prepared = reused?.prepared ?? await prepare();
        if (
          boundAssignment !== null
          && stableStringify(boundAssignment.execution_bindings.at(-1))
            !== stableStringify(targetExecutionBinding)
        ) throw new CliError("Refresh target assignment binding drifted before admission", 73);
        const admissionCapability = issueRefreshAdmissionCapability({
          sourceNamespace: handoff.intent.source.namespace,
          target: {
            ...targetExecutionBinding,
            assignment_id: boundAssignment?.assignment_id ?? null,
          },
        });
        const admitted = reused?.admitted ?? await admit(
          repositoryLockToken,
          admissionCapability,
        );
    ...
    const { prepared, admitted } = assignmentBinding.operation_result;
    ...
    await hooks.afterAssignmentBinding?.(clone(assignmentBinding));
    if (handoff.state !== "consumed") {
      handoff = await writeTransition(common, handoff, {
        state: "consumed",
        target_consumption: {
    ...
    return { admitted, prepared, origin, assignment_binding: assignmentBinding, refresh_status: "consumed" };
  });
}
