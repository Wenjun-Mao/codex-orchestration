...
export async function classifyFreshStartPredecessors({ commonDir, currentNamespace }) {
  const common = resolve(commonDir);
  const candidates = await refreshNamespaceCandidates({
    commonDir: common,
    currentNamespace,
  });
  const reconciliations = await Promise.all(candidates.map((candidate) => (
    reconcileSettledV097CoordinatorPredecessor({ commonDir: common, candidate })
  )));
  return {
    candidates,
    reconciliations,
    blocked_reconciliation: reconciliations.find((entry) => entry.status === "blocked") ?? null,
    settled_predecessors: reconciliations
      .filter((entry) => entry.status === "reconciled")
      .map((entry) => entry.summary),
    unresolved_candidates: candidates.filter((candidate, index) => (
      reconciliations[index].status !== "reconciled"
    )),
  };
}
...
export async function consumeRefreshActivation({
  ...
  return withProcessLock({
    ...
    const assignmentBinding = await bindAssignmentRefreshExecution({
      commonDir: common,
      sender: {
        host_id: admitted.run.binding.host.host_id,
      ...
      targetExecutionBinding: {
        ...
        repository_digest: admitted.run.binding.repository_hash,
        repository_root: runtime.repository.root,
        repository_branch: runtime.repository.branch,
        plan_id: admitted.run.workflow_plan_id,
        revision_digest: admitted.run.workflow_revision_digest,
        namespace: basename(resolve(stateRoot)),
        bound_at: origin.recorded_at,
      },
    });
