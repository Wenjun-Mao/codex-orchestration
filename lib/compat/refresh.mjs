...
function validateOrphanLocatorRecoveryDisposition(value) {
  requireExactFields(value, {
    required: [
      "schema_version", "kind", "disposition_id", "handoff_digest", "locator_sha256",
      "route_sha256", "source_tree_digest", "approved_by", "approved_at",
      "report_observation",
    ],
...
export async function recoverRefreshReportLocator({
  ...
  const common = resolve(commonDir);
  const handoff = validateRefreshHandoff(handoffInput);
  ...
  if (
    ...
    || handoff.target_consumption?.mode !== "clean-start"
    || handoff.source_retirement === null
  ) throw new CliError("Orphan locator recovery requires a consumed clean-start refresh", 73);
  const sourceRoot = resolve(common, "codex-flow", handoff.intent.source.namespace);
  if (
    locator.state_root !== sourceRoot
    || route.state !== "active"
    || route.assignment.run_id !== handoff.intent.source.run_id
    || route.assignment.common_dir !== common
    || route.sender.thread_id !== handoff.intent.source.coordinator.thread_id
    || handoff.intent.target.coordinator_thread_id !== locator.sender_thread_id
  ...
  if (
    disposition.handoff_digest !== handoff.handoff_digest
    ...
    || disposition.source_tree_digest !== handoff.source_retirement.final_source_tree.tree_digest
    ...
    || Date.parse(disposition.report_observation.observed_at) > Date.parse(handoff.source_retirement.retired_at)
  ...
  return retireOrphanedRepositoryReportLocator({
    ...
    recovery: {
      ...
      source_tree_digest: handoff.source_retirement.final_source_tree.tree_digest,
