...
export function refreshDiscardCreationAuthority(taskState) {
  ...
  if (hasLaunch) {
    if (taskState.launch === null) return null;
    throw new CliError(`Refresh handoff v1 cannot represent launch-identity source discard cleanup: ${taskState.task.task_id}`, 73);
  }
...
function buildReplacementEvidence(source, targetWorkflow, mappings) {
  ...
  for (const mapping of mappings) {
    ...
    evidence.push({
      ...
      source_operation_id: sourceState.creation?.operation_id
...
async function buildCleanupEvidence(source, decisions) {
  ...
  for (const decision of decisions.filter((entry) => entry.disposition === "discard")) {
    ...
    const creation = refreshDiscardCreationAuthority(task);
    if (creation === null) continue;
    if (["prepared", "not-created", "session-blocked"].includes(creation.status)) {
      if (
        creation.provisional !== null
        || creation.ready !== null
        || creation.worktree_binding !== null
    ...
    result.push({
      ...
      archive_intent_id: `refresh-archive-v1-${sha256(stableStringify({
        ...
        source_task_id: task.task.task_id,
        creation_operation_id: creation.operation_id,
        thread_id: creation.ready.thread_id,
