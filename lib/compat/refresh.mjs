...
export async function inspectRefresh({
  ...
  try {
    ...
    const discoveredCandidates = await refreshNamespaceCandidates({
      commonDir: common,
      currentNamespace,
    });
    const reconciliations = await Promise.all(discoveredCandidates.map((candidate) => (
      reconcileSettledV097CoordinatorPredecessor({ commonDir: common, candidate })
    )));
    const blockedReconciliation = reconciliations.find((entry) => entry.status === "blocked");
    if (blockedReconciliation !== undefined) {
      return inspection("blocked", { target }, blockedReconciliation.reason);
    }
    const settledPredecessors = reconciliations
      .filter((entry) => entry.status === "reconciled")
      .map((entry) => entry.summary);
    const candidates = discoveredCandidates.filter((candidate, index) => (
