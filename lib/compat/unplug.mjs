...
const PLAN_V1_KIND = "codex-flow-v09-unplug-plan";
const PLAN_V2_KIND = "codex-flow-v09-unplug-plan-v2";
const JOURNAL_KIND = "codex-flow-v09-unplug-journal";
const RECEIPT_KIND = "codex-flow-v09-unplug-receipt";
const JOURNAL_DIRECTORY = "codex-flow-unplug-v09";
...
function validatePlanEnvelope(value, {
  ...
  if (value.schema_version !== schemaVersion || value.kind !== kind || !planIdPattern.test(value.plan_id)) {
    throw new CliError("Invalid v0.9 unplug plan");
  }
...
export function validateUnplugPlan(value) {
  if (value?.schema_version === 1) return validateUnplugPlanV1(value);
  if (value?.schema_version === 2) return validateUnplugPlanV2(value);
  throw new CliError("Invalid v0.9 unplug plan");
}

export function unplugPlanDigest(plan) {
  return sha256(stableStringify(planSeed(validateUnplugPlan(plan))));
}

async function buildUnplugPlan({ repositoryPath, resources = [] }, schemaVersion) {
...
export async function unplugPlan(options) {
  return buildUnplugPlan(options, 2);
}

export async function observePrivateUnplug({ plan, codexHome, now = Date.now() }) {
  const checked = validateUnplugPlan(plan);
  const tasks = checked.resources.filter((resource) => resource.kind === "task");
  ...
  for (const task of tasks) {
    ...
    archiveEvidence[task.id] = {
      thread_id: task.thread_id,
      archived: true,
      observed_at: privateObservation.observed_at,
  ...
  return {
    schema_version: 1,
    kind: "codex-flow-v09-unplug-private-observation",
    plan_id: checked.plan_id,
    plan_digest: unplugPlanDigest(checked),
    repository: {
      root: checked.repository.root,
      common_dir: checked.repository.common_dir,
    },
    observed_at: new Date(now instanceof Date ? now.getTime() : now).toISOString(),
    mutation_performed: false,
...
function archiveEvidenceForPlan(plan, archiveEvidence) {
  ...
  for (const task of tasks) {
    ...
    const privateSource = evidence?.source === PRIVATE_ARCHIVE_OBSERVATION_SOURCE;
    requireExactFields(evidence, {
      required: privateSource
        ? [
          "thread_id", "archived", "observed_at", "source", "plan_id",
          "resource_id", "private_observation",
        ]
        : ["thread_id", "archived", "observed_at", "source"],
    }, `archive_evidence.${task.id}`);
    if (
      evidence.thread_id !== task.thread_id
      || evidence.archived !== true
      || !["codex-app", PRIVATE_ARCHIVE_OBSERVATION_SOURCE].includes(evidence.source)
    ) {
      throw new CliError(`archive_evidence.${task.id} does not prove the exact task is archived`);
    }
    const observedAt = timestamp(evidence.observed_at, `archive_evidence.${task.id}.observed_at`);
    if (privateSource) {
      const privateObservation = validatePrivateArchiveObservation(
        evidence.private_observation,
        `archive_evidence.${task.id}.private_observation`,
      );
      if (
        evidence.plan_id !== plan.plan_id
        || evidence.resource_id !== task.id
        || privateObservation.thread_id !== task.thread_id
        || privateObservation.observed_at !== observedAt
      ) throw new CliError(`archive_evidence.${task.id} private provenance does not match the exact plan task`);
      normalized[task.id] = {
        thread_id: task.thread_id,
        archived: true,
...
export async function assertNoUnplugInProgress({ gitCommonDirectory }) {
...
export async function unplugApply({
  repositoryPath,
  plan,
  archiveEvidence = {},
  now = Date.now(),
  ...
  if (retained.state === "missing" || retained.state === "empty") {
    ...
    const fresh = await buildUnplugPlan(
