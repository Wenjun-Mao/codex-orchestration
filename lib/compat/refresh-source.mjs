...
function assertClosedSettlementAuthority({ audit, commonDir, journal, namespace, run, runtime }) {
  const root = sourceNamespaceRoot(commonDir, namespace);
  ...
  if (!authorityMatches) {
    throw new CliError(
      `Earlier closed source run has inconsistent reclaimed-checkout authority: ${run.run_id}`,
      73,
    );
  }
}
...
async function assertReclaimedClosedRunSettled({ commonDir, namespace, run }) {
  ...
  if (audits.length !== 1) {
    throw new CliError(`Earlier closed source run lacks one authenticated settlement audit: ${run.run_id}`, 73);
  }
  ...
  ) throw new CliError(`Earlier closed source run is not settlement-ready: ${run.run_id}`, 73);
  ...
  assertClosedSettlementAuthority({ audit, commonDir: common, journal, namespace, run, runtime });
}
