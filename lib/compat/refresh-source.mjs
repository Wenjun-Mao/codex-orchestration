...
const RC8_REFRESH_SOURCE_SHA256 = "4224a950a329111f11f4a2968ffb8cf21cd793457fe3588851d0791978a913d2";
const RC8_REFRESH_BUNDLE_PATH_GUARD =
  "|| !REFRESH_BUNDLE_PATH_PREFIXES.some((prefix) => path.startsWith(prefix))";
const RC8_REFRESH_BUNDLE_PATH_REPAIR =
...
function sourceExportRequest(options, located) {
...
function invokeSourceExporter({ exporterPath, cwd, request }) {
  return spawnSync(process.execPath, [exporterPath], {
    cwd,
    env: { ...process.env, GIT_OPTIONAL_LOCKS: "0", GIT_TERMINAL_PROMPT: "0" },
    input: request,
    encoding: "utf8",
...
async function invokeRc8SourceExporterCapsule({ located, request }) {
  if (
    located.manifest.package_version !== "0.9.7-rc.8"
    || located.manifest.files["lib/compat/refresh-source.mjs"] !== RC8_REFRESH_SOURCE_SHA256
    || !Object.hasOwn(located.manifest.files, "package.json")
  ) return null;
  const temporaryRoot = await mkdtemp(resolve(tmpdir(), "codex-flow-rc8-refresh-export-"));
  try {
    const bundleRoot = resolve(temporaryRoot, "files");
    await cp(located.bundle_root, bundleRoot, { recursive: true, force: false, errorOnExist: true });
    const sourcePath = resolve(bundleRoot, "lib", "compat", "refresh-source.mjs");
    const source = await readFile(sourcePath, "utf8");
    if (sha256(source) !== RC8_REFRESH_SOURCE_SHA256) {
      throw new CliError("RC8 refresh compatibility capsule source hash changed", 73);
    }
    const occurrences = source.split(RC8_REFRESH_BUNDLE_PATH_GUARD).length - 1;
    if (occurrences !== 1) {
      throw new CliError("RC8 refresh compatibility capsule patch site changed", 73);
    }
    await writeFile(
      sourcePath,
      source.replace(RC8_REFRESH_BUNDLE_PATH_GUARD, RC8_REFRESH_BUNDLE_PATH_REPAIR),
      { encoding: "utf8", mode: 0o600 },
    );
    return invokeSourceExporter({
      exporterPath: resolve(bundleRoot, "bin", "codex-flow-refresh-source.mjs"),
...
export async function loadRefreshSourceAuthority(options) {
  ...
  const request = sourceExportRequest(options, located);
  const exportedResult = await invokeRc8SourceExporterCapsule({ located, request })
    ?? invokeSourceExporter({
      exporterPath: located.exporter_path,
      cwd: located.context.repository.root,
      request,
    });
