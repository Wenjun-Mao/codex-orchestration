...
export const V06_ADOPTION_SCHEMA_VERSION = 2;
...
const LEGACY_INSTRUCTIONS_RELATIVE_PATH = ".codex/orchestration/v0.6/INSTRUCTIONS.md";
...
export function adoptionInstructionsPath(repositoryRoot) {
  return join(validateAbsoluteRepositoryRoot(repositoryRoot), ...LEGACY_INSTRUCTIONS_RELATIVE_PATH.split("/"));
}
...
function validateLegacyReviewedInstructions(value, label = "reviewed_instructions") {
...
export function validateAdoptionManifest(value, label = "adoption manifest") {
  requireExactFields(value, {
    required: [
      "schema_version", "kind", "adopted_at", "bundle", "config", "policy",
    ],
  }, label);
...
function validateLegacyAdoptionManifest(value, label = "legacy adoption manifest") {
  requireExactFields(value, {
    required: [
  ...
  if (value.schema_version !== 1 || value.kind !== V06_ADOPTION_KIND) {
    throw new CliError(`${label} must be an exact schema-v1 tracked adoption`);
  }
  return {
    schema_version: 1,
    kind: V06_ADOPTION_KIND,
    ...
    config: validateRuntimeConfig(value.config),
    policy: validateRuntimePolicy(value.policy),
    reviewed_instructions: validateLegacyReviewedInstructions(
      value.reviewed_instructions,
      `${label}.reviewed_instructions`,
    ),
  };
}

function validateAnyAdoptionManifest(value, label = "adoption manifest") {
  return value?.schema_version === 1
    ? validateLegacyAdoptionManifest(value, label)
    : validateAdoptionManifest(value, label);
}
...
function renderLegacyAdoptionInstructions(manifest) {
  const adoption = validateLegacyAdoptionManifest(manifest);
  return [
...
function manifestBytes(manifest) {
  return Buffer.from(`${stableStringify(validateAnyAdoptionManifest(manifest), 2)}\n`);
}
...
function isSafeAdoptionPath(path) {
  if ([ADOPTION_RELATIVE_PATH, LEGACY_INSTRUCTIONS_RELATIVE_PATH, BUNDLE_RELATIVE_PATH].includes(path)) {
    return true;
  }
...
function allowedPathsFor(adoption) {
  ...
  if (adoption.schema_version === 1) paths.push(LEGACY_INSTRUCTIONS_RELATIVE_PATH);
...
async function readRawAdoption(root) {
  ...
  try {
    return {
      adoption: validateAnyAdoptionManifest(JSON.parse(raw.toString("utf8"))),
      path,
    };
  } catch (error) {
...
export async function readAdoption({ repositoryRoot }) {
  ...
  if (record.adoption.schema_version === 1) {
    const rawInstructions = await readOptionalBytes(root, adoptionInstructionsPath(root));
    if (
      rawInstructions === null
      || !rawInstructions.equals(Buffer.from(renderLegacyAdoptionInstructions(record.adoption), "utf8"))
    ) {
      throw new CliError("Legacy tracked v0.6 adoption instructions do not match its schema-v1 manifest");
    }
  }
