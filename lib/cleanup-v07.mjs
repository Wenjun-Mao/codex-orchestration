...
const CLEANUP_KIND = "codex-flow-v07-cleanup-plan";
...
function originalWorktreePath(creation, archive) {
  ...
  return creation.worktree_binding?.state === "completed"
    ? creation.worktree_binding.worktree_path
    : null;
}
...
export function cleanupPlanDigestV07(value) {
  return sha256(stableStringify(cleanupPlanSeed(validateCleanupPlanV07(value))));
}

export function validateCleanupPlanV07(value) {
  requireExactFields(value, {
    required: [
  ...
  if (value.schema_version !== 1 || value.kind !== CLEANUP_KIND) {
    throw new CliError("Invalid v0.7 cleanup plan");
  }
  ...
  if (plan.mutation_performed !== false) throw new CliError("v0.7 cleanup plan must be read-only");
...
export async function cleanupPlanV07({ stateRoot, runId }) {
  ...
  if (requestedStateRoot !== v07RuntimeRoot(commonDir)) {
    throw new CliError("v0.7 cleanup plan requires the exact v0.7 state root");
  }
  ...
  const [creations, releases, callbacks, dispositions, integrations, archives] = await Promise.all([
    ...
    listRecords(requestedStateRoot, "callbacks/journal", validateCallbackRecordV07, "callback records"),
    listRecords(requestedStateRoot, "dispositions/records", validateDispositionRecord, "disposition records"),
    listRecords(requestedStateRoot, "integration-lifecycle/records", validateIntegrationRecordV07, "integration records"),
