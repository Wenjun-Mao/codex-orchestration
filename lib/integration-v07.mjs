...
const INTEGRATION_KIND = "codex-flow-v07-serial-integration";
...
function authority(record) {
  return Object.fromEntries([
    ...CANONICAL_IDENTITY_KEYS, "release_record_digest", "callback_id",
...
export function integrationRecordDigest(value) {
  const record = validateIntegrationRecordV07(value);
...
export function validateIntegrationRecordV07(value) {
  requireExactFields(value, {
    required: [
      ...
      "task_id", "task_digest", "contract_id", "operation_id", "release_id",
      "executor_thread_id", "release_record_digest", "callback_id", "receipt_digest",
  ...
  if (value.schema_version !== 1 || value.kind !== INTEGRATION_KIND) {
    throw new CliError("Invalid v0.7 serial integration record");
  }
...
async function boundAuthority({ stateRoot, dispositionId, commonDir }) {
  ...
  if (
    !identityMatches(release, receipt, canonicalReceiptKeys.filter((key) => key !== "executor_thread_id"))
    || release.ready_thread_id !== receipt.executor_thread_id
    || !identityMatches(release.coordinator_binding, receipt.recipient, [
      "lineage_id", "thread_id", "generation", "binding_digest",
    ])
...
async function readRecord(stateRoot, id) {
  ...
  return validateIntegrationRecordV07(value);
}
...
async function integrationRecords(stateRoot) {
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    records.push(validateIntegrationRecordV07(await readJson(
...
export async function prepareSerialIntegration({
  ...
  const base = {
    ...
    coordinator_binding: bindings.release.coordinator_binding,
  ...
  const record = validateIntegrationRecordV07({
    ...base,
  ...
  return withProcessLock({
    ...
    if (existing) {
      const validated = validateIntegrationRecordV07(existing);
...
export async function reconcileSerialIntegration({
  ...
  return withProcessLock({
    ...
    const validated = validateIntegrationRecordV07(reconciled);
