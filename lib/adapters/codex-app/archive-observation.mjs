...
export function codexAppArchiveToNativeObservation(value) {
  const evidence = validateCodexAppHostEvidence(value, "Codex App archive evidence");
  if (evidence.evidence_type !== "archive" || evidence.classification !== "current") {
    throw new CliError("Codex App archive evidence does not prove one exact archived task");
  }
  return {
    execution_kind: "task-thread",
    thread_id: evidence.claims.thread_id,
    source: "typed-host-evidence-v1",
    active_visible: false,
    archived_visible: true,
    host_evidence_digest: evidence.binding_digest,
