...
async function findSessionFile(codexHome, readyThreadId) {
  ...
  const suffix = `-${readyThreadId}.jsonl`;
  const candidates = [];
    ...
    for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      if (entry.isSymbolicLink()) {
        if (entry.name.endsWith(suffix)) {
          throw new CliError("Codex App task session candidate is a symbolic link");
        }
        continue;
      }
  ...
  if (candidates.length !== 1) {
    throw new CliError("Codex App private task session is missing or ambiguous");
  }
  return candidates[0];
}
...
export async function resolveCodexAppPrivateTask({
  ...
  if (Date.parse(resolvedAt) >= Date.parse(deadline)) {
    throw new CliError("Private task resolution window is closed");
  }
  ...
  const binding = privateBinding(atom, provisional);
