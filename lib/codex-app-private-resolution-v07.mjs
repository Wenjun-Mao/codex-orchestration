...
// Long-lived coordinator sessions can legitimately span many releases and
// multiple GiB. Keep the compatibility scan finite, but size the bound from
// observed App histories rather than the much smaller executor-session cap.
export const MAX_SOURCE_SESSION_BYTES = 16 * 1024 * 1024 * 1024;
const MAX_SESSION_ENTRIES = 50_000;
const MAX_SESSION_LINES = 20_000;
export const MAX_SOURCE_SESSION_LINES = 1_000_000;
// Tool outputs can make one otherwise valid coordinator event substantially
// larger than ordinary session rows. A production history contained a 12.2
// MiB row, so retain bounded memory with reviewed headroom rather than the old
// 4 MiB assumption.
export const MAX_SOURCE_SESSION_LINE_BYTES = 32 * 1024 * 1024;
...
function sourceCreateCompletion(row) {
  ...
  // Newer App rollouts record the same completed MCP call as a
  // mcp_tool_call_end event. This shape omits payload.thread_id, so the
  // filename plus primary source session_meta remain coordinator authority.
  if (payload.type === "mcp_tool_call_end" && isPlainObject(payload.invocation)) {
    const invocation = payload.invocation;
    const result = isPlainObject(payload.result)
      && stableStringify(Object.keys(payload.result).sort()) === stableStringify(["Ok"])
      ? payload.result.Ok
      : null;
    return {
      thread_id: null,
...
async function scanStableSourceSession(root, path, label, bootstrap, sourceThreadId) {
  await assertNoSymlinkComponents(root, path, label);
  const beforePath = await lstat(path).catch((error) => {
    if (error?.code === "ENOENT") throw new CliError(`${label} is unavailable`);
    throw error;
  });
  if (!beforePath.isFile() || beforePath.size < 1 || beforePath.size > MAX_SOURCE_SESSION_BYTES) {
    throw new CliError(`${label} must be a bounded regular file`);
  }
  let handle = null;
  let before;
  let primaryMetadataId = null;
  let sourceMetadataCount = 0;
  let matchingCompletionCount = 0;
  let matchingCompletion = null;
  ...
  const consume = (line) => {
    ...
    if (row.type === "session_meta") {
      if (!isPlainObject(row.payload)) {
        throw new CliError(`${label} contains malformed session metadata`);
      }
      if (lineCount === 1) primaryMetadataId = row.payload.id;
      if (row.payload.id === sourceThreadId) sourceMetadataCount += 1;
    } else if (lineCount === 1) {
      throw new CliError(`${label} must begin with canonical session metadata`);
    }
  ...
  try {
    try {
      ...
      if (["ELOOP", "ENOENT"].includes(error?.code)) {
        throw new CliError(`${label} is unavailable or no longer a regular file`, 75);
      }
      throw error;
    }
    ...
    ) throw new CliError(`${label} changed before it could be read`, 75);
    ...
    if (
      ...
      || verificationBytes !== before.size
      || verificationHash.digest("hex") !== sourceDigest
    ) throw new CliError(`${label} changed while it was being read`, 75);
  } finally {
  ...
  const after = await lstat(path).catch((error) => {
    if (error?.code === "ENOENT") throw new CliError(`${label} changed while it was being read`, 75);
    throw error;
  });
  if (
    !after.isFile()
...
function sourceCreateEvidence({
  ...
  if (primaryMetadataId !== sourceThreadId || sourceMetadataCount < 1) {
    throw new CliError("Codex App source session metadata does not match the coordinator task");
  }
  if (matchingCompletionCount !== 1 || matchingCompletion === null) {
  ...
  if (
    (completion.thread_id !== null && completion.thread_id !== sourceThreadId)
    || stableStringify(completion.arguments) !== stableStringify(expectedArguments)
  ) throw new CliError("Codex App source create_thread completion does not match exact selectors and placement");
  const hostResult = createThreadClientId(completion.result);
...
export async function resolveCodexAppPrivateTask({
  ...
  const source = sourceCreateEvidence({
    primaryMetadataId: sourceSessionFile.primaryMetadataId,
    sourceMetadataCount: sourceSessionFile.sourceMetadataCount,
    matchingCompletionCount: sourceSessionFile.matchingCompletionCount,
    matchingCompletion: sourceSessionFile.matchingCompletion,
