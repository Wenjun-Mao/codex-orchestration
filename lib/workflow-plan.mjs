...
import { normalizeOwnedPath } from "./repository-paths-v07.mjs";
...
function acceptedDispositionAuthority(value, label) {
  ...
  if (value.schema_version !== 1 || value.kind !== "codex-flow-v07-task-disposition") {
    throw new CliError(`${label} is not a durable v0.7 task disposition`);
  }
...
function validateDependencyAuthorities(value, task, plan, authority) {
  ...
  const dependencies = value.map((entry, index) => {
    ...
    const dependency = entry?.kind === "codex-flow-v07-task-disposition"
