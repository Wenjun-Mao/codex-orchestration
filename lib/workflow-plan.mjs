...
function acceptedDispositionAuthority(value, label) {
  requireExactFields(value, {
    required: [
      ...
      "contract_id", "operation_id", "release_id", "executor_thread_id", "callback_id",
  ...
  if (value.schema_version !== 1 || value.kind !== "codex-flow-v06-task-disposition") {
    throw new CliError(`${label} is not a durable v0.6 task disposition`);
  }
  ...
  const record = {
    ...
    operation_id: requireText(value.operation_id, `${label}.operation_id`, {
      max: 128,
      safeId: true,
    }),
    release_id: requireText(value.release_id, `${label}.release_id`, { max: 128, safeId: true }),
...
function acceptedSubagentAuthority(value, label) {
  requireExactFields(value, {
    required: [
      "schema_version", "kind", "operation_id", "task_contract_id", "run_id",
      "runtime_context_digest", "configuration_digest", "repository_id", "common_dir",
      "coordinator_binding", "plan_id", "revision_digest", "task_id", "task_digest",
  ...
  const operation = {
    ...
    task_contract_id: requireDigest(value.task_contract_id, `${label}.task_contract_id`),
    ...authority,
    plan_id: requireText(value.plan_id, `${label}.plan_id`, { max: 128, safeId: true }),
    revision_digest: requireDigest(value.revision_digest, `${label}.revision_digest`),
    task_id: taskId,
...
function validateDependencyAuthorities(value, task, plan, authority) {
  ...
  const dependencies = value.map((entry, index) => {
    ...
    const dependency = entry?.kind === "codex-flow-v06-task-disposition"
    ...
    if (
      dependency.run_id !== authority.run_id
      || dependency.plan_id !== plan.plan_id
      || dependency.revision_digest !== plan.revision_digest
    ) {
      throw new CliError(`${label} belongs to a different run or workflow revision`);
    }
    return dependency;
