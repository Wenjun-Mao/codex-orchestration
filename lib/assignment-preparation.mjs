...
function seed(value) {
  const { preparation_id: ignored, ...rest } = value;
  return rest;
}
...
export async function prepareCoordinatorAssignment({ commonDir, approvedPlanPath, approvedPlanDigest, recipient: recipientInput, iterationLabel, purpose, outcome, scope, acceptanceCriteria, constraints, reasons, now = Date.now() }) {
  ...
  await withProcessLock({ path: location.lock, guardRoot: commonDir, label: `assignment preparation ${record.preparation_id}` }, async () => {
    ...
    if (existing !== null && stableStringify(validateAssignmentPreparation(existing)) !== stableStringify(record)) throw new CliError("Assignment preparation conflicts", 73);
    if (existing === null) await atomicWriteJson(location.record, record, { guardRoot: commonDir, mode: 0o600 });
  });
  return { state_root: stateRoot, preparation: record, text: preparedAssignmentText(record) };
}
...
export function preparedAssignmentText(recordInput) {
  ...
  sections.push("", "## Authenticated start", "", `Preparation: \`${record.preparation_id}\``, `Approved plan: \`${record.approved_plan.digest}\` at \`${record.approved_plan.snapshot_path}\``, `Reporting recipient: \`${record.recipient.thread_id}\` on \`${record.recipient.host_id}\``, "Activate the run, then register the coordinator report route using this preparation ID before substantive work.");
