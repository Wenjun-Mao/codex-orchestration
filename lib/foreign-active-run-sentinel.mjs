...
import { isSharedFlowStateDirectory } from "./runtime-context.mjs";

export const MAX_FOREIGN_RUN_NAMESPACES = 32;
export const MAX_FOREIGN_LIFECYCLE_BYTES = 64 * 1024;

async function foreignNamespaceNames({ gitCommonDirectory, currentNamespace }) {
  ...
  try {
    ...
    if (error?.code === "ENOENT") return { commonDir, namespaces: [] };
    throw error;
  }
  const sentinelLock = entries.find((entry) => entry.name === "foreign-active-run.lock");
  if (sentinelLock && (sentinelLock.isSymbolicLink() || !sentinelLock.isFile())) {
    malformed(sentinelLock.name, "sentinel lock is not a safe regular file");
  }
  ...
  const invalid = foreignEntries.find((entry) => (
    entry.isSymbolicLink() || !entry.isDirectory() || !SAFE_ID.test(entry.name)
  ));
  if (invalid) malformed(invalid.name, "namespace is not a safe real directory");
  const namespaces = foreignEntries
...
export async function foreignActiveRunCollisions({ gitCommonDirectory, currentNamespace }) {
  const { commonDir, namespaces } = await foreignNamespaceNames({
    gitCommonDirectory,
    currentNamespace,
  });
  const collisions = [];
...
export async function incompatibleFlowNamespaces(options) {
  return (await foreignNamespaceNames(options)).namespaces;
}

export async function assertNoIncompatibleFlowNamespace(options) {
  const namespaces = await incompatibleFlowNamespaces(options);
  if (namespaces.length > 0) {
    throw new CliError(
      `Clean start required before activation; incompatible Codex Flow namespace exists: ${namespaces[0]}. `
