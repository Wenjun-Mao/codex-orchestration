...
function repositoryLocatorLockPath(commonDir, senderThreadId) {
  return safeChild(resolve(commonDir, "codex-flow", "report-locators", "locks"), `${sha256(senderThreadId)}.lock.json`);
}

function repositoryLocatorRetirementPath(commonDir, senderThreadId, routeId) {
  return safeChild(
...
function activeRouteDigestForClosedRoute(route) {
  const closed = validateReportRoute(route);
  if (closed.state !== "closed") throw new CliError("Report locator retirement requires a closed route", 73);
  return sha256(stableStringify(validateReportRoute({
    ...closed,
    state: "active",
    lifecycle: {
      opened_at: closed.lifecycle.opened_at,
      closed_at: null,
      closure_reason: null,
    },
  })));
...
function retirementFor({ locator, route, cleanup, reason, recovery, retiredAt }) {
  const seed = {
    schema_version: 1,
  ...
  return validateRetirement({ ...seed, retirement_id: retirementIdFor(seed) });
}

function assertRetirementMatches({ retirement, route, stateRoot }) {
  const closed = validateReportRoute(route);
  if (
    closed.state !== "closed"
    || retirement.sender_thread_id !== closed.sender.thread_id
    || retirement.state_root !== resolve(stateRoot)
    || retirement.route_id !== closed.route_id
    || retirement.active_route_sha256 !== activeRouteDigestForClosedRoute(closed)
    || retirement.closed_route_sha256 !== sha256(stableStringify(closed))
...
async function retireLocatorUnderLock({ commonDir, locator, route, cleanup, reason, recovery, now, hooks }) {
  ...
  let retirement = retirementFor({ locator, route, cleanup, reason, recovery, retiredAt: now });
  ...
  if (existing === null) {
    ...
    const validated = validateRetirement(existing);
  ...
  return withRepositoryReportLocatorRegistration({
    ...
    if (raw === null) {
      ...
      return {
        ...
        retirement: assertRetirementMatches({
          retirement: validateRetirement(existing),
          route,
          stateRoot,
        }),
      };
 ...
 * first authenticate the full handoff and exact active route evidence.
 */
export async function retireOrphanedRepositoryReportLocator({
  ...
  return withRepositoryReportLocatorRegistration({
    ...
    if (actualRaw === null) {
      ...
      const retirement = validateRetirement(existing);
