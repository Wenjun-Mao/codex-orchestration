...
export async function assertRepositoryReportLocatorAvailable({
  ...
  const route = await assertActiveReportRoute({
    stateRoot,
...
export async function installReportHookLocator({ pluginData, stateRoot, route, packageRoot, nativeQueue }) {
  ...
  const active = await assertActiveReportRoute({
    stateRoot,
...
export async function retireRepositoryReportLocator({
  ...
  return withRepositoryReportLocatorRegistration({
    ...
    if (raw === null) {
      const existing = await readJson(retirementPath, { allowMissing: true, guardRoot: commonDir });
      if (existing === null && allowNeverInstalled) {
        return { status: "not-installed", retirement: null };
      }
...
/** Describe locator persistence without treating expected partial registration as corruption. */
export async function repositoryReportLocatorStatus({ stateRoot, route }) {
  const checkedRoute = validateReportRoute(route);
  const commonDir = guardRoot(stateRoot);
  const active = await readJson(repositoryLocatorPath(commonDir, checkedRoute.sender.thread_id), {
    allowMissing: true,
    guardRoot: commonDir,
  });
  if (active !== null) {
    const locator = validateReportHookLocator(active);
    if (
      locator.state_root !== resolve(stateRoot)
      || locator.route_id !== checkedRoute.route_id
      || locator.route_sha256 !== (checkedRoute.state === "active"
        ? sha256(stableStringify(checkedRoute))
        : activeRouteDigestForClosedRoute(checkedRoute))
    ) throw new CliError("Persisted report locator conflicts with assignment route", 73);
    return { status: "active", locator };
  }
  const retirement = await readJson(
    repositoryLocatorRetirementPath(commonDir, checkedRoute.sender.thread_id, checkedRoute.route_id),
    { allowMissing: true, guardRoot: commonDir },
  );
  if (retirement === null) return { status: "absent", locator: null, retirement: null };
  return {
    status: "retired",
    locator: null,
    retirement: assertReportLocatorRetirementMatches({
      retirement: validateReportLocatorRetirement(retirement),
      route: checkedRoute,
      stateRoot,
    }),
  };
...
export async function retireRepositoryReportLocatorsForRun({
  ...
  for (const route of await reportRoutes({ stateRoot, state: "closed" })) {
    ...
    const { assignmentAuthorityForRoute, assignmentRegistration } = await import("./assignment-authority.mjs");
    const assignment = await assignmentAuthorityForRoute({ stateRoot, routeId: route.route_id });
    const allowNeverInstalled = assignment !== null
      && assignment.state === "registering"
      && assignmentRegistration(assignment).locator === "pending";
