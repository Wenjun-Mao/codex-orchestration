...
import { validateRepositoryRelativePath } from "./repository-paths-v07.mjs";
...
export const V07_RUN_LIFECYCLE_SCHEMA_VERSION = 1;
export const V07_RUN_LIFECYCLE_KIND = "codex-flow-v07-run-lifecycle";
export const V07_RUN_ACTIVATION_KIND = "codex-flow-v07-run-activation";
export const V07_RUN_FENCE_SCHEMA_VERSION = 1;
export const V07_RUN_FENCE_KIND = "codex-flow-v07-run-fence";
...
export function validateFencePlan(value, label = "run fence") {
  ...
  if (value.schema_version !== V07_RUN_FENCE_SCHEMA_VERSION) {
    throw new CliError(`${label}.schema_version must be ${V07_RUN_FENCE_SCHEMA_VERSION}`);
  }
  if (value.kind !== V07_RUN_FENCE_KIND) {
    throw new CliError(`${label}.kind must be ${V07_RUN_FENCE_KIND}`);
  }
  ...
  return {
    schema_version: V07_RUN_FENCE_SCHEMA_VERSION,
    kind: V07_RUN_FENCE_KIND,
...
export function validateRunActivation(value, label = "run activation") {
  ...
  if (value.schema_version !== V07_RUN_LIFECYCLE_SCHEMA_VERSION) {
    throw new CliError(`${label}.schema_version must be ${V07_RUN_LIFECYCLE_SCHEMA_VERSION}`);
  }
  if (value.kind !== V07_RUN_ACTIVATION_KIND) {
    throw new CliError(`${label}.kind must be ${V07_RUN_ACTIVATION_KIND}`);
  }
  ...
  const activation = {
    schema_version: V07_RUN_LIFECYCLE_SCHEMA_VERSION,
    kind: V07_RUN_ACTIVATION_KIND,
...
export function validateRunLifecycleState(value) {
  ...
  if (value.schema_version !== V07_RUN_LIFECYCLE_SCHEMA_VERSION) {
    throw new CliError(`run lifecycle state.schema_version must be ${V07_RUN_LIFECYCLE_SCHEMA_VERSION}`);
  }
  if (value.kind !== V07_RUN_LIFECYCLE_KIND) {
    throw new CliError(`run lifecycle state.kind must be ${V07_RUN_LIFECYCLE_KIND}`);
  }
...
export function runLifecyclePath(gitCommonDirectory) {
  return join(v07RuntimeRoot(gitCommonDirectory), "runs", "lifecycle.json");
}

function lifecycleLockPath(gitCommonDirectory) {
  return join(v07RuntimeRoot(gitCommonDirectory), "locks", "run-lifecycle.lock");
}
...
function activeRunMutationLockPath(gitCommonDirectory, runId) {
  return join(
    v07RuntimeRoot(gitCommonDirectory),
    "locks",
...
async function prepareLifecycleRoot(gitCommonDirectory) {
  ...
  await ensureDirectory(v07RuntimeRoot(commonDir), {
    guardRoot: commonDir,
    mode: 0o700,
  });
  return commonDir;
...
function assertActiveRun(state, runId) {
  ...
  if (!run) throw new CliError(`Unknown v0.7 run: ${runId}`);
  if (state.active_run_id !== runId || run.status !== "active") {
    throw new CliError(`v0.7 run is not active: ${runId}`);
  }
  return run;
}
...
export async function withActiveRunMutation({ gitCommonDirectory, runId }, operation) {
  ...
  return withProcessLock({
    ...
    guardRoot: commonDir,
    label: `v0.7 active run mutation ${safeRunId}`,
  }, async () => {
    const state = await loadLifecycleState(commonDir);
  ...
  return updateLifecycleState(commonDir, async (state, lifecycleCommonDir) => {
    if (state.active_run_id !== null) {
      ...
      if (active.run_id === safeRunId) {
        throw new CliError(`v0.7 run admission does not match its immutable activation: ${safeRunId}`);
      }
      throw new CliError(`A different v0.7 run is already active: ${state.active_run_id}`);
    }
    if (state.runs[safeRunId]) {
      throw new CliError(`v0.7 run id has already been used: ${safeRunId}`);
    }
...
export async function readRun({ gitCommonDirectory, runId }) {
  ...
  if (!run) throw new CliError(`Unknown v0.7 run: ${safeRunId}`);
...
export async function resumeRun({ gitCommonDirectory, runId, resume }) {
  ...
  if (run.status !== "active") throw new CliError(`v0.7 run is not active: ${safeRunId}`);
...
export async function listRunLifecycleFiles({ gitCommonDirectory }) {
  const commonDir = await prepareLifecycleRoot(gitCommonDirectory);
  const root = join(v07RuntimeRoot(commonDir), "runs");
