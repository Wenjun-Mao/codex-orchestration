...
import { validateRepositoryRelativePath } from "./config.mjs";
import {
  readRuntimeContext,
...
export function validateFencePlan(value, label = "run fence") {
  requireExactFields(value, {
    required: [
      ...
      "branch_fences", "operation_fences",
    ],
  }, label);
  ...
  const operations = requireStringArray(value.operation_fences, `${label}.operation_fences`, {
    maxItems: 128,
    maxText: 128,
    safeIds: true,
  });
  return {
    ...
    operation_fences: sortedStrings(operations),
  };
}
...
export function buildFencePlan({
  ...
  return validateFencePlan({
    ...
    operation_fences: operationFences,
  });
}
...
export function fencePlanIsEmpty(value) {
  ...
  return (
    ...
    && plan.operation_fences.length === 0
  );
}
...
export function fencePlanConflicts(left, right) {
  ...
  return [
    ...
    ...exactFenceConflicts("operation", expected.operation_fences, candidate.operation_fences),
  ];
}
...
function fencePlanIsCoveredBy(planned, unresolved) {
  const source = validateFencePlan(planned, "planned run fence");
  const candidate = validateFencePlan(unresolved, "unresolved run fence");
  return (
    candidate.path_fences.every((path) => source.path_fences.some(
      (plannedPath) => path === plannedPath || path.startsWith(`${plannedPath}/`),
    ))
    && candidate.resource_fences.every((resource) => source.resource_fences.includes(resource))
    && candidate.branch_fences.every((branch) => source.branch_fences.includes(branch))
    && candidate.operation_fences.every((operation) => source.operation_fences.includes(operation))
  );
}
...
function validateRunBinding(value, label = "run binding") {
  requireExactFields(value, {
    required: [
      "runtime_context_hash", "config_hash", "repository_hash", "host",
      "lineage", "generation", "fence_token",
    ],
  }, label);
  return {
...
function validateTerminal(value, status, label) {
  ...
  const unresolved = validateFencePlan(value.unresolved_fences, `${label}.unresolved_fences`);
  if (fencePlanIsEmpty(unresolved)) {
    throw new CliError("abandoned runs must retain at least one unresolved fence");
  }
  return {
    kind: "abandoned",
...
export function validateRunActivation(value, label = "run activation") {
  requireExactFields(value, {
    required: [
      ...
      "status", "plan", "binding", "rebind_history", "admitted_at", "updated_at", "terminal",
    ],
  }, label);
  ...
  const activation = {
    ...
    runtime_id: requireText(value.runtime_id, `${label}.runtime_id`, { max: 128, safeId: true }),
    runtime_context_hash: validateDigest(value.runtime_context_hash, `${label}.runtime_context_hash`),
    status,
...
function exactActivationMatches(run, { runtimeId, runtimeContextHash, plan }) {
  return (
...
export async function abandonRun({
  ...
  const unresolved = validateFencePlan(unresolvedFences, "unresolvedFences");
  if (fencePlanIsEmpty(unresolved)) {
    throw new CliError("abandoned runs must retain at least one unresolved fence");
  }
  ...
  return updateLifecycleState(gitCommonDirectory, async (state, commonDir) => {
    const current = assertActiveRun(state, safeRunId);
    assertResume(current, resume);
    if (!fencePlanIsCoveredBy(current.plan, unresolved)) {
      throw new CliError("unresolved fences must be covered by the admitted run plan");
    }
    const run = validateRunActivation({
      ...current,
      status: "abandoned",
