...
import { assertNoForeignActiveRunCollision } from "./foreign-active-run-sentinel.mjs";
import {
  readRuntimeContext,
...
function activeRunMutationLockPath(gitCommonDirectory, runId) {
  return join(
    runtimeRoot(gitCommonDirectory),
    "locks",
...
export async function admitRun(options) {
  ...
  }, () => admitRunWhileRepositoryLocked({ ...options, gitCommonDirectory: commonDir }));
}

export async function admitRunWithRepositoryLockHeld({ repositoryLockToken, ...options }) {
  ...
  return admitRunWhileRepositoryLocked({ ...options, gitCommonDirectory: commonDir });
}
