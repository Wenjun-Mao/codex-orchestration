...
import { validateRepositoryRelativePath } from "./repository-paths.mjs";
import { consumeRefreshAdmissionCapability } from "./refresh-admission-capability.mjs";
import {
  assertNoForeignActiveRunCollision,
  incompatibleFlowNamespaces,
} from "./foreign-active-run-sentinel.mjs";
import {
  readRuntimeContext,
...
export const RUN_LIFECYCLE_SCHEMA_VERSION = 1;
export const RUN_LIFECYCLE_KIND = "codex-flow-v09-run-lifecycle";
export const RUN_ACTIVATION_KIND = "codex-flow-v09-run-activation";
export const RUN_FENCE_SCHEMA_VERSION = 1;
export const RUN_FENCE_KIND = "codex-flow-v09-run-fence";
...
export function validateFencePlan(value, label = "run fence") {
  ...
  if (value.schema_version !== RUN_FENCE_SCHEMA_VERSION) {
    throw new CliError(`${label}.schema_version must be ${RUN_FENCE_SCHEMA_VERSION}`);
  }
  if (value.kind !== RUN_FENCE_KIND) {
    throw new CliError(`${label}.kind must be ${RUN_FENCE_KIND}`);
  }
  ...
  return {
    schema_version: RUN_FENCE_SCHEMA_VERSION,
    kind: RUN_FENCE_KIND,
...
export function validateRunBinding(value, label = "run binding") {
  requireExactFields(value, {
    required: [
...
export function validateRunActivation(value, label = "run activation") {
  ...
  if (value.schema_version !== RUN_LIFECYCLE_SCHEMA_VERSION) {
    throw new CliError(`${label}.schema_version must be ${RUN_LIFECYCLE_SCHEMA_VERSION}`);
  }
  if (value.kind !== RUN_ACTIVATION_KIND) {
    throw new CliError(`${label}.kind must be ${RUN_ACTIVATION_KIND}`);
  }
...
export function validateRunLifecycleState(value) {
  ...
  if (value.schema_version !== RUN_LIFECYCLE_SCHEMA_VERSION) {
    throw new CliError(`run lifecycle state.schema_version must be ${RUN_LIFECYCLE_SCHEMA_VERSION}`);
  }
  if (value.kind !== RUN_LIFECYCLE_KIND) {
    throw new CliError(`run lifecycle state.kind must be ${RUN_LIFECYCLE_KIND}`);
  }
...
export function runLifecyclePath(gitCommonDirectory) {
  return join(runtimeRoot(gitCommonDirectory), "runs", "lifecycle.json");
}

function lifecycleLockPath(gitCommonDirectory) {
  return join(runtimeRoot(gitCommonDirectory), "locks", "run-lifecycle.lock");
}

function foreignActiveRunLockPath(gitCommonDirectory) {
  return join(resolve(gitCommonDirectory), "codex-flow", "foreign-active-run.lock");
}

function activeRunMutationLockPath(gitCommonDirectory, runId, stateRoot = null) {
  return join(
    stateRoot ?? runtimeRoot(gitCommonDirectory),
    "locks",
...
async function prepareLifecycleRoot(gitCommonDirectory) {
  ...
  await ensureDirectory(runtimeRoot(commonDir), {
    guardRoot: commonDir,
    mode: 0o700,
  });
  return commonDir;
...
async function persistAssignmentRunRetirement({ gitCommonDirectory, run }) {
  const {
    assignmentForExecutionRetirement,
    assignmentStateRoot,
    recordAssignmentExecutionRetirement,
  } = await import("./assignment-authority.mjs");
  const commonDir = resolve(gitCommonDirectory);
  const stateRoot = assignmentStateRoot(commonDir);
  const assignment = await assignmentForExecutionRetirement({
    stateRoot,
    hostId: run.binding.host.host_id,
    threadId: run.binding.lineage.thread_id,
    runId: run.run_id,
  });
  ...
  const binding = assignment.execution_bindings.find((entry) => (
    entry.run_id === run.run_id && entry.runtime_context_digest === run.runtime_context_hash
  ));
  if (binding === undefined) {
    throw new CliError("Terminal run is not bound to its open assignment", 73);
  }
  const terminalDigest = sha256(stableStringify(run.terminal));
  const observedAt = run.status === "closed" ? run.terminal.closed_at : run.terminal.abandoned_at;
...
function assertActiveRun(state, runId) {
  ...
  if (!run) throw new CliError(`Unknown v0.9 run: ${runId}`);
  if (state.active_run_id !== runId || run.status !== "active") {
    throw new CliError(`v0.9 run is not active: ${runId}`);
  }
  return run;
}
...
export async function withActiveRunMutation({ gitCommonDirectory, runId, stateRoot = null }, operation) {
  ...
  const commonDir = await prepareLifecycleRoot(gitCommonDirectory);
  const exactStateRoot = stateRoot === null ? runtimeRoot(commonDir) : resolve(stateRoot);
  if (dirname(exactStateRoot) !== resolve(commonDir, "codex-flow")) {
    throw new CliError("Active-run mutation requires an exact versioned state root", 73);
  }
  return withProcessLock({
    path: activeRunMutationLockPath(commonDir, safeRunId, exactStateRoot),
    guardRoot: commonDir,
    label: `v0.9 active run mutation ${safeRunId}`,
  }, async () => {
    const raw = await readJson(resolve(exactStateRoot, "runs", "lifecycle.json"), {
      allowMissing: true,
      guardRoot: commonDir,
    });
    const state = raw === null ? emptyLifecycleState() : validateRunLifecycleState(raw);
...
export async function retainedFenceConflicts(state, plan, commonDir) {
  const conflicts = [];
  for (const run of Object.values(state.runs)) {
    if (run.status !== "abandoned") continue;
    const runConflicts = fencePlanConflicts(run.terminal.unresolved_fences, plan);
    if (runConflicts.length === 0) continue;
    let settlementBlocker = null;
    try {
      const { assessRetainedRunSettlement } = await import("./retained-obligation-settlement.mjs");
      ...
      settlementBlocker = error.message;
    }
    conflicts.push(...runConflicts.map((conflict) => ({
      run_id: run.run_id,
      ...conflict,
      settlement_blocker: settlementBlocker,
    })));
  }
  return conflicts;
}

async function foreignRetainedFenceConflicts(commonDir, plan, excludedNamespace = null) {
  const namespaces = await incompatibleFlowNamespaces({
    gitCommonDirectory: commonDir,
    currentNamespace: RUNTIME_DIRECTORY,
  });
  const conflicts = [];
  for (const namespace of namespaces) {
    if (namespace === excludedNamespace || !/^v0\.9(?:\.\d+(?:-[0-9A-Za-z.-]+)?)?$/.test(namespace)) {
      continue;
    }
    const path = resolve(commonDir, "codex-flow", namespace, "runs", "lifecycle.json");
    const raw = await readJson(path, { allowMissing: true, guardRoot: commonDir });
    if (raw === null) {
      throw new CliError(`Retained-fence admission cannot authenticate historical namespace: ${namespace}`, 73);
    }
    const state = validateRunLifecycleState(raw);
    for (const run of Object.values(state.runs)) {
      if (run.status !== "abandoned") continue;
      const runConflicts = fencePlanConflicts(run.terminal.unresolved_fences, plan);
      if (runConflicts.length === 0) continue;
      ...
      try {
        const { assessRetainedRunSettlement } = await import("./retained-obligation-settlement.mjs");
        const settlement = await assessRetainedRunSettlement({ commonDir, namespace, run });
        if (settlement.status === "settled") continue;
      } catch (error) {
        if (!(error instanceof CliError)) throw error;
        settlementBlocker = error.message;
...
async function admitRunWhileRepositoryLocked({
  gitCommonDirectory,
  runId,
  runtimeId,
  workflowPlanId,
  ...
  plan,
  admittedAt,
  refreshAdmissionCapability = null,
}) {
  const safeRunId = requireText(runId, "runId", { max: 128, safeId: true });
  ...
  if (normalizedPlan.branch_fences.includes(context.repository.branch)) {
    throw new CliError(
      `Run branch fences must not include the coordinator repository branch: ${context.repository.branch}`,
      73,
    );
  }
  ...
  const commonDir = await prepareLifecycleRoot(gitCommonDirectory);
  let excludedRetainedNamespace = null;
  if (refreshAdmissionCapability !== null) {
    const runtimeBinding = runtimeBindingFromContext(context);
    const exclusion = consumeRefreshAdmissionCapability({
      capability: refreshAdmissionCapability,
      expectedTarget: {
        run_id: safeRunId,
        runtime_context_digest: runtimeBinding.runtime_context_hash,
        ...
        plan_id: safeWorkflowPlanId,
        revision_digest: workflowRevision,
        namespace: RUNTIME_DIRECTORY,
        bound_at: timestamp,
        assignment_id: refreshAdmissionCapability.target?.assignment_id ?? null,
      },
    });
    excludedRetainedNamespace = exclusion.namespace;
  }
  await assertNoForeignActiveRunCollision({
    gitCommonDirectory: commonDir,
    currentNamespace: RUNTIME_DIRECTORY,
  });
  const foreignConflicts = await foreignRetainedFenceConflicts(
    commonDir,
    normalizedPlan,
    excludedRetainedNamespace,
  );
  if (foreignConflicts.length > 0) {
    const conflict = foreignConflicts[0];
    throw new CliError(
      `Run plan conflicts with retained fences from ${conflict.namespace}/${conflict.run_id}`
      + (conflict.settlement_blocker === null ? "" : `: ${conflict.settlement_blocker}`),
      73,
    );
  }
  return updateLifecycleState(commonDir, async (state, lifecycleCommonDir) => {
    if (state.active_run_id !== null) {
      ...
      if (
        ...
        return {
          ...
          run: clone(active),
          path: runLifecyclePath(lifecycleCommonDir),
        };
      }
      if (active.run_id === safeRunId) {
        throw new CliError(`v0.9 run admission does not match its immutable activation: ${safeRunId}`);
      }
      throw new CliError(`A different v0.9 run is already active: ${state.active_run_id}`);
    }
    if (state.runs[safeRunId]) {
      throw new CliError(`v0.9 run id has already been used: ${safeRunId}`);
    }
    const conflicts = await retainedFenceConflicts(state, normalizedPlan, lifecycleCommonDir);
    if (conflicts.length > 0) {
      throw new CliError(
        `Run plan conflicts with retained fences from ${conflicts[0].run_id}`
        + (conflicts[0].settlement_blocker === null ? "" : `: ${conflicts[0].settlement_blocker}`),
      );
    }
...
export async function admitRun(options) {
  const commonDir = await prepareLifecycleRoot(options.gitCommonDirectory);
  return withProcessLock({
    path: foreignActiveRunLockPath(commonDir),
    guardRoot: commonDir,
    label: "foreign active-run collision sentinel",
  }, () => admitRunWhileRepositoryLocked({
    ...options,
    gitCommonDirectory: commonDir,
    refreshAdmissionCapability: null,
  }));
...
export async function admitRunWithRepositoryLockHeld({
  repositoryLockToken,
  refreshAdmissionCapability = null,
  ...options
}) {
  requireExactFields(repositoryLockToken, {
    required: ["schema_version", "kind", "host", "pid", "token", "started_at"],
  }, "repositoryLockToken");
  const commonDir = await prepareLifecycleRoot(options.gitCommonDirectory);
  const persistedLock = await readJson(foreignActiveRunLockPath(commonDir), {
    guardRoot: commonDir,
  });
  if (stableStringify(persistedLock) !== stableStringify(repositoryLockToken)) {
    throw new CliError("Refresh admission does not hold the repository-wide run lock", 73);
  }
  return admitRunWhileRepositoryLocked({
    ...options,
    gitCommonDirectory: commonDir,
...
export async function readRun({ gitCommonDirectory, runId }) {
  ...
  if (!run) throw new CliError(`Unknown v0.9 run: ${safeRunId}`);
...
export async function resumeRun({ gitCommonDirectory, runId, resume }) {
  ...
  if (run.status !== "active") throw new CliError(`v0.9 run is not active: ${safeRunId}`);
...
export async function closeRun({
  ...
  const result = await updateLifecycleState(gitCommonDirectory, async (state, commonDir) => {
    const current = assertActiveRun(state, safeRunId);
    assertResume(current, resume);
    const run = validateRunActivation({
      ...current,
  ...
  const { closeReportRoutesForRun } = await import("./report-routes.mjs");
  await persistAssignmentRunRetirement({ gitCommonDirectory, run: result.run });
  await closeReportRoutesForRun({
    stateRoot: runtimeRoot(gitCommonDirectory),
    runId: safeRunId,
    reason: "terminal",
...
export async function listRunLifecycleFiles({ gitCommonDirectory }) {
  const commonDir = await prepareLifecycleRoot(gitCommonDirectory);
  const root = join(runtimeRoot(commonDir), "runs");
