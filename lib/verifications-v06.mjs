...
import { resolveTaskReleaseExecutorWorktree } from "./release-lifecycle.mjs";
...
function validateIntegrationScope(value, identity = null) {
  ...
  requireExactFields(value, {
    required: [
      ...
      "release_id", "main_branch", "reconciled_main_tip", "outcome",
    ],
  ...
  const scope = {
    ...
    operation_id: requireText(value.operation_id, "integration_scope.operation_id", {
      max: 128,
      safeId: true,
    }),
    release_id: requireText(value.release_id, "integration_scope.release_id", {
      max: 128,
      safeId: true,
    }),
...
export async function resolveNoChangeVerificationSubject({ stateRoot, receipt }) {
  ...
  const executor = await resolveTaskReleaseExecutorWorktree({
    stateRoot,
    releaseId: terminalReceipt.release_id,
  });
  ...
  return {
    release_id: terminalReceipt.release_id,
