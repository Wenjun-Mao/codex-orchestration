...
const VERIFICATION_KIND = "codex-flow-v06-combined-verification";
const INDEX_KIND = "codex-flow-v06-verification-request-index";
...
function validateIdentity(value) {
  requireExactFields(value, {
    required: [
      "callback_id", "receipt_digest", "recipient_binding_digest", "executor_id",
      "run_id", "runtime_digest", "config_digest", "plan_id", "revision_id",
      "task_id", "task_contract_digest", "operation_id", "release_id",
    ],
  ...
  return {
    ...
    executor_id: requireText(value.executor_id, "identity.executor_id", { max: 128, safeId: true }),
    run_id: requireText(value.run_id, "identity.run_id", { max: 128, safeId: true }),
    runtime_digest: digest(value.runtime_digest, "identity.runtime_digest"),
    config_digest: digest(value.config_digest, "identity.config_digest"),
    plan_id: requireText(value.plan_id, "identity.plan_id", { max: 128, safeId: true }),
    revision_id: requireText(value.revision_id, "identity.revision_id", { max: 128, safeId: true }),
    task_id: requireText(value.task_id, "identity.task_id", { max: 128, safeId: true }),
    task_contract_digest: digest(value.task_contract_digest, "identity.task_contract_digest"),
    operation_id: requireText(value.operation_id, "identity.operation_id", { max: 128, safeId: true }),
    release_id: requireText(value.release_id, "identity.release_id", { max: 128, safeId: true }),
  };
}
...
function validateIntegrationScope(value, identity = null) {
  ...
  requireExactFields(value, {
    required: [
      "integration_id", "integration_record_digest", "run_id", "runtime_digest",
      "config_digest", "plan_id", "revision_id", "task_id", "task_contract_digest",
  ...
  const scope = {
    ...
    runtime_digest: digest(value.runtime_digest, "integration_scope.runtime_digest"),
    config_digest: digest(value.config_digest, "integration_scope.config_digest"),
    plan_id: requireText(value.plan_id, "integration_scope.plan_id", { max: 128, safeId: true }),
    revision_id: requireText(value.revision_id, "integration_scope.revision_id", {
      max: 128,
      safeId: true,
    }),
    task_id: requireText(value.task_id, "integration_scope.task_id", { max: 128, safeId: true }),
    task_contract_digest: digest(
      value.task_contract_digest,
      "integration_scope.task_contract_digest",
    ),
...
export function validateVerificationRecordV06(value) {
  requireExactFields(value, {
    required: [
  ...
  if (value.schema_version !== 1 || value.kind !== VERIFICATION_KIND) {
    throw new CliError("Invalid v0.6 combined verification record");
  }
...
export function verificationRecordDigest(value) {
  const record = validateVerificationRecordV06(value);
...
async function canonicalRepositoryContext({ stateRoot, repositoryPath, target }) {
...
async function listVerificationRecords(stateRoot) {
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    verificationId(id);
    const record = validateVerificationRecordV06(await readJson(
...
export async function runCombinedVerification({
  ...
  const terminalReceipt = validateTerminalReceiptV3(receipt);
  ...
  return withProcessLock({
    ...
    const record = validateVerificationRecordV06({
      ...base,
