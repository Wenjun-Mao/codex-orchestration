...
| Same-host final capture, shared-director recipient preservation across fresh sender fences, staged reporter upgrade survival, runtime-bundle staging, harmless unregistered stops, assignment-owned registering-to-open publication, status/resume/lawful abort, accepted-state terminal persistence, atomic accept/cancel terminal selection, exact runtime retirement, and monotonic refresh-retirement supersession | `cli-v09.test.mjs`, `assignment-lifecycle-v097.test.mjs`, `coordinator-recipient-routing-v097.test.mjs`, `report-hook-v093.test.mjs`, `report-lifecycle-v093.test.mjs`, `refresh-v09.test.mjs` |
| Assignment-owned exact worktree reclamation after typed idle or archived/no-active proof, including active/unknown rejection, owning-host action/result reconciliation, crash-safe run-archive-before-member convergence, public/private already-archived recovery, authenticated patch-equivalent preservation, named and detached cleanup, shared/dirty/drift/protected/preservation rejection, interruption, archive no-replay, append-only wrong-binding correction, and accepted worktree-plus-branch absence recovery from an exact pre-archive capture or uniquely bound completed coordinator-work result | `assignment-lifecycle-v097.test.mjs`, `coordinator-closeout-recovery-v09.test.mjs` |
| Public/private archive evidence | `codex-app-private-archive-v09.test.mjs` |
| Exact v0.8.3 and live-shaped RC7 semantic refresh, actual unfinished/completed coordinator-work claims, zero-child reissue, true no-work clean start, selected already-terminal evidence, dirty discard, branch/worktree deletion, assignment target binding, target consumption, joined crash-replay boundaries, exact closed-v0.9.0 maintenance refresh, and settled v0.9.7 coordinator admission with accepted-report/ref-preservation failures | `refresh-v09.test.mjs`, `assignment-lifecycle-v097.test.mjs`, `coordinator-closeout-recovery-v09.test.mjs` |
...
| CLI inventory, scoped help, real activation wiring, and Codex worktree-owner routing authority | `cli-v09.test.mjs`, `coordinator-closeout-recovery-v09.test.mjs` |
