...
8. **Reporting is separate from source ownership.** Bind exact sender/recipient;
   ...
   For same-host Relay, genuine hook capture followed by the exact recipient's
   public read and explicit acknowledgement establishes shared-storage receipt,
   not native message-delivery proof. Native waiting is a notification aid.
...
9. **Adoption at safe boundaries.** Existing Flow work finishes or undergoes a
   ...
   repositories requires qualification before product rollout. The subsequently
   authorized candidate installation is limited to the disposable qualification
   journey after fixture non-interference checks; it is not coexistence acceptance.
...
The user approved beginning bounded native integration after source acceptance
at `71f75b22f09180016e723edb57e5e8ca8ddb108a`. Continue with the existing delivery
coordinator, preserving that checkpoint. Implement only the small adapter needed
to connect the accepted source interface to actual task identity, final capture,
report delivery/receipt and affirmative task archive observations.

First establish the current host's documented/observable capabilities and an
isolated way to exercise them. Record the actual host/binary version; do not infer
compatibility from Flow's pinned constants. Prefer current purpose-built App tools
for host actions. No private IPC, second archival app-server, broad schema-discovery
framework or new lifecycle engine. After that bounded capability checkpoint,
attempt the connected outcome or report a precise blocker rather than continuing
instrument-only work.

Source/fixture tests remain distinct from native evidence. Prepare one disposable
repository journey with explicit ownership and actor/model choices before native
dispatch: coordinator → executor → verification → genuine final receipt →
child-first archival → useful successor. Measure fresh-agent startup on that same
journey. No repeated standalone feasibility probe. The director retains ownership
of any canary coordinator; the implementation coordinator must not silently create
a nested coordinator or assume permission to manage existing tasks.

The initial adapter phase authorized no shared installation, marketplace/cache/config
changes, hook trust changes, App restart, Flow-state changes or product-project
access. The later candidate authorization is recorded below. If a genuine final
hook cannot be exercised without those changes, preserve
the adapter candidate and report the exact required operation to the director.
Do not replace idle-final evidence with a synthetic event or broaden installation
scope. Shared coexistence qualification precedes any installation decision;
pilot/release approval remains separate.
...
### 3a. Approved amendment — shared-storage receipt (2026-09-12)

The user authorized personal Relay candidate installation, reviewed/trusted its
Stop hook, added the disposable saved project and confirmed a restart. The current
candidate is `0.1.0+codex.20260912052222`, source `c08dec4`. Flow and product
repositories remain outside the modification scope; pilot/release are not approved.

The actual journey exposed two distinct boundaries: the host returns structured
messages (the parser correction is installed), and after restart the real Stop
capture succeeded while both public wait/read APIs omitted the completed final.
The fallback also has a source-confirmed ordering gap: capture occurs after the
sender stops, but submission requires that sender active; a new Stop conflicts
with the frozen event. Do not solve this by spoofing an actor or inventing receipt.

Approved correction: use the captured report as the same-host delivery artifact.
Provide a read-only public report command for its exact recipient, returning the
frozen sender/recipient, assignment, event identity, exact final bytes/digest and
result association, plus a generated explicit acknowledgement command. Reading is
not acknowledgement; acknowledgement is not semantic acceptance. Recheck the exact
association on acknowledgement and record transport as `shared-storage`. Native
waiting may notify completion but is neither required message retrieval nor proof
of receipt. Missing capture remains blocking. No private transcript parsing,
synthetic hook events, sender reactivation, self-messaging, daemon or new transport
service. Do not add parallel authority records or a new lifecycle engine.

Keep source ownership, verification, dependency acceptance and child-first task
archival rules unchanged. A receipt cannot restore write permission or close an
unresolved source result. Pending obligations stay assignment-local. Historical
captures/receipts and the original failed observations remain preserved; never
relabel them as a successful native delivery.

Implement in the existing source-only Relay coordinator and document the durable
boundary in Relay's decision record. Test exact/wrong recipient, stale or mismatched
event/digest/association, missing capture, idempotent acknowledgement, notification
with missing text, acceptance separation and unchanged retirement gates. Prefer
the smallest relevant suite; no new audit or broad compatibility work.

After source review, install only the corrected candidate through the managed
path and resume the same coordinator/executor from commit `c7d352a`, with no source
replay or journal migration. Complete public shared-storage receipt, verification,
acceptance, child-first task archival and the useful Terra-high successor. Label
this a repaired journey, not uninterrupted-first-attempt success. An actual new
hook-trust or reload requirement remains a user boundary, not permission to bypass.
...
- Genuine hook-captured reports are read and explicitly acknowledged by the exact
  recipient through the shared-storage public interface; acceptance and eligible
  task archival complete separately. Report this transport honestly, not as native
  message delivery. Observe ambiguous host results without replay. Synthetic hook
  calls are not genuine idle-final evidence.
...
Current authorization: amended design, accepted checkpoint 2 source implementation,
and the bounded native-integration work described in checkpoint 3, within the
independent `plugins/relay` package and disposable evidence surface.
Only the candidate installation and exact disposable task retirement described in
3a are authorized; no migration, Plotloom changes or broader cleanup. Prior
