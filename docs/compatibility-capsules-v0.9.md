...
| RC8 runtime-bundle bootstrap repair | After authenticating every file in the exact `0.9.7-rc.8` runtime bundle and its known refresh-source hash, copy the bundle to temporary storage and add only the omitted root-level `package.json` allowlist clause before invoking RC8's own exporter. Repository and run state remain read-only. | Any package version, source hash, patch-site, bundle inventory, or exported-envelope mismatch blocks. The capsule never substitutes current-package workflow semantics or changes the stored source snapshot. | Retire after no supported long-lived coordinator can retain an RC8 execution namespace. |
