...
| Long-lived private task resolution stays exact | Multi-GiB source sessions stream within a finite envelope with bounded retained state; both known App completion shapes preserve a twice-read stable-prefix digest and duplicate checks while allowing append-only growth; an authenticated exact-v0.8.1 bridge emits only a reconcile request that the v0.8.1 snapshot must consume. |
