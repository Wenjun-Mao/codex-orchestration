...
import {
  cleanupPlanDigestV06,
  cleanupPlanV06,
  validateCleanupPlanV06,
} from "../lib/cleanup-v06.mjs";
import {
...
const recipient = {
  lineage_id: "cleanup-lineage-v06",
  thread_id: "cleanup-coordinator-v06",
  generation: 1,
};
...
async function completedArchiveFixture(root, remoteRoot) {
  ...
  const branch = "codex/cleanup-v06-candidate";
  const context = await createAcceptedVisibleTask(root, "cleanup-plan", {
    coordinator: recipient,
    executorBranch: branch,
  });
  const worktreeParent = await mkdtemp(resolve(tmpdir(), "codex-flow-v06-cleanup-worktree-"));
  ...
  const verification = await runCombinedVerification({
    ...
    receipt: payload,
    checks: [{
      check_id: "cleanup-v06-no-change",
...
test("v0.6 cleanup plan is deterministic, read-only, and blocks closure until exact candidate cleanup", async () => {
  const root = await createGitFixture("codex-flow-v06-cleanup-plan-");
  const remoteParent = await mkdtemp(resolve(tmpdir(), "codex-flow-v06-cleanup-remote-"));
  const remoteRoot = resolve(remoteParent, "origin.git");
  const alternateParent = await mkdtemp(resolve(tmpdir(), "codex-flow-v06-cleanup-alternate-"));
  ...
  try {
    ...
    const first = await cleanupPlanV06({ stateRoot: context.stateRoot, runId: context.contract.run_id });
    const second = await cleanupPlanV06({ stateRoot: context.stateRoot, runId: context.contract.run_id });
    assert.deepEqual(second, first);
    assert.deepEqual(validateCleanupPlanV06(first), first);
    assert.equal(cleanupPlanDigestV06(first), first.plan_id.slice("cleanup-plan-v1-".length));
    ...
    const attached = await cleanupPlanV06({ stateRoot: context.stateRoot, runId: context.contract.run_id });
...
test("every unbound run branch fence is represented and a live one blocks closure", async () => {
  const root = await createGitFixture("codex-flow-v06-cleanup-fence-");
  try {
    ...
    const live = await cleanupPlanV06({ stateRoot: context.stateRoot, runId: context.contract.run_id });
    ...
    const absent = await cleanupPlanV06({ stateRoot: context.stateRoot, runId: context.contract.run_id });
