...
test("v0.7 callback is quiet, durable, and consumable only by a disposition", async () => {
  const root = await createGitFixture("codex-flow-v07-callback-");
  try {
    ...
    assert.equal((await deliverCallbackV07({
      stateRoot: context.stateRoot,
      receipt: payload,
    ...
    await assert.rejects(
      deliverCallbackV07({ stateRoot: context.stateRoot, receipt: conflicting }),
    ...
    await assert.rejects(
      consumeCallbackV07({
        ...
        dispositionId: "disposition-v07",
      }),
      /must be observed/,
    );
    assert.equal((await observeCallbackV07({
      stateRoot: context.stateRoot,
      callbackId: delivered.callback_id,
      recipient: recipient(context),
    })).status, "observed");
    ...
    const status = await callbackStatusV07({
      stateRoot: context.stateRoot,
...
test("v0.7 callback rejects cross-run delivery", async () => {
  const root = await createGitFixture("codex-flow-v07-callback-run-");
  try {
...
test("v0.7 callback rejects contradictory selector evidence before journal persistence", async () => {
  const root = await createGitFixture("codex-flow-v07-callback-selector-");
  try {
    ...
    await assert.rejects(
      deliverCallbackV07({ stateRoot: context.stateRoot, receipt: contradictory }),
      /model evidence/,
    );
    const status = await callbackStatusV07({
      stateRoot: context.stateRoot,
...
test("v0.7 callback rejects a disposition-shaped lookalike with invalid canonical identity", async () => {
  const root = await createGitFixture("codex-flow-v07-callback-lookalike-");
  try {
