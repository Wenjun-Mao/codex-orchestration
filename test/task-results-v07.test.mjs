...
function identity(commonDir = "/tmp/codex-flow-terminal-receipt/.git") {
  const recipient = {
    lineage_id: "lineage-v07",
    thread_id: "coordinator-v07",
    generation: 1,
  };
  return {
    recipient: {
    ...
    executor_thread_id: "executor-v07",
    run_id: "run-v07",
    runtime_context_digest: E,
    configuration_digest: F,
    repository_id: "repository-v07",
    ...
    task_digest: H,
    contract_id: I,
    operation_id: "operation-v07",
    release_id: "release-v07",
    model_evidence: {
...
test("Git outcomes distinguish unchanged, clean local commit, and dirty blocker", async () => {
  const root = await createGitFixture("codex-flow-v07-result-");
  try {
    ...
    assert.throws(() => deriveGitOutcome({
      worktreePath: root,
      baselineRevision: baseline,
      expectedBranch: "codex/expected-executor-v07",
      classification: "PASS",
    }), /wrong Git branch/);

    execFileSync("git", ["switch", "-c", "codex/result-v07"], { cwd: root });
    ...
    const committed = deriveGitOutcome({
      worktreePath: root,
      baselineRevision: baseline,
      expectedBranch: "codex/result-v07",
      classification: "PASS",
    });
...
test("terminal receipt v3 binds full identity and derives callback identity", async () => {
  const root = await createGitFixture("codex-flow-v07-receipt-");
  try {
    const baseline = execFileSync("git", ["rev-parse", "HEAD"], { cwd: root, encoding: "utf8" }).trim();
