...
function legacyPlanWithNamespaces(plan, namespaces) {
  const draft = {
    schema_version: 1,
    kind: "codex-flow-v09-unplug-plan",
    plan_id: "",
  ...
  return validateUnplugPlan(draft);
}
...
test("unplug v2 authenticates mixed namespace directories and opaque root files", async () => {
  ...
  try {
    ...
    const first = await unplugPlan({ repositoryPath: root, resources: [] });
    const second = await unplugPlan({ repositoryPath: root, resources: [] });
    assert.deepEqual(first, second);
    assert.equal(first.schema_version, 2);
    assert.equal(first.kind, "codex-flow-v09-unplug-plan-v2");
    ...
    assert.deepEqual(validateUnplugPlan(first), first);

    const receipt = await unplugApply({ repositoryPath: root, plan: first });
...
test("host-managed Codex turn-diff refs do not invalidate an exact unplug plan", async () => {
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: root, resources: [] });
    ...
    const refreshed = await unplugPlan({ repositoryPath: root, resources: [] });
    assert.equal(refreshed.plan_id, plan.plan_id);
    assert.equal(refreshed.git_digest, plan.git_digest);

    const receipt = await unplugApply({ repositoryPath: root, plan });
    assert.equal(receipt.residue, false);
    assert.equal(await pathExists(stateRoot), false);
  } finally {
    await removeFixture(root);
  ...
  for (const ref of authoritativeRefs) {
    ...
    try {
      ...
      await assert.rejects(
        () => unplugApply({ repositoryPath: root, plan }),
        /plan drifted/,
      );
      assert.equal(await pathExists(stateRoot), true);
      assert.equal(await pathExists(resolve(common, "codex-flow-unplug-v09")), false);
    } finally {
      await removeFixture(root);
    }
  }
...
test("opaque state content and type drift block before deletion", async () => {
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: contentRoot, resources: [] });
    await writeFile(opaque, "two\n");
    await assert.rejects(
      () => unplugApply({ repositoryPath: contentRoot, plan }),
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: typeRoot, resources: [] });
    await rm(opaque);
    await mkdir(opaque);
    await assert.rejects(
      () => unplugApply({ repositoryPath: typeRoot, plan }),
      /plan drifted|state drifted/,
    );
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: directoryTypeRoot, resources: [] });
    await rm(retained, { recursive: true });
    await writeFile(retained, "replacement\n");
    await assert.rejects(
      () => unplugApply({ repositoryPath: directoryTypeRoot, plan }),
      /plan drifted|state drifted/,
    );
...
test("unplug rejects root symlinks, special files, and inventory overflow", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => unplugPlan({ repositoryPath: symlinkRoot, resources: [] }),
      /symbolic link/,
    );
  } finally {
  ...
  try {
    ...
    await assert.rejects(
      () => unplugPlan({ repositoryPath: specialRoot, resources: [] }),
      /special file/,
    );
  } finally {
  ...
  try {
    ...
    await assert.rejects(
      () => unplugPlan({ repositoryPath: overflowRoot, resources: [] }),
  ...
  try {
    ...
    await assert.rejects(
      () => unplugPlan({ repositoryPath: byteRoot, resources: [] }),
      /inventory bound/,
    );
  } finally {
  ...
  try {
    ...
    await assert.rejects(
      () => unplugPlan({ repositoryPath: depthRoot, resources: [] }),
      /depth bound/,
    );
  } finally {
...
test("opaque-file removal resumes safely and v1 directory-only journals remain resumable", async () => {
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: opaqueRoot, resources: [] });
    await assert.rejects(
      () => unplugApply({ repositoryPath: opaqueRoot, plan, testHook: "after-state-entry-1" }),
      /Test interruption/,
    );
    assert.equal(await pathExists(opaque), false);
    assert.equal(await pathExists(stateRoot), true);
    const receipt = await unplugApply({ repositoryPath: opaqueRoot, plan });
    assert.equal(receipt.residue, false);
  } finally {
  ...
  try {
    ...
    const stateRoot = resolve(common, "codex-flow");
    await namespace(common, "v0.7.0");
    const v2 = await unplugPlan({ repositoryPath: legacyRoot, resources: [] });
    const v1 = legacyPlanFromV2(v2);
    await assert.rejects(
      () => unplugApply({ repositoryPath: legacyRoot, plan: v1, testHook: "before-state-removal" }),
      /Test interruption/,
    );
    const receipt = await unplugApply({ repositoryPath: legacyRoot, plan: v1 });
    assert.equal(receipt.residue, false);
    assert.equal(await pathExists(stateRoot), false);
  } finally {
...
test("v1 resume preserves the original per-namespace inventory byte bound", async () => {
  ...
  try {
    ...
    const base = await unplugPlan({ repositoryPath: root, resources: [] });
    ...
    const receipt = await unplugApply({ repositoryPath: root, plan: v1 });
    assert.equal(receipt.residue, false);
    assert.equal(await pathExists(stateRoot), false);
  } finally {
    await removeFixture(root);
...
test("unplug plan is deterministic and exact apply leaves zero repository residue", async () => {
  ...
  try {
    ...
    const first = await unplugPlan({ repositoryPath: fixture.root, resources: resources(fixture) });
    const second = await unplugPlan({ repositoryPath: fixture.root, resources: resources(fixture) });
    assert.deepEqual(first, second);
    assert.deepEqual(validateUnplugPlan(first), first);
    assert.equal(first.mutation_performed, false);
    assert.equal(first.active_runs.length, 0);
    const receipt = await unplugApply({ repositoryPath: fixture.root, plan: first });
    assert.equal(receipt.residue, false);
    assert.equal(await pathExists(resolve(fixture.common, "codex-flow")), false);
    assert.equal(await pathExists(resolve(fixture.common, "codex-flow-unplug-v09")), false);
...
test("unplug accepts ignored artifacts but blocks ordinary untracked files before mutation", async () => {
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: ignored.root, resources: resources(ignored) });
    const receipt = await unplugApply({ repositoryPath: ignored.root, plan });
    assert.equal(receipt.residue, false);
  } finally {
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: dirty.root, resources: resources(dirty) });
    await assert.rejects(() => unplugApply({ repositoryPath: dirty.root, plan }), /worktree-dirty/);
    assert.equal(await pathExists(state), true);
    assert.equal(await pathExists(resolve(dirty.common, "codex-flow-unplug-v09")), false);
...
test("unplug removes a clean detached Codex App worktree already integrated into the base", async () => {
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: fixture.root, resources: planned });
    assert.equal(plan.resources[0].branch, null);
    const receipt = await unplugApply({ repositoryPath: fixture.root, plan });
    assert.equal(receipt.residue, false);
...
test("unplug rejects a detached worktree whose commit is not integrated into the base", async () => {
  ...
  try {
    await namespace(fixture.common);
    await assert.rejects(
      () => unplugPlan({ repositoryPath: fixture.root, resources: resources(fixture) }),
...
test("detached worktree plans fail closed on attachment mismatch and protected paths", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => unplugPlan({ repositoryPath: mismatch.root, resources: incorrect }),
  ...
  try {
    ...
    await assert.rejects(
      () => unplugApply({ repositoryPath: protectedFixture.root, plan }),
      /protected-worktree/,
    );
  } finally {
...
test("active and malformed lifecycle state fail closed", async () => {
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: active.root, resources: resources(active) });
    assert.deepEqual(plan.active_runs, ["still-running"]);
    assert.equal(plan.state_entries.some((entry) => entry.kind === "opaque-file"), true);
    await assert.rejects(() => unplugApply({ repositoryPath: active.root, plan }), /active run/);
    assert.equal(await pathExists(opaque), true);
  } finally {
  ...
  try {
    ...
    await assert.rejects(
      () => unplugPlan({ repositoryPath: malformed.root, resources: resources(malformed) }),
...
test("structured archive evidence must exactly prove every planned task", async () => {
  ...
  try {
    ...
    await assert.rejects(() => unplugApply({ repositoryPath: fixture.root, plan }), /exactly cover/);
    await assert.rejects(() => unplugApply({
      repositoryPath: fixture.root,
      plan,
      archiveEvidence: {
        "executor-task": {
    ...
    const receipt = await unplugApply({
      repositoryPath: fixture.root,
      plan,
      archiveEvidence: {
        "executor-task": {
...
test("private archive evidence authenticates unplug when the public archive index is stale", async () => {
  ...
  try {
    ...
    const observed = await observePrivateUnplug({ plan, codexHome });
    ...
    await assert.rejects(
      unplugApply({ repositoryPath: fixture.root, plan, archiveEvidence: forged }),
...
test("remote refs and unmerged branches block apply", async () => {
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: remote.root, resources: resources(remote) });
    await assert.rejects(() => unplugApply({ repositoryPath: remote.root, plan }), /remote-ref-present/);
  } finally {
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: unmerged.root, resources: resources(unmerged) });
    await assert.rejects(() => unplugApply({ repositoryPath: unmerged.root, plan }), /branch-not-ancestor/);
  } finally {
...
test("a branch attached to an unplanned worktree cannot be removed", async () => {
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: fixture.root, resources: [branchOnly] });
    await assert.rejects(
      () => unplugApply({ repositoryPath: fixture.root, plan }),
...
test("invoking linked worktree and primary checkout are dynamically protected", async () => {
  ...
  try {
    ...
    await assert.rejects(() => unplugApply({ repositoryPath: fixture.worktree, plan }), /protected-worktree/);
  } finally {
    await cleanupFixture(fixture);
  }
});
...
test("action and state-removal interruptions both resume to zero residue", async () => {
  for (const hook of ["after-action-1", "before-state-removal", "after-state-removal"]) {
    ...
    try {
      await namespace(fixture.common);
      const plan = await unplugPlan({ repositoryPath: fixture.root, resources: resources(fixture) });
      await assert.rejects(
        () => unplugApply({ repositoryPath: fixture.root, plan, testHook: hook }),
        /Test interruption/,
      );
      await assert.rejects(
        () => assertNoUnplugInProgress({ gitCommonDirectory: fixture.common }),
        /already in progress/,
      );
      const receipt = await unplugApply({ repositoryPath: fixture.root, plan });
      assert.equal(receipt.residue, false);
...
test("new namespace drift after journal creation blocks state deletion", async () => {
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: fixture.root, resources: resources(fixture) });
    await assert.rejects(
      () => unplugApply({ repositoryPath: fixture.root, plan, testHook: "before-action" }),
      /Test interruption/,
    );
    const added = await namespace(fixture.common, "v0.7.1");
    await assert.rejects(() => unplugApply({ repositoryPath: fixture.root, plan }), /state drifted/);
...
test("new opaque-file drift during state-removal phase blocks without deleting it", async () => {
  ...
  try {
    ...
    const plan = await unplugPlan({ repositoryPath: fixture.root, resources: resources(fixture) });
    await assert.rejects(
      () => unplugApply({ repositoryPath: fixture.root, plan, testHook: "before-state-removal" }),
      /Test interruption/,
    );
    const added = resolve(fixture.common, "codex-flow", "late-evidence.json");
    await writeFile(added, "late\n");
    await assert.rejects(() => unplugApply({ repositoryPath: fixture.root, plan }), /changed during state removal/);
    assert.equal(await pathExists(added), true);
  } finally {
    await cleanupFixture(fixture);
  }
...
test("a malformed or reordered unplug journal never authorizes cleanup", async () => {
  ...
  try {
    ...
    const journalPath = resolve(fixture.common, "codex-flow-unplug-v09", "journal.json");
    const journal = JSON.parse(await readFile(journalPath, "utf8"));
    journal.actions.reverse();
    await writeFile(journalPath, `${JSON.stringify(journal)}\n`);
    await assert.rejects(() => unplugApply({ repositoryPath: fixture.root, plan }), /action identity or state is invalid/);
    assert.equal(await pathExists(retained), true);
