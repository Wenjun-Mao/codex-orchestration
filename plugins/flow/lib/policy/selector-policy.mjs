...
const POLICY_BY_LANE = Object.freeze({
  ...
  bounded_coordination: Object.freeze({
    model: "gpt-5.6-terra",
    reasoning_effort: "high",
...
/** Choose a coordinator selector and staffing shape from explicit workload facts. */
export function selectCoordinatorDelivery({
  settledScope,
  establishedVerification,
  boundedDemandingWork,
  substantialUncertainty,
  systemicDecision,
  difficultIntegrationDecision,
  complexMultiExecutorCoordination,
  childHasConcreteBenefit,
}) {
  const facts = {
    settledScope: requiredBoolean(settledScope, "settledScope"),
    establishedVerification: requiredBoolean(establishedVerification, "establishedVerification"),
    boundedDemandingWork: requiredBoolean(boundedDemandingWork, "boundedDemandingWork"),
    substantialUncertainty: requiredBoolean(substantialUncertainty, "substantialUncertainty"),
    systemicDecision: requiredBoolean(systemicDecision, "systemicDecision"),
    difficultIntegrationDecision: requiredBoolean(
      difficultIntegrationDecision,
      "difficultIntegrationDecision",
    ),
    complexMultiExecutorCoordination: requiredBoolean(
      complexMultiExecutorCoordination,
      "complexMultiExecutorCoordination",
    ),
    childHasConcreteBenefit: requiredBoolean(childHasConcreteBenefit, "childHasConcreteBenefit"),
  };
  const governance = !facts.settledScope
    || facts.substantialUncertainty
    || facts.systemicDecision
    || facts.difficultIntegrationDecision
    || facts.complexMultiExecutorCoordination;
  const demanding = facts.boundedDemandingWork || !facts.establishedVerification;
  const selector = selectSelectorPolicy(
    governance ? "governance" : demanding ? "integration" : "bounded_coordination",
  );
  return Object.freeze({
    delivery_shape: facts.childHasConcreteBenefit ? "bounded-child-recommended" : "coordinator-direct",
    staffing_rationale: facts.childHasConcreteBenefit
      ? "A separate bounded lane has a concrete independent benefit."
      : "No separate bounded lane has enough benefit to justify coordination cost.",
    selector,
    coordinator_retains: Object.freeze([
