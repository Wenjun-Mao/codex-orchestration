...
async function assertCoordinatorRouteAuthority(stateRoot, route, { allowRegistering = false } = {}) {
  const assignmentAuthority = await assignmentAuthorityForRoute({
    stateRoot,
    routeId: route.route_id,
  });
  if (assignmentAuthority !== null) {
    await assertAssignmentRouteAuthority({ stateRoot, route, allowRegistering });
    return;
  }
...
async function assertRouteAuthority(
  stateRoot,
  route,
  { requireActive = false, allowRegistering = false } = {},
) {
  if (route.assignment.kind === "coordinator-delegation") {
    await assertCoordinatorRouteAuthority(stateRoot, route, { allowRegistering });
  } else {
...
export async function registerReportRoute({
  ...
  const observedCreationHost = launch.creation_evidence?.classification === "opaque"
    ? null
    : launch.creation_evidence?.host_id ?? null;
  if (observedCreationHost !== null && observedCreationHost !== senderHost) {
...
export async function registerCoordinatorReportRoute({
  ...
  await markAssignmentRegistrationStage({
    stateRoot: assignmentAuthority.state_root,
    assignmentId,
    stage: "iteration",
    now,
  });
  ...
  await markAssignmentRegistrationStage({
    stateRoot: assignmentAuthority.state_root,
    assignmentId,
    stage: "recipient_binding",
    now,
  });
  await assertRouteAuthority(reportingStateRoot, route, {
    requireActive: true,
    allowRegistering: true,
  });
  const registration = await withProcessLock({
  ...
  const assignment = await markAssignmentRegistrationStage({
    stateRoot: assignmentAuthority.state_root,
    assignmentId,
    stage: "route",
    now,
...
export async function reportRoute({ stateRoot, routeId: id, allowMissing = false }) {
  return readRoute(stateRoot, id, { allowMissing });
}
...
export async function assertActiveReportRoute({ stateRoot, routeId: id, allowRegistering = false }) {
  return withRouteLock(stateRoot, id, async (route) => {
    await assertRouteAuthority(stateRoot, route, { requireActive: true, allowRegistering });
    return route;
  });
}
