...
export function validateTaskLaunchRecord(value) {
  ...
  ) throw new CliError("Task launch selector authority does not match its generated contract");
...
function initialRecord(contract, requested, taskTitle, launchId, launchNonce, runtimeCliPath, preparedAt) {
  const record = {
    schema_version: TASK_LAUNCH_SCHEMA_VERSION,
    kind: TASK_LAUNCH_KIND,
    launch_id: launchId,
...
export async function prepareTaskLaunch({
  stateRoot,
  taskContract,
  requestedSelectors,
  taskTitle = null,
  ...
  const requestedTaskTitle = taskTitle === null
    ? contract.task.title
    : requireText(taskTitle, "taskTitle", { max: 160 });
...
function normalizeSelectorUpdate(record, value, { requireAccepted }) {
  if (value === null) {
    if (requireAccepted && record.selector_evidence.accepted === null) {
      throw new CliError("Created task evidence requires accepted selectors");
    }
  ...
  const mergeObservation = (current, incoming, field) => {
    if (current === null) return incoming;
    if (incoming === null) return current;
    for (const selector of ["project_id", "model", "reasoning_effort"]) {
      if (
        current[selector] !== null
        && incoming[selector] !== null
        && current[selector] !== incoming[selector]
    ...
    return {
      ...
      // A replay confirms the original observation; it does not create a new
      // selector event or replace its timestamp.
      observed_at: current.observed_at,
    };
  };
  return {
    requested: clone(record.selector_evidence.requested),
    accepted: mergeObservation(record.selector_evidence.accepted, next.accepted, "accepted"),
    observed: mergeObservation(record.selector_evidence.observed, next.observed, "observed"),
  };
}

function sameCreationResult(left, right) {
  return left.schema_version === right.schema_version
    && left.kind === right.kind
    && left.classification === right.classification
    && left.host_id === right.host_id
    && left.ready_thread_id === right.ready_thread_id
    && left.provisional_id === right.provisional_id
    && left.opaque_digest === right.opaque_digest
    && left.opaque_length === right.opaque_length;
}
  ...
  return withProcessLock({
    ...
    if (CREATION_CLASSIFICATIONS.includes(result)) {
      ...
      if (record.creation_evidence !== null) {
        if (!sameCreationResult(record.creation_evidence, evidence)) {
          throw new CliError("Task launch creation reconciliation conflicts with recorded evidence", 73);
        }
        if (stableStringify(record.selector_evidence) !== stableStringify(selectors)) {
          record = await writeRecord(stateRoot, {
            ...record,
...
export async function startTaskLaunch({
  ...
  return withProcessLock({
    ...
    if (record.git_activation?.state === "completed") {
      ...
      const [worktreePath, commonDir] = await Promise.all([
        realpath(snapshot.root),
        realpath(snapshot.commonDir),
      ]);
      if (
        worktreePath !== record.git_activation.worktree_path
        || commonDir !== record.git_activation.common_dir
        || snapshot.branch !== record.git_activation.executor_branch
      ) {
        throw new CliError("Task launch completed activation belongs to a different worktree", 73);
      }
      // Completed activation is durable identity authority. Replaying this
      // bookkeeping edge must remain possible after the launch window closes or
      // the executor advances its branch, and must never rerun Git activation.
...
export function taskLaunchIdForContract({ taskContract, requestedSelectors, taskTitle = null }) {
  const contract = validateGeneratedTaskContract(taskContract);
  const requested = validateRequestedSelectors(requestedSelectors, contract.current_baseline.revision);
  const requestedTaskTitle = taskTitle === null
    ? contract.task.title
