...
import { validateRunClosureAudit } from "../run-audit.mjs";
...
function refreshBundlePath(value, label) {
  ...
  if (
    ...
    || (path !== "package.json" && !REFRESH_BUNDLE_PATH_PREFIXES.some((prefix) => path.startsWith(prefix)))
  ) throw new CliError(`${label} must be a safe runtime path: ${path}`, 73);
  return path;
}
...
function runGit(cwd, args, label, { allow = [] } = {}) {
  ...
  if (result.status !== 0 && !allow.includes(result.status)) {
    ...
    throw new CliError(detail === "" ? `${label} failed` : `${label}: ${detail}`, 73);
  }
  return result;
}

function exactSettledBinding(run, binding, { namespace }) {
  return (
...
async function canonicalRepositoryPath(path) {
...
function worktreePaths(commonDir) {
  const result = runGit(
    commonDir,
    ["worktree", "list", "--porcelain"],
    "Completed coordinator worktree inventory",
  );
  return result.stdout.split("\n\n").filter(Boolean).map((block) => {
    const worktree = block.split("\n").find((line) => line.startsWith("worktree "));
    if (worktree === undefined) throw new CliError("Completed coordinator worktree inventory is malformed", 73);
    return resolve(worktree.slice("worktree ".length));
  });
}

function preservedNonHostRef(commonDir, tip) {
  const result = runGit(
    commonDir,
    ["for-each-ref", "--format=%(refname)%00%(objecttype)%00%(objectname)%00%(*objectname)"],
    "Completed coordinator preservation inspection",
  );
  return result.stdout.split("\n").filter(Boolean).some((line) => {
    const [ref, objectType, objectName, peeledObjectName] = line.split("\0");
    return (
      !ref.startsWith("refs/codex/")
      && [objectName, peeledObjectName].includes(tip)
      && ["commit", "tag"].includes(objectType)
    );
  });
}

function settledCoordinatorSummary({ namespace, run, assignment, iteration, report }) {
  ...
  return {
    ...
    preserved_tip: coordinator.archive_attempt.branch_tip,
  };
}

function settledCoordinatorBlocker(error) {
  return {
    status: "blocked",
    reason: `Completed v0.9.7 coordinator evidence is incomplete: ${error.message}. Do not recreate, remove, or rebind the retired coordinator; preserve its evidence and ask the director to resolve the predecessor.`,
  };
}

/**
 * Reconciles precisely one v0.9.7 predecessor whose coordinator has already
 * completed closeout. This is intentionally narrower than general refresh:
 * a missing checkout is acceptable only when every persisted assignment,
 * report, iteration, archive, and Git-preservation proof agrees.
 */
export async function reconcileSettledV097CoordinatorPredecessor({
  commonDir,
  candidate,
}) {
  const common = resolve(commonDir);
  if (candidate.namespace !== "v0.9.7") return { status: "not-applicable" };

  let lifecycle;
  try {
    lifecycle = validateRunLifecycleState(candidate.lifecycle);
  } catch {
    return { status: "not-applicable" };
  }
  const runs = Object.values(lifecycle.runs);
  if (
    lifecycle.active_run_id !== null
    || runs.length !== 1
    || runs[0].status !== "closed"
  ) return { status: "not-applicable" };

  const run = runs[0];
  let context;
  try {
    const root = sourceNamespaceRoot(common, candidate.namespace);
    context = validateRuntimeContext(await readJson(
      resolve(root, "contexts", `${run.runtime_id}.json`),
      { guardRoot: common },
    ));
    if (
      context.runtime_id !== run.runtime_id
      || runtimeContextHash(context) !== run.runtime_context_hash
      || resolve(context.repository.common_dir) !== common
      || context.bundle.package_version !== "0.9.7"
    ) throw new CliError("completed coordinator runtime context does not match its run", 73);
  } catch (error) {
    return settledCoordinatorBlocker(error);
  }
  const sourceRoot = await canonicalRepositoryPath(context.repository.root);
  const sourceInfo = await lstat(sourceRoot).catch((error) => {
    if (error?.code === "ENOENT") return null;
    throw error;
  });
  if (sourceInfo !== null) return { status: "not-applicable" };

  try {
    const runtimeBinding = runtimeBindingFromContext(context);
    if (
      ...
      || stableStringify(runtimeBinding.host) !== stableStringify(run.binding.host)
      || stableStringify(runtimeBinding.lineage) !== stableStringify(run.binding.lineage)
    ) throw new CliError("completed coordinator runtime binding does not match its run", 73);

    const reportingRoot = assignmentStateRoot(common);
    const assignments = await boundedJsonRecords(
      resolve(reportingRoot, "records"),
      validateAssignmentAuthority,
      common,
    );
    const matches = [];
    for (const assignment of assignments) {
      const initial = assignment.execution_bindings[0];
      if (
        ...
        && exactSettledBinding(run, initial, {
          namespace: candidate.namespace,
        })
        && initial.repository_root !== null
        && await canonicalRepositoryPath(initial.repository_root) === sourceRoot
      ) matches.push(assignment);
    }
    if (matches.length !== 1) {
      throw new CliError("exactly one retired coordinator assignment must bind the absent source checkout", 73);
    }
    const assignment = matches[0];
    if (sha256(await readFile(assignment.approved_plan.snapshot_path)) !== assignment.approved_plan.digest) {
      throw new CliError("retired coordinator approved plan bytes do not match their preserved digest", 73);
    }

    const route = await reportRoute({ stateRoot: reportingRoot, routeId: assignment.route_id });
    if (
      route.state !== "closed"
      || route.lifecycle.closure_reason !== "archived"
      || route.assignment.assignment_id !== assignment.assignment_id
      || route.assignment.common_dir !== assignment.common_dir
      || route.assignment.run_id !== run.run_id
      || route.assignment.runtime_context_digest !== run.runtime_context_hash
      || route.assignment.configuration_digest !== run.binding.config_hash
      || route.assignment.repository_digest !== run.binding.repository_hash
      || route.assignment.plan_id !== run.workflow_plan_id
      || route.assignment.revision_digest !== run.workflow_revision_digest
      || route.assignment.approved_plan_digest !== assignment.approved_plan.digest
      || route.sender.host_id !== assignment.sender.host_id
      || route.sender.thread_id !== assignment.sender.thread_id
      || stableStringify(route.recipient) !== stableStringify(assignment.recipient)
    ) throw new CliError("retired coordinator report route does not match its exact assignment", 73);

    let report;
    try {
      report = await reportDelivery({ stateRoot: reportingRoot, reportId: assignment.acceptance.report_id });
    } catch {
      throw new CliError("retired coordinator accepted report record is unavailable", 73);
    }
    if (
    ...
    ) throw new CliError("retired coordinator accepted report does not match its assignment", 73);

    const iteration = await iterationStatus({ commonDir: common, iterationId: assignment.iteration_id });
    const coordinators = iteration.members.filter((member) => member.role === "coordinator");
    if (
      iteration.assignment_id !== assignment.assignment_id
      || iteration.state !== "closed"
      || coordinators.length !== 1
    ) {
      throw new CliError("retired coordinator iteration is not exactly closed", 73);
    }
    const coordinator = coordinators[0];
    const initial = assignment.execution_bindings[0];
    if (
      coordinator.state !== "archived"
      || coordinator.retained !== false
      || coordinator.host_id !== assignment.sender.host_id
      || coordinator.thread_id !== assignment.sender.thread_id
      || coordinator.reporting_parent_thread_id !== assignment.recipient.thread_id
      || coordinator.worktree_path !== initial.repository_root
      || coordinator.branch !== initial.repository_branch
      || coordinator.authority.kind !== "assignment"
      || coordinator.authority.authority_id !== assignment.assignment_id
      || coordinator.authority.state_root !== reportingRoot
      || coordinator.archive_attempt?.state !== "accepted"
      || coordinator.archive_attempt.branch_tip === null
    ) throw new CliError("retired coordinator archive authority does not match its assignment", 73);
    if (worktreePaths(common).includes(sourceRoot)) {
      throw new CliError("retired coordinator worktree remains registered", 73);
    }
    if (initial.repository_branch === null || !initial.repository_branch.startsWith("codex/")) {
      throw new CliError("retired coordinator branch is not an exact disposable Codex branch", 73);
    }
    if (runGit(
      common,
      ["show-ref", "--verify", "--quiet", `refs/heads/${initial.repository_branch}`],
      "Retired coordinator branch inspection",
      { allow: [1] },
    ).status !== 1) throw new CliError("retired coordinator branch remains present", 73);
    if (!preservedNonHostRef(common, coordinator.archive_attempt.branch_tip)) {
      throw new CliError("retired coordinator tip lacks a preserved non-host Git ref", 73);
    }
    return { status: "reconciled", summary: settledCoordinatorSummary({
      namespace: candidate.namespace,
      run,
      assignment,
      iteration,
...
function terminalFencesRemain(run) {
  const fences = run.terminal?.unresolved_fences;
  return fences === undefined || (
    fences.path_fences.length !== 0
    || fences.resource_fences.length !== 0
    || fences.branch_fences.length !== 0
  );
}

function closedSettlementAuthorityMatches({ audit, commonDir, journal, namespace, run, runtime }) {
  const root = sourceNamespaceRoot(commonDir, namespace);
  const expectedBranch = runtime.context.repository.branch === "detached"
    ? null
    : runtime.context.repository.branch;
  const authority = audit.authority;
  const activationSources = audit.source_records.filter((entry) => (
    entry.record_kind === "run-activation"
    && entry.record_id === run.run_id
    && entry.source_locator === `runs/lifecycle.json#${run.run_id}`
  ));
  const authorityMatches = (
    authority.run_id === run.run_id
    && authority.common_dir === resolve(commonDir)
    && authority.state_root === root
    && authority.runtime_id === run.runtime_id
    && authority.runtime_context_digest === run.runtime_context_hash
    && authority.bundle_digest === run.binding.bundle_hash
    && authority.configuration_digest === run.binding.config_hash
    && authority.policy_digest === run.binding.policy_hash
    && authority.repository_id === run.binding.repository_hash
    && authority.repository_digest === run.binding.repository_hash
    && authority.workflow_plan_id === run.workflow_plan_id
    && authority.activated_revision_digest === run.workflow_revision_digest
    && authority.current_revision_digest === journal.current_revision_digest
    && authority.run_binding_generation === run.binding.generation
    && authority.run_binding_digest === sha256(stableStringify(run.binding))
    && activationSources.length === 1
    && activationSources[0].record_digest === authority.run_activation_digest
    && audit.repository.root === runtime.context.repository.root
    && audit.repository.expected_root === runtime.context.repository.root
    && audit.repository.common_dir === resolve(commonDir)
    && audit.repository.expected_common_dir === resolve(commonDir)
    && audit.repository.branch === expectedBranch
    && audit.repository.expected_branch === expectedBranch
    && Date.parse(audit.audited_at) <= Date.parse(run.terminal.closed_at)
  );
  return authorityMatches;
}

function settlementAuditReady(audit, journal) {
  return (
    audit.terminal_ready === true
    && audit.blockers.length === 0
    && audit.cleanup.counts.cleanup_candidates === 0
    && audit.cleanup.counts.cleanup_required === 0
    && audit.cleanup.counts.close_blocked === 0
    && audit.authority.workflow_journal_digest === journal.journal_digest
    && audit.authority.current_revision_digest === journal.current_revision_digest
  );
}

async function assertReclaimedClosedRunSettled({ commonDir, namespace, run }) {
  const common = resolve(commonDir);
  const runtime = await loadRuntimeAuthority({ commonDir: common, namespace, run });
  const root = sourceNamespaceRoot(common, namespace);
  const workflowRoot = resolve(root, "workflows", run.run_id, run.workflow_plan_id);
  const journal = validateWorkflowJournal(await readJson(
    resolve(workflowRoot, "journal.json"),
    { guardRoot: common },
  ));
  if (
    journal.run_id !== run.run_id
    || journal.plan_id !== run.workflow_plan_id
    || !journal.revisions.some((entry) => entry.revision_digest === run.workflow_revision_digest)
  ) throw new CliError(`Earlier closed source run has inconsistent workflow authority: ${run.run_id}`, 73);

  const audits = (await boundedJsonRecords(
    resolve(root, "run-closure-audits", "records"),
    validateRunClosureAudit,
    common,
  )).filter((audit) => audit.authority.run_id === run.run_id);
  const settlements = audits.filter((audit) => (
    settlementAuditReady(audit, journal)
    && closedSettlementAuthorityMatches({ audit, commonDir: common, journal, namespace, run, runtime })
  ));
  if (settlements.length === 0) {
    throw new CliError(`Earlier closed source run lacks matching settlement evidence: ${run.run_id}`, 73);
  }
  if (settlements.length > 1) {
    throw new CliError(`Earlier closed source run has ambiguous settlement evidence: ${run.run_id}`, 73);
  }

  // A closure audit captures the active lifecycle entry. Closing then changes
  // only the terminal lifecycle fields, so validate the immutable authority
  // and the authorized audit-before-close ordering instead of rehashing it.
  // Earlier blocked audits remain historical evidence, not a competing settlement.
  return { audit: settlements[0], journal, runtime };
}

function modernHistoricalSource(packageVersion) {
  const match = /^0\.9\.(\d+)(?:-(?:dev|rc)\.[0-9]+)?$/.exec(packageVersion);
  return match !== null && Number(match[1]) >= 8;
}

async function settledAssignmentForRun({ commonDir, namespace, run, runtime, execution }) {
  const stateRoot = assignmentStateRoot(commonDir);
  const assignments = await boundedJsonRecords(
    resolve(stateRoot, "records"),
    validateAssignmentAuthority,
    commonDir,
  );
  const matches = [];
  for (const assignment of assignments) {
    const binding = assignment.execution_bindings.find((entry) => (
      exactSettledBinding(run, entry, { namespace })
    ));
    if (
      assignment.state === "retired"
      && assignment.common_dir === resolve(commonDir)
      && assignment.sender.host_id === run.binding.host.host_id
      && assignment.sender.thread_id === run.binding.lineage.thread_id
      && binding !== undefined
    ) matches.push({ assignment, binding });
  }
  if (matches.length !== 1) {
    throw new CliError(
      `Historical run requires exactly one retired assignment authority: ${namespace}/${run.run_id}`,
      73,
    );
  }
  const { assignment, binding } = matches[0];
  if (assignmentRegistration(assignment).status !== "ready") {
    throw new CliError("Historical assignment registration is incomplete", 73);
  }
  ...
  const retirementEvidenceDigest = sha256(stableStringify({
    namespace,
    run_id: run.run_id,
  ...
  if (
    ...
    || retirement.terminal_status !== "closed"
    ...
    || retirement.evidence_digest !== retirementEvidenceDigest
    || retirement.resource_disposition !== "released"
    || retirement.owner_thread_id !== assignment.sender.thread_id
    || retirement.next_action !== null
  ) throw new CliError("Historical execution retirement does not match the closed run", 73);

  const route = await reportRoute({ stateRoot, routeId: assignment.route_id });
  const routedAssignment = await assertAssignmentRouteAuthority({
    stateRoot,
    route,
    allowRetired: true,
  });
  if (
    routedAssignment.assignment_id !== assignment.assignment_id
    || route.state !== "closed"
    || route.lifecycle.closure_reason !== "archived"
  ) throw new CliError("Historical report route is not exactly closed", 73);
  ...
  ) throw new CliError("Historical accepted report does not match its assignment", 73);

  const settledIteration = await assertArchivedCoordinatorIterationSettled({
    commonDir,
    iterationId: assignment.iteration_id,
    assignmentId: assignment.assignment_id,
    stateRoot,
    hostId: assignment.sender.host_id,
    threadId: assignment.sender.thread_id,
    reportingParentThreadId: assignment.recipient.thread_id,
    requiredResultTip: execution.audit.repository.expected_head_revision,
  });
  const locatorRetirement = await repositoryReportLocatorRetirement({
    stateRoot,
    routeId: route.route_id,
    successorRouteId: null,
  });
  if (
    locatorRetirement.reason !== "archived"
    || locatorRetirement.recovery !== null
    || !locatorRetirement.accepted_report_ids.includes(report.report_id)
  ) throw new CliError("Historical report locator retirement does not include its accepted result", 73);
  return {
    schema_version: 1,
    kind: "codex-flow-v0912-settled-predecessor",
    namespace,
    run_id: run.run_id,
    runtime_id: run.runtime_id,
    runtime_context_digest: run.runtime_context_hash,
    workflow_plan_id: run.workflow_plan_id,
    workflow_revision_digest: run.workflow_revision_digest,
    closure_audit_id: execution.audit.audit_id,
    closure_audit_digest: execution.audit.record_digest,
    assignment_id: assignment.assignment_id,
    execution_binding_index: assignment.execution_bindings.indexOf(binding),
    report_id: report.report_id,
    route_id: route.route_id,
    iteration_id: settledIteration.iteration.iteration_id,
    locator_retirement_id: locatorRetirement.retirement_id,
    preserved_tip: settledIteration.preserved_tip,
  };
}

async function settledExecutionInOpenAssignment({ commonDir, namespace, run, source }) {
  const stateRoot = assignmentStateRoot(commonDir);
  const assignments = await boundedJsonRecords(
    resolve(stateRoot, "records"),
    validateAssignmentAuthority,
  ...
  const matches = assignments.filter((assignment) => (
    ...
    && assignment.common_dir === resolve(commonDir)
    && assignment.sender.host_id === source.run.binding.host.host_id
    && assignment.sender.thread_id === source.run.binding.lineage.thread_id
    && exactSettledBinding(source.run, assignment.execution_bindings.at(-1), {
      namespace: source.namespace,
    })
    && assignment.execution_bindings.some((entry) => exactSettledBinding(run, entry, { namespace }))
  ));
  if (matches.length === 0) return null;
  if (matches.length > 1) {
    throw new CliError("Earlier execution matches multiple open assignment authorities", 73);
  }
  const assignment = matches[0];
  if (assignmentRegistration(assignment).status !== "ready") {
    throw new CliError("Open assignment registration is incomplete", 73);
  }
  const retirement = (assignment.execution_retirements ?? []).find((entry) => (
    entry.namespace === namespace && entry.run_id === run.run_id
  ));
  ...
  if (
    ...
    || retirement.owner_thread_id !== assignment.sender.thread_id
    || retirement.next_action !== null
  ) throw new CliError("Earlier open-assignment execution retirement is incomplete", 73);
  return { assignment_id: assignment.assignment_id, run_id: run.run_id };
}

function historicalSettlementBlocker(namespace, error) {
  return {
    status: "blocked",
    reason: `Historical predecessor evidence is incomplete for ${namespace}: ${error.message}`,
  };
}

/**
 * Classify a modern v0.9 namespace before refresh chooses any executable
 * source. Missing checkouts are never reconstructed: a closed run is historical
 * only when every owning durable record and current Git preservation fact agrees.
 */
export async function classifyV09HistoricalPredecessor({ commonDir, candidate }) {
  const common = resolve(commonDir);
  let lifecycle;
  try {
    lifecycle = validateRunLifecycleState(candidate.lifecycle);
  ...
  if (candidate.namespace === "v0.9.7") return { status: "not-applicable" };
  const runs = Object.values(lifecycle.runs);
  if (runs.length === 0) return { status: "not-applicable" };
  ...
  try {
    for (const run of runs) {
      ...
      if (!modernHistoricalSource(runtime.manifest.package_version)) {
        return { status: "not-applicable" };
      }
      const checkoutExists = await sourceCheckoutExists(runtime);
      if (run.status === "active" && !checkoutExists) {
        throw new CliError(`Active source run requires a live checkout: ${run.run_id}`, 73);
      }
      entries.push({ run, runtime, checkoutExists });
    }
    const live = entries.filter((entry) => (
      entry.run.status === "active" || entry.checkoutExists
    )).map((entry) => entry.run);
    const selected = live.length === 0 ? null : lifecycle.active_run_id === null
      ? [...live].sort((left, right) => Date.parse(right.updated_at) - Date.parse(left.updated_at))[0]
      : live.find((run) => run.run_id === lifecycle.active_run_id);
    if (live.length > 0 && selected === undefined) {
      throw new CliError("Active lifecycle does not resolve to its live source", 73);
    }
    for (const { run, runtime, checkoutExists } of entries) {
      if (run.status === "active" || checkoutExists) continue;
      if (terminalFencesRemain(run)) {
        const settlement = await assessRetainedRunSettlement({
          commonDir: common,
          namespace: candidate.namespace,
          run,
        });
        if (settlement.status !== "settled" || settlement.stage !== "retired") {
          throw new CliError(`Earlier source run retains unresolved fences: ${run.run_id}`, 73);
        }
        const stateRoot = assignmentStateRoot(common);
        const assignment = (await boundedJsonRecords(
          resolve(stateRoot, "records"),
          validateAssignmentAuthority,
          common,
        )).find((entry) => entry.assignment_id === settlement.assignment_id);
        if (assignment === undefined) throw new CliError("Retained settlement assignment authority is absent", 73);
        const binding = assignment.execution_bindings.find((entry) => (
          exactSettledBinding(run, entry, { namespace: candidate.namespace })
        ));
        if (binding === undefined) throw new CliError("Retained settlement execution binding drifted", 73);
        settled.push({
          schema_version: 1,
          kind: "codex-flow-v0913-retained-settled-predecessor",
          namespace: candidate.namespace,
          run_id: run.run_id,
          runtime_id: run.runtime_id,
          runtime_context_digest: run.runtime_context_hash,
          ...
          iteration_id: settlement.iteration_id,
          locator_retirement_id: settlement.locator_retirement_id,
          preserved_tip: settlement.preserved_tip,
        });
        continue;
      }
      if (run.status !== "closed") {
        throw new CliError(`Earlier abandoned source run requires a live checkout: ${run.run_id}`, 73);
      }
      const execution = await assertReclaimedClosedRunSettled({
        commonDir: common,
        namespace: candidate.namespace,
        run,
      });
      const openAssignmentExecution = selected === null ? null : await settledExecutionInOpenAssignment({
        commonDir: common,
        namespace: candidate.namespace,
        run,
        source: { namespace: candidate.namespace, run: selected },
      });
      if (openAssignmentExecution !== null) continue;
      settled.push(await settledAssignmentForRun({
        commonDir: common,
        namespace: candidate.namespace,
        run,
        runtime,
    ...
    if (live.length === 0) return { status: "settled-history", summaries: settled };
    return {
      status: "live-source-required",
      selected_run_id: selected.run_id,
      summaries: settled,
    };
  } catch (error) {
    return historicalSettlementBlocker(candidate.namespace, error);
  }
}

async function sourceCheckoutExists(runtime) {
  const path = await canonicalRepositoryPath(runtime.context.repository.root);
  const info = await lstat(path).catch((error) => {
    if (error?.code === "ENOENT") return null;
    throw error;
  });
  if (info !== null && !info.isDirectory()) {
    throw new CliError("Refresh source runtime repository root is not a directory", 73);
  }
  return info !== null;
}
...
export async function assertOtherRefreshSourceRunsSafe(source) {
  for (const run of Object.values(source.lifecycle.runs)) {
    ...
    if (terminalFencesRemain(run)) {
      if (source.runtime.manifest.kind === "codex-flow-v09-runtime-bundle") {
        const settlement = await assessRetainedRunSettlement({
          commonDir: source.baseline.common_dir,
          namespace: source.namespace,
          run,
        });
        if (settlement.status === "settled" && settlement.stage === "retired") continue;
      }
      throw new CliError(`Earlier source run retains unresolved fences: ${run.run_id}`, 73);
    }
    if (source.runtime.manifest.kind !== "codex-flow-v09-runtime-bundle") {
      // Older source runtimes do not persist the v0.9 closure-audit contract.
      // Keep their established live-source cleanup path rather than treating
      // an incompatible record shape as settled historical evidence.
    ...
    const checkoutExists = await sourceCheckoutExists(runtime);
    if (run.status === "abandoned" && !checkoutExists) {
      throw new CliError(`Earlier abandoned source run requires a live checkout: ${run.run_id}`, 73);
    }
    if (run.status === "closed" && !checkoutExists) {
      const execution = await assertReclaimedClosedRunSettled({
        commonDir: source.baseline.common_dir,
        namespace: source.namespace,
        run,
      });
      const openAssignmentExecution = await settledExecutionInOpenAssignment({
        commonDir: source.baseline.common_dir,
        namespace: source.namespace,
        run,
        source,
...
export async function refreshNamespaceCandidates({ commonDir, currentNamespace }) {
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    if (
      entry.name === currentNamespace
      || entry.name === "foreign-active-run.lock"
      || isSharedFlowStateDirectory(entry.name)
    ) continue;
