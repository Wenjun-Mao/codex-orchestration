...
const TYPED_HOST_EVIDENCE_SOURCE = "typed-host-evidence-v1";
const TYPED_HOST_ACTIVITY_SOURCE = "typed-host-activity-v1";
...
function validateDispositionClaim(value) {
  ...
  if (value.schema_version !== 1 || value.kind !== "codex-flow-v09-archive-disposition-claim") {
    throw new CliError("Invalid archive disposition claim");
  }
  return {
    schema_version: 1,
    kind: "codex-flow-v09-archive-disposition-claim",
...
function validateTaskObservation(value, label) {
  ...
  requireExactFields(value, {
    required: source === TYPED_HOST_EVIDENCE_SOURCE
      ? [
        "execution_kind", "thread_id", "source", "active_visible", "archived_visible",
        "host_evidence_digest", "observed_at",
      ]
      : source === TYPED_HOST_ACTIVITY_SOURCE
        ? [
          "execution_kind", "thread_id", "source", "active_visible", "archived_visible",
          "activity_state", "observed_at",
        ]
      : [
        "execution_kind", "thread_id", "source", "active_visible", "archived_visible",
      ],
    optional: source === "host-observed" ? ["observed_at"] : [],
  }, label);
  const result = {
    ...
    source: requireEnum(
      value.source,
      ["host-observed", TYPED_HOST_EVIDENCE_SOURCE, TYPED_HOST_ACTIVITY_SOURCE],
      `${label}.source`,
    ),
  ...
  if (result.source === TYPED_HOST_EVIDENCE_SOURCE) {
    if (result.active_visible !== false || result.archived_visible !== true) {
      throw new CliError(`${label} typed host evidence does not prove archived visibility`);
    }
    result.host_evidence_digest = digest(
      value.host_evidence_digest,
      `${label}.host_evidence_digest`,
    );
    result.observed_at = timestamp(value.observed_at, `${label}.observed_at`);
  }
  if (result.source === TYPED_HOST_ACTIVITY_SOURCE) {
    if (result.active_visible !== true || result.archived_visible !== false) {
      throw new CliError(`${label} typed activity evidence must identify the active surface`);
    }
    result.activity_state = requireEnum(
  ...
  if (result.source === "host-observed" && value.observed_at !== undefined) {
    result.observed_at = timestamp(value.observed_at, `${label}.observed_at`);
  }
  return result;
}
...
function validateArchiveObservation(value) {
  ...
  const observation = {
    ...
    worktree_state: requireEnum(
      ...
      ["not-applicable", "present", "absent"],
      "archive observation.worktree_state",
    ),
    observed_at: timestamp(value.observed_at, "archive observation.observed_at"),
  };
  if (
    observation.task.source === TYPED_HOST_EVIDENCE_SOURCE
    && observation.task.observed_at !== observation.observed_at
  ) throw new CliError("Archive host-evidence timestamp does not match its typed provenance");
  return observation;
}
...
export function validateArchiveOperation(value) {
  requireExactFields(value, {
    required: [
      ...
      "launch_id", "executor_thread_id", "callback_id", "disposition_id",
  ...
  if (value.schema_version !== 1 || value.kind !== ARCHIVE_KIND) {
    throw new CliError("Invalid v0.9 task archive operation schema");
  }
  const record = {
    schema_version: 1,
    kind: ARCHIVE_KIND,
  ...
  const taskWasActive = record.task.active_visible === true
    && record.task.archived_visible === false;
  const taskWasAlreadyArchived = record.task.active_visible === false
    && record.task.archived_visible === true;
  ...
  if (record.observation) {
    const observedAt = Date.parse(record.observation.observed_at);
    const preparedAt = Date.parse(record.prepared_at);
    const preexistingArchiveObservation = taskWasAlreadyArchived
      && observedAt >= preparedAt - 30_000;
    if (
      observedAt > Date.parse(record.updated_at)
      || (observedAt < preparedAt && !preexistingArchiveObservation)
  ...
  if (!taskWasActive && !taskWasAlreadyArchived) {
    throw new CliError("Archive preparation requires one exact task visibility surface");
  }
  if (taskWasAlreadyArchived && ![
    "archived-awaiting-worktree-reclamation",
    "completed",
  ].includes(record.state)) {
    throw new CliError("Already archived preparation must begin from its observed postcondition");
  }
  ...
  const observedArchiveStates = [
    "archived-awaiting-worktree-reclamation",
    "completed",
  ];
  if ((record.observation !== null) !== observedArchiveStates.includes(record.state)) {
    throw new CliError("Archive observation is inconsistent with state");
  }
  if (observedArchiveStates.includes(record.state)) {
    if (!["accepted", "ambiguous"].includes(record.setter?.outcome)) {
      throw new CliError("Observed archive requires accepted delivery or an exact postcondition after ambiguity");
    }
    if (
    ...
    ) throw new CliError("Observed archive requires independent archived visibility");
  }
  if (record.state === "archived-awaiting-worktree-reclamation") {
    if (
      record.worktree.management !== "host-managed"
      || record.observation.worktree_state !== "present"
    ) throw new CliError("Pending archive reclamation requires the exact host-managed worktree to remain present");
  }
  ...
  if (
    ...
    && !["ambiguous", "archived-awaiting-worktree-reclamation", "completed"].includes(record.state)
...
function archiveWorktreeAuthority(launch) {
  if (launch.git_activation?.state !== "completed") {
    throw new CliError("Task archive requires completed launch worktree activation", 73);
  }
  return { management: "host-managed", path: launch.git_activation.worktree_path };
}

function noChangePreservation(launch, receipt) {
  const requestedWorktree = launch.selector_evidence?.requested?.worktree;
  const gitOutcome = receipt.git_outcome;
  if (
    requestedWorktree === undefined
    || gitOutcome.kind !== "unchanged"
    || launch.git_activation.baseline_revision !== requestedWorktree.starting_revision
    || launch.task_contract.current_baseline.revision !== requestedWorktree.starting_revision
    || gitOutcome.baseline_revision !== requestedWorktree.starting_revision
    || gitOutcome.final_revision !== requestedWorktree.starting_revision
  ) {
    throw new CliError("Accepted no-change archive lacks one exact launch-source preservation authority", 73);
  }
  return {
    source_branch: requestedWorktree.starting_branch,
    source_tip: requestedWorktree.starting_revision,
    executor_tip: gitOutcome.final_revision,
  };
}

export async function resolvedTaskArchiveAuthority(stateRoot, dispositionId) {
  ...
  if (!["accepted-no-change", "accepted-for-integration"].includes(disposition.decision)) {
    throw new CliError("Blocked or rejected tasks must remain visible", 73);
  }
  const callback = await callbackRecord({ stateRoot, callbackId: disposition.callback_id });
  if (
  ...
  const launch = await taskLaunchStatus({ stateRoot, launchId: callback.receipt.launch_id });
  if (
    launch.status !== "active"
    || launch.start_claim === null
    || launch.git_activation?.state !== "completed"
    || !identityMatches(launch, callback.receipt, CANONICAL_IDENTITY_KEYS.filter(
      (key) => !["coordinator_binding", "executor_thread_id"].includes(key),
    ))
    || !identityMatches(launch.coordinator_binding, callback.receipt.recipient, [
      "lineage_id", "thread_id", "generation", "binding_digest",
    ])
    || launch.start_claim.executor_thread_id !== callback.receipt.executor_thread_id
  ) throw new CliError("Task archive requires the exact authenticated task launch", 73);
  const worktreeAuthority = archiveWorktreeAuthority(launch);
  ...
  if (disposition.decision === "accepted-no-change") {
    ...
    return {
      ...
      launch,
      worktreeAuthority,
      noChangePreservation: noChangePreservation(launch, callback.receipt),
      gitResolution: {
        kind: "unchanged",
        integration_id: null,
  ...
  return {
    ...
    launch,
    worktreeAuthority,
    integrationPreservation: {
      outcome: integration.outcome,
      main_branch: integration.main_branch,
      executor_tip: integration.executor_tip,
      reconciled_main_tip: integration.reconciled_main_tip,
    },
    gitResolution: {
      kind: "integrated",
...
export async function prepareTaskArchive(options) {
  ...
  const authority = await resolvedTaskArchiveAuthority(stateRoot, dispositionId);
  const task = validateTaskObservation(taskObservation, "task observation");
  const preparingHostAction = task.active_visible === true && task.archived_visible === false;
  const reconcilingExistingArchive = task.active_visible === false && task.archived_visible === true;
  if (!preparingHostAction && !reconcilingExistingArchive) {
    throw new CliError("Task archive preparation requires one exact task visibility surface", 73);
  }
  if (preparingHostAction && (
    task.source !== TYPED_HOST_ACTIVITY_SOURCE
    || task.activity_state !== "idle"
  )) {
    throw new CliError("Task archive preparation requires fresh typed idle evidence", 73);
  }
  if (preparingHostAction) {
    const boundary = now instanceof Date ? now.getTime() : now;
    const observedAt = Date.parse(task.observed_at);
    if (
      !Number.isFinite(boundary)
      || Number.isNaN(observedAt)
      || observedAt < boundary - 30_000
      || observedAt > boundary + 5_000
    ) throw new CliError("Task archive preparation idle evidence is stale or future-dated", 73);
  }
  if (reconcilingExistingArchive) {
    const boundary = now instanceof Date ? now.getTime() : now;
    const observedAt = Date.parse(task.observed_at);
    if (
      !Number.isFinite(boundary)
      ...
      || observedAt < boundary - 30_000
      || observedAt > boundary + 5_000
    ) throw new CliError("Existing archive evidence is missing, stale, or future-dated", 73);
  }
  if (task.thread_id !== authority.launch.start_claim.executor_thread_id) {
    throw new CliError("Observed task thread does not match the authenticated launch", 73);
  }
  ...
  const causal = {
    ...
    launch_id: authority.launch.launch_id,
  ...
  const worktreeState = archiveWorktree.management === "none"
    ? "not-applicable"
    : archiveWorktree.prepared_state === "absent" ? "absent" : "present";
  const initiallyCompleted = reconcilingExistingArchive
  ...
  const record = validateArchiveOperation({
    ...
    ...causal,
    setter: reconcilingExistingArchive
      ? { outcome: "ambiguous", reconciled_at: preparedAt }
      : null,
    observation: reconcilingExistingArchive ? {
      task,
      worktree_state: worktreeState,
      observed_at: task.observed_at ?? preparedAt,
    } : null,
    state: reconcilingExistingArchive
      ? initiallyCompleted ? "completed" : "archived-awaiting-worktree-reclamation"
      : "prepared",
  ...
  return withProcessLock({
    ...
    return view(record, { dispatchPermitted: preparingHostAction });
  });
}
...
async function reconciledWorktreeState(worktree) {
  ...
  return await pathPresent(worktree.path) ? "present" : "absent";
}
...
export async function reconcileTaskArchive({
  ...
  return withProcessLock({
    ...
    if (current.state === "completed") {
      ...
      if (worktreeState !== current.observation.worktree_state) {
        throw new CliError("Completed archive worktree postcondition is no longer true", 73);
      }
      return view(current);
    }
    ...
    if (
      current.state === "archived-awaiting-worktree-reclamation"
    ...
    const worktreeState = await reconciledWorktreeState(current.worktree);
    const observedAt = taskObservation.observed_at ?? reconciledAt;
    if (worktreeState === "present") {
      if (current.state === "archived-awaiting-worktree-reclamation") return view(current);
      const next = validateArchiveOperation({
        ...current,
        setter,
        observation: {
...
export async function taskArchiveForDisposition({ stateRoot, dispositionId }) {
  const location = claimPaths(stateRoot, dispositionId);
  const raw = await readJson(location.claim, {
    allowMissing: true,
    guardRoot: guardRoot(stateRoot),
  });
  if (raw === null) return null;
  const claim = validateDispositionClaim(raw);
  if (claim.disposition_id !== dispositionId) {
    throw new CliError("Archive disposition claim identity is invalid", 73);
  }
  return taskArchiveStatus({ stateRoot, archiveId: claim.archive_id });
}
