...
const CLEANUP_KIND = "codex-flow-v09-cleanup-plan";
...
function sameIdentity(left, right) {
  return [
    ...
    "task_id", "task_digest", "contract_id", "launch_id",
...
function originalWorktreePath(launch, archive) {
  if (archive?.worktree?.management === "host-managed") return archive.worktree.path;
  return launch.git_activation?.state === "completed"
    ? launch.git_activation.worktree_path
    : null;
}

function lifecycleAuthority({ launch, callbacks, dispositions, integrations, archives }) {
  const callbackMatch = one(callbacks, (record) => (
    record.receipt.launch_id === launch.launch_id
  ));
  ...
  const authorityConflict = [
    callbackMatch, dispositionMatch, integrationMatch, archiveMatch,
  ].some((entry) => entry.conflict) || [callback?.receipt, disposition, integrationMatch.record, archiveMatch.record]
    .filter(Boolean)
    .some((record) => !sameIdentity(launch, record));
  return {
    callback,
    disposition,
...
function lifecycleReasonCodes(authority, launch) {
  const reasons = [];
  ...
  if (launch.status !== "active" || launch.git_activation?.state !== "completed") {
    reasons.push("launch-not-active");
  }
...
async function cleanupItem({ launch, authority, repositoryRoot, inventory }) {
  ...
  const originalPath = originalWorktreePath(launch, authority.archive);
  ...
  const lifecycleReasons = lifecycleReasonCodes(authority, launch);
  const gitReasons = [];
  ...
  return validateCleanupItem({
    ...
    executor_thread_id: authority.callback?.receipt.executor_thread_id
      ?? launch.start_claim?.executor_thread_id
      ?? null,
    branch,
    ref,
...
function validateCleanupItem(value, label = "cleanup item") {
  requireExactFields(value, {
    required: [
      "run_id", "task_id", "launch_id", "callback_id",
  ...
  const disposition = value.disposition === null ? null : (() => {
    ...
    return {
      decision: requireEnum(value.disposition.decision, [
        "accepted-for-integration", "accepted-no-change", "rejected", "retained-blocked",
  ...
  const archive = value.archive === null ? null : (() => {
    ...
    return {
      state: requireEnum(value.archive.state, [
        "prepared", "accepted-awaiting-observation", "archived-awaiting-worktree-reclamation",
        "completed", "rejected-before-send", "ambiguous",
      ], `${label}.archive.state`),
      worktree_state: value.archive.worktree_state === null
        ? null
        : requireEnum(
          value.archive.worktree_state,
          ["not-applicable", "present", "absent"],
          `${label}.archive.worktree_state`,
        ),
    };
  })();
...
export function cleanupPlanDigest(value) {
  return sha256(stableStringify(cleanupPlanSeed(validateCleanupPlan(value))));
}
...
export function validateCleanupPlan(value) {
  requireExactFields(value, {
    required: [
      ...
      "repository", "branch_fences", "items", "candidate_launch_ids",
      "blocking_launch_ids", "unbound_branch_fences", "blocking_branch_fences",
      "counts", "mutation_performed",
    ],
  }, "v0.9 cleanup plan");
  if (value.schema_version !== 1 || value.kind !== CLEANUP_KIND) {
    throw new CliError("Invalid v0.9 cleanup plan");
  }
  ...
  const items = value.items.map((entry, index) => validateCleanupItem(entry, `cleanup plan items[${index}]`))
    .sort((left, right) => left.launch_id.localeCompare(right.launch_id));
  ...
  const candidateIds = stringList(value.candidate_launch_ids, "cleanup plan candidate_launch_ids");
  const blockingIds = stringList(value.blocking_launch_ids, "cleanup plan blocking_launch_ids");
  ...
  const plan = {
    ...
    items,
    candidate_launch_ids: candidateIds,
    blocking_launch_ids: blockingIds,
  ...
  if (plan.mutation_performed !== false) throw new CliError("v0.9 cleanup plan must be read-only");
  ...
  const expectedCandidateIds = plan.items.filter((item) => item.candidate).map((item) => item.launch_id).sort();
  const expectedBlockingIds = plan.items.filter((item) => item.close_blocked).map((item) => item.launch_id).sort();
  ...
  if (
    stableStringify(expectedCandidateIds) !== stableStringify(plan.candidate_launch_ids)
    || stableStringify(expectedBlockingIds) !== stableStringify(plan.blocking_launch_ids)
...
export async function cleanupPlan({ stateRoot, runId }) {
  ...
  const flowRoot = resolve(commonDir, "codex-flow");
  if (dirname(requestedStateRoot) !== flowRoot || basename(requestedStateRoot) === "") {
    throw new CliError("v0.9 cleanup plan requires an exact versioned state root");
  }
  const safeRunId = requireText(runId, "runId", { max: 128, safeId: true });
  const lifecycle = validateRunLifecycleState(await readJson(resolve(requestedStateRoot, "runs", "lifecycle.json"), {
    guardRoot: commonDir,
  }));
  const run = lifecycle.runs[safeRunId];
  if (!run) throw new CliError(`Unknown v0.9 run: ${safeRunId}`);
  const runtime = validateRuntimeContext(await readJson(resolve(
    requestedStateRoot,
    "contexts",
    `${run.runtime_id}.json`,
  ), {
  ...
  const [launches, callbacks, dispositions, integrations, archives] = await Promise.all([
    listRecords(requestedStateRoot, "task-launches/records", validateTaskLaunchRecord, "task launch records"),
    listRecords(requestedStateRoot, "callbacks/journal", validateCallbackRecord, "callback records"),
    listRecords(requestedStateRoot, "dispositions/records", validateDispositionRecord, "disposition records"),
    listRecords(requestedStateRoot, "integration-lifecycle/records", validateIntegrationRecord, "integration records"),
  ...
  const runLaunches = runRecords(launches);
  const repositoryId = run.binding.repository_hash;
  const foreignAuthority = runLaunches.find((record) => (
  ...
  const items = [];
  for (const launch of runLaunches) {
  ...
  items.sort((left, right) => left.launch_id.localeCompare(right.launch_id));
  ...
  const draft = {
    ...
    items,
    candidate_launch_ids: items.filter((item) => item.candidate).map((item) => item.launch_id).sort(),
    blocking_launch_ids: items.filter((item) => item.close_blocked).map((item) => item.launch_id).sort(),
    unbound_branch_fences: unboundBranchFences,
