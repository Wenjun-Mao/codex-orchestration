...
import { PACKAGE_VERSION } from "./core.mjs";
...
export function gitSnapshot(cwd = process.cwd()) {
  ...
  const porcelain = gitOutput(context.root, [
...
export function gitIsAncestor(cwd, ancestor, descendant) {
  ...
  if (![0, 1].includes(result.status)) {
    throw new CliError(String(result.stderr || result.stdout).trim() || "Unable to inspect Git ancestry");
  }
