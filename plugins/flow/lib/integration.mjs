...
const INTEGRATION_KIND = "codex-flow-v09-serial-integration";
...
function authority(record) {
  return Object.fromEntries([
    ...CANONICAL_IDENTITY_KEYS, "launch_record_digest", "callback_id",
...
export function integrationRecordDigest(value) {
  const record = validateIntegrationRecord(value);
...
export function validateIntegrationRecord(value) {
  requireExactFields(value, {
    required: [
      "schema_version", "kind", "integration_id", "run_id",
      ...
      "executor_thread_id", "launch_record_digest", "callback_id", "receipt_digest",
      "disposition_id", "disposition_record_digest", "main_branch",
  ...
  if (value.schema_version !== 1 || value.kind !== INTEGRATION_KIND) {
    throw new CliError("Invalid v0.9 serial integration record");
  }
  ...
  const record = {
    ...
    launch_record_digest: digest(value.launch_record_digest, "launch_record_digest"),
...
function launchRecordDigest(launch) {
  const {
    dispatch_permitted: ignoredDispatch,
    activation_performed: ignoredActivation,
    execution_permitted: ignoredExecution,
    host_request: ignoredHostRequest,
    ...record
  } = launch;
...
function dispositionRecordDigest(disposition) {
  const {
    unblocks_dependencies: ignoredDependencies,
    safe_to_finalize: ignoredSafe,
    ...record
  } = disposition;
  return sha256(stableStringify(record));
}
...
function assertVerificationIdentity(verification, current, bindings) {
  ...
  const expected = {
    ...
    recipient_binding_digest: current.coordinator_binding.binding_digest,
    executor_thread_id: current.executor_thread_id,
    run_id: current.run_id,
    runtime_context_digest: current.runtime_context_digest,
...
async function boundAuthority({ stateRoot, dispositionId, commonDir }) {
  ...
  const canonicalReceiptKeys = [
    "run_id", "runtime_context_digest", "configuration_digest", "repository_id",
    "common_dir", "plan_id", "revision_digest", "task_id", "task_digest",
    "contract_id", "launch_id", "executor_thread_id",
  ];
  if (
    !identityMatches(disposition, receipt, canonicalReceiptKeys)
    || !identityMatches(disposition.coordinator_binding, receipt.recipient, [
  ...
  const launch = await taskLaunchStatus({ stateRoot, launchId: receipt.launch_id });
  if (launch.status !== "active" || launch.start_claim === null || launch.git_activation?.state !== "completed") {
    throw new CliError("Serial integration requires an active authenticated task launch", 73);
  }
  if (
    !identityMatches(launch, receipt, canonicalReceiptKeys.filter((key) => key !== "executor_thread_id"))
    || launch.start_claim.executor_thread_id !== receipt.executor_thread_id
    || !identityMatches(launch.coordinator_binding, receipt.recipient, [
      "lineage_id", "thread_id", "generation", "binding_digest",
    ])
    || launch.launch_id !== receipt.launch_id
    || launch.common_dir !== commonDir
  ) throw new CliError("Authenticated launch does not match the integration receipt", 73);
  return { disposition, callback, receipt, launch };
}
...
async function readRecord(stateRoot, id) {
  ...
  return validateIntegrationRecord(value);
}
...
async function integrationRecords(stateRoot) {
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    records.push(validateIntegrationRecord(await readJson(
...
export async function runIntegrationRecords({ stateRoot, runId }) {
  const id = requireText(runId, "runId", { max: 128, safeId: true });
  return (await integrationRecords(stateRoot)).filter((record) => record.run_id === id);
}
...
export async function prepareSerialIntegration({
  ...
  if (context.mainBranch === executorBranch) {
    throw new CliError("Integration target branch must differ from the disposable executor branch", 73);
  }
  ...
  const base = {
    ...
    runtime_context_digest: bindings.receipt.runtime_context_digest,
    configuration_digest: bindings.receipt.configuration_digest,
    repository_id: bindings.receipt.repository_id,
    common_dir: context.commonDir,
    coordinator_binding: bindings.launch.coordinator_binding,
    plan_id: bindings.receipt.plan_id,
    revision_digest: bindings.receipt.revision_digest,
    task_id: bindings.receipt.task_id,
    task_digest: bindings.receipt.task_digest,
    contract_id: bindings.receipt.contract_id,
    launch_id: bindings.receipt.launch_id,
    executor_thread_id: bindings.receipt.executor_thread_id,
    launch_record_digest: launchRecordDigest(bindings.launch),
  ...
  return withProcessLock({
    ...
    if (existing) {
      const validated = validateIntegrationRecord(existing);
...
export async function reconcileSerialIntegration({
  ...
  return withProcessLock({
    ...
    if (
      ...
      || launchRecordDigest(bindings.launch) !== current.launch_record_digest
    ...
    const validated = validateIntegrationRecord(reconciled);
