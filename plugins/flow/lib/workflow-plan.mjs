...
function validateWorkflowTask(value, index) {
  ...
  const executionKind = requireEnum(value.execution_kind, ["coordinator", "task-thread", "subagent"], `${label}.execution_kind`);
  ...
  if (["coordinator", "task-thread"].includes(executionKind) && forkTurns !== null) {
    throw new CliError(`${label}.fork_turns must be null for coordinator and task-thread work`);
  }
...
function completedCoordinatorWorkAuthority(value, label) {
  requireExactFields(value, {
    required: [
      "schema_version", "kind", "local_work_id", "run_id",
      "runtime_context_digest", "configuration_digest", "repository_id", "common_dir",
      "coordinator_binding", "plan_id", "revision_digest", "task_id", "task_digest",
      "contract_id", "baseline", "result", "verification", "state", "started_at",
      "completed_at",
    ],
  }, label);
  if (value.schema_version !== 1 || value.kind !== "codex-flow-v097-coordinator-work") {
    throw new CliError(`${label} is not durable coordinator work`);
  }
  if (value.state !== "completed" || value.result === null || value.verification === null) {
    throw new CliError(`${label} must be completed coordinator work`);
  }
  ...
  const localWorkId = requireText(value.local_work_id, `${label}.local_work_id`, {
    max: 128,
    safeId: true,
  });
  if (localWorkId !== `coordinator-work-v1-${sha256(stableStringify(identity))}`) {
    throw new CliError(`${label}.local_work_id does not match its task-contract identity`);
  }
  requireExactFields(value.baseline, {
    required: ["root", "common_dir", "branch", "revision", "cleanliness"],
  }, `${label}.baseline`);
  requireAbsolutePath(value.baseline.root, `${label}.baseline.root`);
  if (
    requireAbsolutePath(value.baseline.common_dir, `${label}.baseline.common_dir`) !== authority.common_dir
    || requireEnum(value.baseline.cleanliness, ["clean"], `${label}.baseline.cleanliness`) !== "clean"
  ) throw new CliError(`${label}.baseline belongs to another repository`);
  requireText(value.baseline.branch, `${label}.baseline.branch`, { max: 256 });
  const baselineRevision = requireConcreteRevision(
    value.baseline.revision,
    `${label}.baseline.revision`,
  );
  requireExactFields(value.result, {
    required: ["kind", "baseline_revision", "final_revision"],
  }, `${label}.result`);
  const result = {
    kind: requireEnum(value.result.kind, ["no-change", "mutation"], `${label}.result.kind`),
    baseline_revision: requireConcreteRevision(
      value.result.baseline_revision,
      `${label}.result.baseline_revision`,
    ),
    final_revision: requireConcreteRevision(
      value.result.final_revision,
      `${label}.result.final_revision`,
    ),
  };
  if (
    result.baseline_revision !== baselineRevision
    || ((result.kind === "no-change") !== (result.baseline_revision === result.final_revision))
  ) throw new CliError(`${label}.result does not match its baseline and classification`);
  requireExactFields(value.verification, {
    required: ["verification_id", "classification", "subject_revision", "checks", "evidence_digest"],
  }, `${label}.verification`);
  if (value.verification.classification !== "PASS") {
    throw new CliError(`${label}.verification must be PASS`);
  }
  const checks = value.verification.checks;
  if (!Array.isArray(checks) || checks.length < 1 || checks.length > 64) {
    throw new CliError(`${label}.verification.checks must contain between 1 and 64 checks`);
  }
  const normalizedChecks = checks.map((check, index) => {
    const checkLabel = `${label}.verification.checks[${index}]`;
    requireExactFields(check, {
      required: ["check_id", "argv", "exit_code", "stdout_digest", "stderr_digest"],
    }, checkLabel);
    if (!Array.isArray(check.argv) || check.argv.length < 1 || check.argv.length > 64) {
      throw new CliError(`${checkLabel}.argv must contain between 1 and 64 arguments`);
    }
    return {
      check_id: requireText(check.check_id, `${checkLabel}.check_id`, { max: 128, safeId: true }),
      argv: check.argv.map((argument, argumentIndex) => requireText(
        argument,
        `${checkLabel}.argv[${argumentIndex}]`,
        { max: 2048 },
      )),
      exit_code: requireInteger(check.exit_code, `${checkLabel}.exit_code`, { min: 0, max: 0 }),
      stdout_digest: requireDigest(check.stdout_digest, `${checkLabel}.stdout_digest`),
      stderr_digest: requireDigest(check.stderr_digest, `${checkLabel}.stderr_digest`),
    };
  });
  if (new Set(normalizedChecks.map((check) => check.check_id)).size !== normalizedChecks.length) {
    throw new CliError(`${label}.verification.checks contains duplicate check IDs`);
  }
  const verificationSeed = {
    classification: "PASS",
    subject_revision: requireConcreteRevision(
      value.verification.subject_revision,
      `${label}.verification.subject_revision`,
    ),
    checks: normalizedChecks,
  };
  if (verificationSeed.subject_revision !== result.final_revision) {
    throw new CliError(`${label}.verification does not verify the coordinator result`);
  }
  const evidenceDigest = requireDigest(
    value.verification.evidence_digest,
    `${label}.verification.evidence_digest`,
  );
  if (
    evidenceDigest !== sha256(stableStringify(verificationSeed))
    || value.verification.verification_id !== `coordinator-verification-v1-${evidenceDigest}`
  ) throw new CliError(`${label}.verification identity does not match its evidence`);
  const startedAt = requireTimestamp(value.started_at, `${label}.started_at`);
  const completedAt = requireTimestamp(value.completed_at, `${label}.completed_at`);
  if (Date.parse(completedAt) < Date.parse(startedAt)) {
    throw new CliError(`${label} completed before it started`);
  }
  return {
    ...
    authority_id: localWorkId,
    authority_digest: sha256(stableStringify(value)),
    result_digest: sha256(stableStringify({ result, verification: verificationSeed })),
  };
}
...
function validateDependencyAuthorities(value, task, plan, authority) {
  ...
  const dependencies = value.map((entry, index) => {
    ...
    const dependency = entry?.kind === "codex-flow-v09-task-disposition"
      ...
      : entry?.kind === "codex-flow-v097-coordinator-work"
        ? completedCoordinatorWorkAuthority(entry, label)
      : entry?.kind === "codex-flow-native-subagent-operation"
        ? acceptedSubagentAuthority(entry, label)
        : (() => { throw new CliError(`${label} must be durable completed workflow authority`); })();
