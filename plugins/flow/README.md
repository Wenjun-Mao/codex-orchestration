...
This product is retained as reference source. Its legacy plugin identifier,
`codex-orchestration`, remains unchanged. Use [Relay](../relay/README.md) for new work.

Historical `vVERSION` tags preserve the original repository-root layout. Use those
tags or their artifacts to reproduce an old release, not this relocated tree.
The instructions below document the retired product, not a recommendation to install it.

## Historical Flow documentation
