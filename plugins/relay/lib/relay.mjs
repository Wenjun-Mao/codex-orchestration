...
export class Relay {
  ...
  creationResponse(control, record) {
    ...
    const skill = fileURLToPath(new URL('../skills/deliver/SKILL.md', import.meta.url));
    const prompt = `Deliver: ${record.outcomeIntent}\nUse Relay's relay:deliver skill from this exact package: ${skill}\nApproved plan: ${record.plan}\nScope: ${record.scope.join(', ')}\nAcceptance: ${record.acceptance}\nRole: ${record.role}; model ${record.selector.model}; thinking ${record.selector.thinking}.\nStart: ${start}\nNo writes before READY. Only the current ticket permits work; stop all source-changing tools before handoff/finish. Checks are read-only. Preserve failures. Emit your real final after source release; do not synthesize native events.`;
