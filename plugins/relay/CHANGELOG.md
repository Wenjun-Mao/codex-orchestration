# Changelog

## 0.1.0 — Source candidate

- Independent Relay package and generated public serial workflow.
- Atomic current source permission, direct verification transfer and explicit recovery.
- Frozen reporting with exact receipt/decision and task-only archival observations.
- Coordinator retirement preserves the recipient until every sequential child report is received.
- Disposable Git/CLI, crash, scope/drift, reporting and packed isolation tests.
- Native adapters, hook delivery, startup measurement, coexistence and pilot remain gates.
